...
| MS-120, MS-126 | MS-128 | l'affermazione che il deposito `repo-locale` esistesse, smentita dall'elenco vuoto: la purga dei pacchetti ne aveva cancellato la registrazione lasciando la cartella |
| PA-003, e MS-129 di riflesso | MS-130 | l'affermazione che chi clona il template non riceva i quattro modelli `_notes`, smentita da `git ls-files`: sono tracciati, e un pattern del `.gitignore` non tocca ciò che è già nell'indice |
| MS-085 | MS-139 | la conclusione che le sottofasi 8.1 e 8.2 non vadano eseguite: resta vera nel suo contesto, ma cambiando motore di Wine il prefix a 32 bit non è più eseguibile e AKABAK va reinstallato in uno a 64 bit |
| MS-140 | MS-141, MS-142 | la sostituzione di Wine con i pacchetti WineHQ, annullata: nessuno dei due rami popola il lato a 32 bit dei prefix e tutti i programmi del progetto sono a 32 bit |
| una mia affermazione in MS-142 stesso | MS-142 | l'attribuzione del messaggio sulla modalità wow64 sperimentale al contatto con Wine 11: i prefix ripristinati dall'archivio, che non l'hanno mai incontrato, producono lo stesso messaggio |
...
| MS-146 | MS-156 | la portata dell'affermazione che una misura raccolta da SSH non sia informativa: era contingente su chi tenesse il posto, e con la sessione dell'utente attiva la stessa lettura da SSH riporta la scheda e una uscita reale |
...
### MS-124 - La regola violata due volte diventa una guardia nel template, con la scappatoia che la prova ha reso necessaria

Perimetro: implementazione nel repository `template-claude-developing` della difesa meccanica richiesta da PA-015, cioè il rifiuto dei tre strumenti tipografici a scrivere sotto `.claude/templates/`, con il suo interruttore di scavalcamento, la prova che lo esercita, la verifica di non vacuità di quella prova, la propagazione della prescrizione sul `cd` nei comandi git e la voce di case-study che registra la lezione. Chiude PA-015 e riduce PA-003.

Legame con il progetto: non serve alcuna fase e lo dichiara apertamente. Serve l'igiene del sistema di lavoro che questo progetto usa, e vive qui perché il difetto è stato osservato qui due volte, mentre la correzione appartiene al template perché è là che gli strumenti hanno la loro origine.

*Perché la correzione non poteva restare in questo repository.* Gli strumenti sotto `tools/` di questo progetto sono copie di quelli del template, e modificarli qui avrebbe prodotto una divergenza in più invece che una correzione: è la stessa ragione per cui `CLAUDE.md` vieta di correggere le copie dei modelli. La modifica è stata quindi fatta nel template, dove gli strumenti sono gli originali, e da lì tornerà qui alla prossima istanziazione. È il verso corretto della propagazione, e questo microstep esiste anche per registrare che la correzione non è stata dimenticata ma spostata.

*Che cosa fa la guardia, e perché parla invece di tacere.* I tre strumenti riconoscono un percorso che attraversa `.claude/templates/` e lo rifiutano, scrivendo il motivo su standard error. La scelta di dichiarare il rifiuto non è cosmetica: una protezione silenziosa produce un file non toccato senza spiegare perché, e chi guarda l'uscita conclude che lo strumento non abbia funzionato. Il rifiuto detto ad alta voce insegna la regola nel momento in cui la si sta violando, che è l'unico momento in cui serve.

*La scappatoia, e il fatto che è stata la prova a renderla necessaria.* La prima versione della guardia rifiutava sempre, e provandola si è visto che sarebbe stata un difetto nuovo: dentro il template quei file non sono copie ma originali, ed è proprio là che vanno corretti. Una guardia senza scavalcamento avrebbe reso impossibile la manutenzione tipografica del template stesso. È stato quindi aggiunto un interruttore esplicito, `--includi-modelli`, che il messaggio di rifiuto non nomina per non invitare a usarlo di riflesso ma che l'aiuto dei comandi documenta. Vale registrare che il difetto è emerso dalla prova e non dalla riflessione: scrivere la guardia e provarla subito ha impedito che una protezione diventasse un impedimento.

*La prova, e perché doveva essere di un tipo diverso dalle altre.* Le prove esistenti della suite tipografica chiamano direttamente la funzione che trasforma il testo, il che è giusto per verificare le regole di trasformazione e sbagliato qui: la guardia non vive in quella funzione ma nella raccolta dei file che la precede, quindi una prova che chiamasse la funzione passerebbe sempre, qualunque cosa la guardia faccia. La prova nuova lancia perciò i tre strumenti come processi separati su un albero temporaneo che contiene un file sotto `.claude/templates/` e uno fuori, e verifica tre cose: che il primo resti intatto, che il secondo venga invece modificato, e che con l'interruttore anche il primo venga modificato. La seconda delle tre è quella che rende la prova non vacua, perché senza di essa una guardia che rifiutasse tutto passerebbe.

*La verifica di non vacuità, e l'errore che ha rivelato.* La regola `prove-che-misurano.md` del template prescrive di rimettere il difetto e osservare che cosa cade. È stato fatto, invertendo il valore restituito dalla guardia in uno dei tre strumenti, e la prova è caduta con il messaggio atteso e su quel solo strumento. Il ripristino successivo è stato però eseguito con `git checkout --` su un file che conteneva anche la guardia appena scritta e non committata, quindi ha riportato il file all'ultimo commit cancellando il lavoro insieme al difetto. Lo ha rilevato la suite stessa, che dopo il ripristino falliva in due controlli invece di nessuno, e la guardia è stata riapplicata. È la dimostrazione pratica della seconda metà di quella regola, cioè che il ripristino si verifica con uno strumento e non a memoria, ed è stata registrata anche nel case-study del template perché là serve a chi legge la regola.

*Che cosa altro è tornato indietro.* Insieme alla guardia è stata propagata al template la prescrizione sul `cd` in testa al blocco dei comandi git quando il repository non è quello aperto nella sessione, che nasce da una richiesta dell'utente di questa sessione. L'altra prescrizione della stessa richiesta, cioè consegnare i comandi nella sola sintassi della macchina in uso, risultava già presente nel template, quindi non è stata duplicata: è stato verificato leggendo il file e non assunto dal fatto di averla scritta qui.

Verificato con: esecuzione dell'autotest interno dei tre strumenti dopo ogni modifica, sempre superato; esecuzione della suite `tools/test-tipografia.py` del template, che passa sette controlli fra cui quello nuovo; prova manuale su un albero temporaneo, con il file sotto i modelli rifiutato con messaggio e quello fuori corretto; conteggio dei file esaminati dai tre strumenti su `.claude/templates` del template, pari a zero senza interruttore e maggiore di zero con esso; reintroduzione del difetto e osservazione della caduta, seguita dal ripristino verificato con la suite.

Esito: fatto. PA-015 è compiuta nel template, e in questo repository si chiuderà alla prossima istanziazione degli strumenti.

### MS-125 - Allineamento al presidio di ripresa del template, e il difetto che l'allineamento stesso ha scoperto

Perimetro: istanziazione in questo progetto dello strumento `tools/verifica-ripresa.py` e della skill `riprendi`, riallineamento di `.claude/PROJECT-SYSTEM.md` alla versione del template, riscrittura della sezione di ripresa in `CLAUDE.md`, rinomina del file di ripresa nella forma canonica, correzione di un difetto di fini riga trovato nel template e propagazione all'indietro dello strumento che lo avrebbe intercettato.

Legame con il progetto: non serve alcuna fase e lo dichiara apertamente. Serve la continuità fra sessioni, che è il presupposto di ogni fase: una sessione nuova che riparte da una fotografia vecchia costruisce su una premessa falsa, e il costo non si paga subito ma alla prima decisione sbagliata presa su quella base.

*Che cosa mancava, e perché è un presidio e non una comodità.* Il template ha introdotto una impronta dello stato di git, registrata come ultimo atto della sessione e riletta come primo atto della successiva. La ragione è che un file di ripresa non aggiornato ha esattamente lo stesso aspetto di uno aggiornato: non c'è modo di distinguerli guardandoli. Una sessione caduta a metà, per un blocco, una compattazione andata male o una finestra chiusa, lascia il progetto in uno stato che quel file non descrive, e la sessione successiva non ha alcun segnale che glielo dica. L'impronta rende meccanica quella distinzione, confrontando commit e forma dell'albero di lavoro registrati alla chiusura con lo stato reale di adesso.

Va detto che cosa il presidio non può fare, perché altrimenti lo si crede più forte di quanto sia. Legge fatti di git e non giudizi, quindi non sa se il lavoro della sessione caduta fosse giusto. E soprattutto non sa se una decisione presa a voce sia stata scritta, perché una decisione non scritta non lascia traccia su disco: è esattamente il buco che la regola `chat-non-e-memoria.md` esiste per prevenire a monte invece di rilevare a valle.

*La prima corsa ha già trovato qualcosa, e senza avere ancora un'impronta.* Lanciato per la prima volta, lo strumento non aveva nulla da confrontare e lo ha dichiarato, ma ha comunque rilevato uno scarto reale per altra via: lo snapshot di memoria dichiarava il commit `dd3d630` mentre `HEAD` era già ad `a7dea2b`, cioè cinque commit più avanti. È il difetto che questo progetto ha già incontrato due volte e che ha descritto nella prosa dello snapshot stesso, cioè che un commit fatto durante la sessione invecchia lo snapshot appena scritto. Averlo trovato da un comando invece che da una lettura attenta è precisamente il guadagno.

*Che cosa è stato istanziato, e che cosa no.* Sono arrivati qui lo strumento `tools/verifica-ripresa.py` e la skill `riprendi`, che è la procedura che ne interpreta l'esito, ed è dichiarata di sola lettura sul codice e non esegue mai comandi di git che scrivano. Il documento `.claude/PROJECT-SYSTEM.md` è stato riportato identico a quello del template, come la convenzione di questo progetto prescrive. La sezione di `CLAUDE.md` che dice da dove partire è stata riscritta per mettere la verifica prima della lettura, e per dichiarare che l'ultimo atto di ogni sessione è registrare l'impronta dopo i commit dell'utente e non prima: registrarla prima produrrebbe alla riapertura un falso positivo, e un presidio che dà falsi positivi è un presidio che si impara a ignorare.

Non è stata istanziata la skill `gate-pacchetti`, ed è una scelta dichiarata: riguarda l'adozione dei pacchetti opzionali e non il tracciamento, quindi verrà portata qui se e quando servirà.

*Il file di ripresa, rinominato e non solo riscritto.* La forma canonica del template è `_notes/RESUME-PROMPT.md` in maiuscolo, mentre qui il file viveva in minuscolo. Su questo filesystem la differenza non cambia nulla a chi apre il file, e ne cambia molto a chi cerca il nome dentro uno strumento o dentro una skill scritta altrove: il nome è una interfaccia, non una etichetta. È stato quindi rinominato con `git mv`, così che la rinomina resti nella storia invece di apparire come una cancellazione seguita da una creazione.

*Il difetto trovato nel template mentre lo si istanziava.* Copiato `PROJECT-SYSTEM.md`, il controllo delle fini riga di questo progetto ha segnalato il file come misto: due righe su duecentonovantatré terminate con il solo avanzamento riga in un file altrimenti a CRLF. Verificato il file originale, il difetto risultava nel template e non nella copia. Entrambi sono stati normalizzati.

La ragione per cui era sopravvissuto vale più del difetto stesso: il template non ha quel controllo. Lo strumento `check-eol.py` è nato in questo progetto il 2026-09-08, quando trovò un difetto reale già entrato nella storia di git, ed è descritto in MS-063; non era però mai tornato indietro. Nessun altro controllo può accorgersene, e la spiegazione è già scritta in `CLAUDE.md` di questo progetto: la convenzione Markdown prescrive di conservare la fine riga di ciascun file, quindi lo strumento che riunisce le righe la rispetta e non ne pretende la coerenza interna, il rendering a video è identico, e la catena tipografica guarda i caratteri e non le interruzioni. Lo strumento è stato quindi portato nel pacchetto degli strumenti del template, accanto a quello di verifica della ripresa, e la lezione è stata aggiunta alla voce di case-study di quella giornata.

Ne discende una osservazione generale su come circolano gli strumenti fra template e progetti istanziati. Il verso che funziona da solo è quello in discesa, perché istanziare è un atto esplicito; il verso in salita non ha alcun automatismo e dipende da qualcuno che si ricordi che una cosa nata quaggiù serve anche lassù. In questo caso sono passati otto giorni e il ritardo ha lasciato un difetto nel file più copiato del template, che è precisamente il posto peggiore dove lasciarlo.

Verificato con: prima corsa di `python tools/verifica-ripresa.py`, che dichiara l'assenza di impronta e segnala lo snapshot più vecchio di HEAD; `git log --oneline` e `git rev-list --count`, che danno cinquantatré commit e origin in pari; `python tools/check-eol.py` sul documento copiato, che lo riporta misto con duecentonovantuno CRLF e due LF, e che dopo la normalizzazione lo riporta interamente a CRLF; stessa verifica sul file del template, prima e dopo; catena di verifica completa del progetto, passata per intero.

Esito: fatto. Il presidio di ripresa è attivo, lo scarto che aveva già prodotto è chiuso, e un difetto del template è stato corretto insieme allo strumento che lo avrebbe intercettato.

### MS-126 - Ricognizione dello stato dell'agente di backup, e un comando consegnato senza dichiarare da quale macchina si esegue

Perimetro: verifica non privilegiata dello stato di Veeam sulla macchina prima di creare il lavoro di backup, accertamento del meccanismo esatto con cui gli strumenti dell'agente rifiutano l'utente non privilegiato, e registrazione di un difetto nella forma con cui ho consegnato il primo comando della sessione. Nessun backup è ancora stato preso.

Legame con il progetto: serve la fase 10 e PA-011, cioè il backup di macchina intera che deve rendere ripetibile in poche ore una ricostruzione costata più di un giorno. Da questa ricognizione dipende la forma esatta del comando di creazione del lavoro, che senza di essa sarebbe scritta per ipotesi invece che per misura.

*Lo stato misurato, che coincide con quello dichiarato alla chiusura precedente.* I tre pacchetti sono `veeam-libs` e `veeam-nosnap` alla versione `6.3.2.1405` e `veeam-release-deb` alla `1.0.11`, tutti in stato `ii`, e il servizio `veeamservice` risponde `active`. Il deposito `/home/veeam-repo` esiste, appartiene a `root:veeam` con permessi `drwxrwxr-x`, ed è vuoto. I volumi sono quelli già noti, cioè `nvme0n1p2` da 74,5 GiB montata su `/` e `nvme0n1p4` da 375,3 GiB montata su `/home`, entrambe ext4 e senza alcuno strato LVM, più la partizione di swap da 14,9 GiB e la partizione EFI da 1 GiB. L'occupazione è di 18 GiB sulla radice e di 14 GiB su `/home`, con 336 GiB liberi sulla destinazione.

Quei due numeri non sono decorazione: sono il controllo di sanità sull'archivio che verrà prodotto. Un archivio molto più piccolo della somma compressa di quei 32 GiB significa che qualcosa non è entrato, e siccome il deposito sta sullo stesso filesystem che il lavoro copia, un archivio che cresce senza fermarsi significa che il deposito sta copiando sé stesso. Entrambi i modi di fallire sono silenziosi se non si guarda la dimensione, ed è per questo che la dimensione va guardata.

*Il meccanismo del rifiuto, che non è quello che la guida lasciava intendere.* La pagina `docs/10-ambiente/veeam-agent-linux.md` dice che gli strumenti dell'agente rifiutano di funzionare da utente non privilegiato, il che è vero ma descrive l'effetto e non la causa. La causa è ordinaria e vale saperla: `veeamconfig` e `veeam` sono due collegamenti simbolici allo stesso binario `/usr/sbin/veeamworker`, che ha permessi `-rwxr-x---` e appartiene a `root:veeam`, mentre l'utente non appartiene al gruppo `veeam`. Il rifiuto arriva quindi dal filesystem prima ancora che il programma parta, e infatti il messaggio è quello della shell e non dell'agente. Ne discende che nemmeno l'aiuto di un sottocomando è leggibile senza privilegi, il che è il motivo per cui la lettura dell'aiuto è un comando dell'utente e non mio.

*Una misura che ha quasi ingannato, e la ragione per cui non lo ha fatto.* Il primo tentativo di leggere l'aiuto passava l'uscita a `head` e stampava subito dopo il valore di `$?`, che ha risposto zero. Quello zero non diceva che il comando fosse riuscito: in una pipeline lo stato restituito è quello dell'ultimo elemento, cioè di `head`, che aveva letto senza problemi le poche righe di errore. Lo stato vero del comando, misurato eseguendolo senza pipeline, è `126`, che in una shell POSIX significa esattamente comando trovato ma non eseguibile. È la stessa lezione che la regola sulle fonti non recuperabili trae dal successo vuoto di una pagina anti-bot, cioè che l'esito di un comando si verifica sul suo effetto e non sul codice che la shell riporta per ultimo.

*Il difetto della consegna, che non sta nel comando ma nel presupposto.* Il comando che ho consegnato per leggere l'aiuto era scritto per la postazione Windows e si apriva con `ssh -t studio`, ma non dichiarava da quale macchina andasse eseguito. L'utente lo ha incollato in un terminale già aperto sulla macchina di destinazione, dove l'alias `studio` non esiste perché è definito nel file di configurazione SSH della postazione, e la risposta è stata `ssh: Could not resolve hostname studio: Temporary failure in name resolution`. Il comando era corretto, la macchina era quella giusta, e il blocco era comunque sbagliato.

*Perché questa causa è nuova rispetto a quelle già censite, ed è la più difficile da vedere.* La regola `git-commands-format.md` prescrive già che un blocco dichiari il proprio punto di esecuzione, e la sezione che il template sta scrivendo in queste ore ne elenca quattro cause osservate, cioè il ramo, lo stato dell'albero, la cartella e le variabili d'ambiente. Tutte e quattro riguardano lo stato di una shell sulla stessa macchina. Questa è una quinta e riguarda la macchina stessa, che le altre quattro davano per scontata: un alias SSH non è una proprietà della rete ma del file di configurazione di un singolo computer, quindi un blocco che lo nomina è eseguibile su una macchina sola e su nessun'altra, e il nome dell'alias non lo dice. La conseguenza operativa è che quando un lavoro si svolge su due macchine, il blocco dichiara su quale delle due va incollato prima ancora di dichiarare in quale cartella.

Il fallimento è stato benigno, e va detto perché altrimenti la lezione sembra sproporzionata: la risoluzione del nome fallisce rumorosamente e nessuno la scambia per un successo. La stessa forma di errore con un alias che sulla seconda macchina esistesse e puntasse altrove sarebbe invece silenziosa, ed è quella la ragione per cui la regola vale a prescindere dal danno osservato questa volta.

Verificato con: `hostname`, `whoami` e `hostname -I` attraverso l'alias, che rispondono `alessio-ubuntustudio`, `alesop95` e `192.168.10.204`, cioè lo stesso host il cui prompt compare nell'errore dell'utente; `dpkg -l` filtrato su veeam, che riporta i tre pacchetti in stato `ii`; `systemctl is-active veeamservice`, che risponde `active`; `lsblk -o NAME,SIZE,TYPE,FSTYPE,MOUNTPOINT` e `df -hT`, per volumi e occupazione; `ls -ld` e `ls -la` su `/home/veeam-repo`, che lo riportano esistente, di `root:veeam` e vuoto; `ls -l` su `/usr/bin/veeamconfig`, `/usr/bin/veeam` e `/usr/sbin/veeamworker`, che riportano i due collegamenti e il binario `-rwxr-x--- root:veeam`; `id`, che non elenca il gruppo `veeam`; esecuzione diretta senza pipeline, che restituisce stato `126`.

Esito: fatto per la ricognizione e per la diagnosi del difetto di consegna. Il comando è stato riconsegnato nella forma locale, da eseguire sulla macchina dove l'utente già si trova. La creazione del lavoro di backup resta da fare.

### MS-127 - Lo scarto dal template si è riaperto in cinque giorni, e una delle regole divergenti è fra quelle che si caricano sempre

Perimetro: censimento dello scarto fra questo progetto e `template-claude-developing` al 2026-09-17, eseguito su domanda esplicita dell'utente. È una lettura e un confronto, non una propagazione: nulla è stato copiato né qui né là.

Legame con il progetto: nessuna fase del progetto lo richiede, e lo dichiaro apertamente perché un legame inventato è peggio di un legame assente. Riguarda l'igiene del sistema di lavoro e non la progettazione dei diffusori, e il suo esito è una azione differita e non un intervento.

*Il fatto da cui parte tutto.* Il template è a `2936129` del 2026-09-17, cioè di oggi, e ha prodotto sei commit dal 2026-09-15 in poi. Questo progetto è stato riallineato il 2026-09-12 con MS-091, e poi parzialmente il 2026-09-16 con MS-124 e MS-125, che hanno però percorso il verso opposto, cioè hanno portato al template la guardia sui modelli e il controllo sulle fini riga nati qui. Il verso discendente non è stato percorso da cinque giorni, e nel frattempo il template è cresciuto.

*Che cosa manca qui, in ordine di quanto pesa.* Pesa più di tutto una regola che qui non esiste affatto, cioè `.claude/rules/prove-che-misurano.md`, nata nel template il 2026-09-16 e rifinita oggi, che raccoglie i tre modi documentati in cui una prova passa senza misurare niente. Pesa quasi altrettanto un paragrafo aggiunto oggi a `interaction-style.md`, che qui è una delle due regole da caricare sempre: introduce l'unica eccezione al divieto di grassetto, cioè l'etichetta di campo a inizio paragrafo in un documento a record fissi, e la distingue dall'enfasi con quattro condizioni verificabili. Una regola che si carica a ogni sessione e che diverge dalla sua sorgente è il caso peggiore, perché la divergenza si applica ogni volta senza che nessuno la veda.

Vengono poi i quattro strumenti tipografici, che qui sono di nuovo indietro: `fix-accents.py` ha 688 righe contro 744, `fix-dashes.py` 321 contro 371, `fix-missing-accents.py` 542 contro 707 e `test-tipografia.py` 278 contro 497. Dentro quello scarto c'è anche la guardia di PA-015, cioè il rifiuto di scrivere sotto `.claude/templates/` che è nato da un errore commesso qui, è stato costruito nel template, e non è ancora tornato indietro: finché non torna, l'errore che quella guardia impedisce resta possibile proprio nel progetto che lo ha commesso due volte.

Restano infine il pacchetto `hooks-starter`, che nel template ha ora quattro hook di ciclo con una cascata fra loro e non è mai stato istanziato qui, e due strumenti nuovi nel pacchetto `tools` del template, cioè `check-catalogo.py` e `check-copie-modelli.py`, il secondo dei quali sembra fatto proprio per misurare lo scarto che questo microstep ha misurato a mano. La skill `gate-pacchetti` resta fuori per scelta dichiarata in `CLAUDE.md` e non conta come scarto.

*Una osservazione che vale come prova che l'eccezione nuova non è teorica.* Il commit di oggi del template, insieme al paragrafo che introduce l'eccezione sul grassetto, toglie un grassetto dalla sezione 9 di `PROJECT-SYSTEM.md`, dove era enfasi in prosa e non etichetta di campo. La regola è stata quindi applicata alla sua stessa sorgente nello stesso atto in cui veniva scritta, che è il modo in cui una convenzione si dimostra applicabile invece di essere soltanto dichiarata.

*Perché non ho propagato nulla adesso.* La propagazione discendente tocca una regola caricata sempre e quattro strumenti usati dalla catena di verifica prima di ogni commit, quindi cambia il comportamento della sessione mentre la sessione ha un lavoro aperto, cioè il backup. Farla adesso mescolerebbe due giri di lavoro e renderebbe ambiguo, se qualcosa andasse storto, quale dei due lo abbia causato. La sequenza giusta è chiudere PA-011 e poi riallineare in un giro dedicato, con la catena di verifica completa eseguita dopo.

Verificato con: `git log --oneline` nel template, che riporta `2936129` del 2026-09-17 come ultimo commit e sei commit dal 2026-09-15; `git show --stat` su `2936129`, `37678bd`, `7f17c67` e `027757f`, per i file toccati; `git show` sui diff di `interaction-style.md` e `PROJECT-SYSTEM.md`, che riportano il paragrafo aggiunto e il grassetto rimosso; `ls .claude/rules/` qui, che elenca otto regole e non contiene `prove-che-misurano.md`; confronto file per file con `diff -q` fra `tools/` di questo progetto e `.claude/templates/fix-typography/tools/` del template, con il conteggio delle righe di ciascuno.

Esito: aperto. Lo scarto è misurato e non chiuso, ed è tracciato come PA-016.

### MS-128 - La purga dei pacchetti del modulo ha azzerato la configurazione dell'agente, e la cartella del deposito sopravvissuta lo nascondeva

Perimetro: accertamento che l'accettazione dei contratti e la registrazione del deposito non siano sopravvissute alla purga e reinstallazione di MS-121 e MS-122, lettura dell'aiuto di `job create fileLevel`, e ritiro di una affermazione scritta poche ore prima in MS-126. Supera in parte MS-120.

Legame con il progetto: serve la fase 10 e PA-011. Da questo accertamento dipende il fatto che la creazione del lavoro debba essere preceduta dalla ricreazione del deposito, che senza di esso sarebbe fallita con un errore di deposito inesistente.

*Il sintomo, e il sospetto che ne è nato.* Il primo comando privilegiato della giornata ha chiesto di nuovo l'accettazione dei due contratti, che MS-119 dava per compiuta il 2026-09-16. Il sospetto immediato era che la sequenza di MS-121 e MS-122, cioè la purga di `blksnap`, `veeam` e `veeamsnap` seguita dalla reinstallazione di `veeam-nosnap`, avesse portato via con sé lo stato dell'agente. Il sospetto valeva più dell'accettazione in sé, perché se lo stato era andato perduto allora anche il deposito creato in MS-120 non esisteva più come registrazione.

*La conferma, che è netta.* Accettati i due contratti, `veeamconfig repository list` stampa la sola riga di intestazione e nessun deposito, e `veeamconfig job list` fa lo stesso. La configurazione dell'agente è vuota: non c'è `repo-locale`, non c'è alcun lavoro, e il 16 settembre esistevano entrambe le cose.

*Il meccanismo, ricostruito dal registro del gestore dei pacchetti e non per ipotesi.* Il registro riporta `remove veeam-nosnap` alle 10:24 del 16 settembre, che è l'installazione della variante con modulo a scalzare quella senza, poi `remove veeamsnap` alle 10:28, e infine `remove veeam`, `purge veeam` e `purge veeamsnap` alle 10:31. Le cartelle `/etc/veeam` e `/var/lib/veeam` portano come data di ultima modifica esattamente quelle 10:31. La variante senza modulo non è mai stata purgata, solo rimossa, ma questo non l'ha protetta: le due varianti condividono le stesse cartelle di stato, quindi la purga dell'una ha cancellato la configurazione dell'altra.

Ne discende una regola che vale oltre Veeam, perché il caso si presenta ogni volta che due pacchetti alternativi implementano lo stesso servizio. Purgare uno dei due distrugge lo stato condiviso anche quando l'altro è quello che si intende conservare, e la rimozione semplice non basta a distinguere le due situazioni. Dopo un ciclo di purga e reinstallazione la configurazione va riverificata con i comandi del programma, e va riverificata sempre, perché non c'è nulla nell'esito dell'installazione che segnali la perdita.

*Il ritiro, ed è mio e di poche ore fa.* In MS-126 ho scritto che il deposito `/home/veeam-repo` esiste, appartiene a `root:veeam` ed è vuoto, e nel riepilogo in sessione ho detto all'utente che il deposito esisteva. La prima affermazione è vera e riguarda una cartella; la seconda è falsa e riguarda una registrazione. Ho letto la cartella come prova del deposito, e non lo è: la purga rimuove la configurazione ma non tocca la cartella dei dati, quindi resta sul disco una directory con il nome giusto, i permessi giusti e nessun deposito dietro. È la stessa forma di errore del successo vuoto già registrata nella regola sulle fonti non recuperabili, cioè un riscontro che ha tutta l'apparenza di quello cercato e misura un'altra cosa.

La forma corretta della verifica è chiedere al programma e non al filesystem. `veeamconfig repository list` è l'unica misura che distingue un deposito registrato da una cartella omonima, e costa esattamente quanto un `ls`.

*Che cosa l'aiuto ha confermato, dato che è stato catturato prima che il comando si appendesse.* Il sottocomando `job create fileLevel` accetta davvero `--name`, `--repoName`, `--includeDirs`, `--excludeDirs`, `--maxPoints` e `--nosnap`, quindi il comando preparato nel file di ripresa era sintatticamente valido. Accetta inoltre `--indexAll`, `--compressionLevel`, `--blockSize` e una serie di opzioni di pianificazione. Non accetta `--backupAllSystem`, che appartiene al livello di volume, e questa assenza è la conferma per via indipendente di ciò che MS-120 aveva accertato per rifiuto.

Resta non risolto dall'aiuto un punto che conta: `--includeDirs` è documentato soltanto come elenco di directory separate da virgola, e non dichiara se l'attraversamento si fermi al confine del filesystem. Poiché `/home` è una partizione distinta dalla radice ed è la parte cara del backup, la domanda non si lascia aperta: i due percorsi vanno passati entrambi in modo esplicito e l'inclusione va verificata sulla configurazione del lavoro e sulla dimensione dell'archivio, non presunta.

Verificato con: `sudo veeamconfig repository list` e `sudo veeamconfig job list`, entrambi con la sola intestazione e nessuna riga; richiesta di accettazione dei due contratti ricomparsa prima di essi; `grep` su `/var/log/dpkg.log` filtrato su veeam, che riporta le cinque operazioni del 16 settembre con i loro orari; `ls -ld` su `/etc/veeam` e `/var/lib/veeam`, entrambe con data di modifica 16 settembre 10:31; lettura dell'aiuto catturato in `~/diag/veeam-prep.txt`, 3109 byte, completo fino alla riga delle opzioni e troncato al punto in cui il comando successivo ha chiesto l'accettazione.

Esito: fatto per l'accertamento. La conseguenza operativa è che il deposito va ricreato prima del lavoro, e il ritiro dell'affermazione di MS-126 è compiuto qui e non altrove.

### MS-129 - Riallineamento discendente al template, con due controlli nuovi e la guardia che finalmente è arrivata dove era nata

Perimetro: propagazione dal template a questo progetto di una regola nuova, di tre modifiche a regole esistenti, della sezione 9 di `PROJECT-SYSTEM.md`, dei quattro strumenti tipografici, di due controlli nuovi e di sei pacchetti di modelli mancanti; istanziazione dei due controlli nella catena di verifica; e aggiunta qui di una quinta causa alla sezione sul contesto di shell, che nel template non c'è. Esegue PA-016, aperta poche ore prima con MS-127.

Legame con il progetto: nessuna fase del progetto lo richiede, e lo dichiaro apertamente. Riguarda il sistema di lavoro e non i diffusori, ed è stato eseguito su istruzione esplicita dell'utente ripetuta due volte, la seconda dopo che avevo proposto di rimandarlo a dopo il backup. La proposta di rimandare resta motivata e la decisione dell'utente la supera: sono due giri di lavoro distinti nello stesso pomeriggio, e la separazione è garantita dal fatto che il backup non era ancora partito quando questo è cominciato.

*Che cosa è arrivato, e perché le prime due voci pesano più delle altre.* È arrivata `.claude/rules/prove-che-misurano.md`, che qui non esisteva affatto: raccoglie i tre modi documentati in cui una prova passa senza misurare niente, cioè una prova che sceglie i propri argomenti e può sceglierli gentili, una prova su funzione pura che non dimostra che il risultato arrivi a destinazione, e la verifica di ciò che si consegna quando chi riceve interpreta. È arrivato il paragrafo che introduce l'unica eccezione al divieto di grassetto in `interaction-style.md`, cioè l'etichetta di campo a inizio paragrafo in un documento a record fissi, riconoscibile da quattro condizioni tutte necessarie. Queste due pesano più delle altre perché `interaction-style.md` è una delle due regole che questo progetto carica sempre, e una regola caricata sempre che diverge dalla sua sorgente applica la divergenza a ogni risposta senza che nessuno la veda.

Sono poi arrivate la riscrittura della sezione sul formato dei comandi in `git-commands-format.md`, che ora prescrive la sola sintassi della macchina in uso con due eccezioni dichiarate, la sezione nuova sul contesto di shell con le sue quattro cause, il paragrafo sulle due vie per trascrivere un video in `web-sources-not-fetchable.md`, e la sezione 9 di `PROJECT-SYSTEM.md`, che ora richiama la regola sulle prove e chiede di verificare che una prova sarebbe caduta se il difetto fosse rimasto.

*Gli strumenti, dove lo scarto era anche un difetto e non solo un ritardo.* I quattro strumenti tipografici sono stati portati alle versioni del template, ed erano indietro di 56, 50, 165 e 219 righe. Dentro quello scarto c'erano tre cose e non una. La guardia di PA-015, cioè il rifiuto di scrivere sotto `.claude/templates/`, era nata da un errore commesso in questo repository, era stata costruita nel template e non era mai tornata indietro: fino a oggi l'errore che quella guardia impedisce restava possibile proprio nel progetto che lo aveva commesso due volte. Il rispetto del marcatore `.md-unwrap-ignore` da parte dei tre strumenti tipografici, che prima lo ignoravano mentre `md-unwrap` lo onorava, cioè una protezione dichiarata che due strumenti su cinque non vedevano. E il rientro di `--check`, che è il difetto più grave dei tre perché rendeva vacuo un controllo: lo strumento usciva zero anche elencando i file da correggere, quindi chiunque lo concatenasse in una catena di verifica riceveva un via libera indistinguibile da quello vero.

*I due controlli nuovi, e il buco che il primo chiude.* Lo strumento `check-copie-modelli.py` confronta byte per byte ogni strumento istanziato con il proprio modello sotto `.claude/templates/`, ed entra nella sequenza obbligatoria prima di un commit. Il buco che chiude non si manifesta nel repository dove nasce: si corregge la copia sotto `tools/` perché è quella che gira, il modello resta indietro, tutte le prove passano perché girano sulle copie, e il difetto compare soltanto nel prossimo progetto allineato sotto forma di uno strumento privo di una protezione che la documentazione dichiara presente. Alla prima corsa qui ha trovato cinque divergenze, di cui una inattesa e non tipografica, cioè `.claude/agents/security-auditor.md`, dove la copia istanziata portava una parola senza accento che il modello ha corretto. Lo strumento `check-catalogo.py` verifica invece che il catalogo dei pacchetti descriva le cartelle presenti sul disco, e alla prima corsa non riusciva nemmeno a leggere il catalogo di questo progetto, fermo a un formato precedente alla riorganizzazione in settori: dopo l'allineamento dichiara settantaquattro voci in dieci settori e nessun problema. Resta fuori dalla sequenza obbligatoria perché qui il catalogo è una copia e non la sorgente.

*Che cosa ho aggiunto qui e non ho preso da là.* La sezione sul contesto di shell elencava quattro cause, tutte riguardanti lo stato di una shell su un computer dato. Il fallimento di stamattina, registrato in MS-126, è una quinta causa e riguarda quale sia il computer: un alias SSH non è una proprietà della rete ma una riga nel file di configurazione di una singola macchina, quindi un blocco che lo nomina è eseguibile su una macchina sola e il suo nome non lo dice. Il paragrafo è stato scritto qui e va portato al template, insieme alla negazione nel `.gitignore` che era già pendente.

*La verifica che la guardia non sia soltanto presente ma efficace, che è il punto della regola appena arrivata.* Non basta che i file siano stati copiati: `prove-che-misurano.md` chiede di accertare che il controllo cadrebbe se il difetto fosse tornato. Due misure. Lanciando `fix-accents.py --check` su un file sotto `.claude/templates/`, lo strumento risponde che lo rifiuta perché sta sotto quella cartella ed esamina zero file, quindi la guardia agisce e lo dichiara invece di tacere. Lanciando `fix-dashes.py --check` su un file di prova che contiene un trattino lungo, l'uscita è `1`, mentre sullo stesso comando applicato a un file senza difetti è `0`: il rientro misura davvero, e prima dell'allineamento sarebbe stato zero in entrambi i casi.

Verificato con: `diff` file per file fra questo progetto e il template su regole, skill, `PROJECT-SYSTEM.md` e strumenti, prima e dopo; `python tools/check-copie-modelli.py`, che prima riportava cinque copie divergenti e ora quattordici allineate e zero divergenti; `python tools/check-catalogo.py`, che prima non leggeva il catalogo e riportava ventuno problemi, poi sei voci senza cartella, e ora zero problemi su settantaquattro voci in dieci settori; catena di verifica completa del progetto, cioè `md-unwrap --check` su 175 file, `lint-md-commands` con zero errori, `check-eol` su 261 file con zero misti, `test-tipografia` con zero controlli falliti, `sync-ambiente --check` senza deriva e `check-pending-actions` con uscita zero; le due misure di non vacuità descritte sopra.

Esito: fatto per la parte documentale, che è quella su cui l'istruzione dell'utente verteva. Resta aperta in PA-016 la sola decisione sul pacchetto `hooks-starter`, che non è un travaso ma una scelta, e restano da portare al template le due cose nate qui.

### MS-130 - Le due voci ascendenti portate al template, e una di esse si è rivelata poggiare su un fatto falso

Perimetro: propagazione al template delle due cose nate qui, cioè la negazione nel `.gitignore` sui modelli `_notes` e la quinta causa nella sezione sul contesto di shell; ritiro di una affermazione presente in PA-003 dal 2026-09-12 e ripetuta da me in `CLAUDE.md` poche ore fa; accertamento di una terza voce discendente comparsa nel template durante questa stessa sessione. Esegue in parte PA-003.

Legame con il progetto: nessuna fase del progetto lo richiede, e lo dichiaro apertamente. Riguarda il sistema di lavoro condiviso fra questo progetto e il template, ed è stato eseguito su istruzione esplicita dell'utente.

*Il ritiro, che è la parte che conta più della propagazione.* PA-003 dichiarava dal 2026-09-12 che nel template i modelli `_notes` restano esclusi senza eccezione, e quindi che chi clona il template non riceve `DIARIO.md`, `RESOCONTO.md`, `RESUME-PROMPT.md` e `TEST-CHECKLIST.md`. Stamattina ho ripetuto la stessa affermazione riscrivendo la sezione degli standard di `CLAUDE.md`. È falsa, e la misura è immediata: `git ls-files` sul template elenca tutti e quattro i file, quindi sono tracciati e un clone li riceve.

Il meccanismo per cui l'affermazione sembrava vera vale più dell'errore, perché è generale. Un pattern di `.gitignore` governa i soli file non tracciati: un file già presente nell'indice sopravvive a un pattern che lo coprirebbe, e continua a essere versionato come se il pattern non ci fosse. I quattro modelli erano entrati nella storia prima che la regola esistesse, quindi il difetto non si è mai manifestato, ed è precisamente per questo che è stato dedotto leggendo il `.gitignore` invece che misurato interrogando git.

C'è una seconda trappola, ed è nello strumento con cui si verifica. Il comando `git check-ignore` non riporta come ignorati i file tracciati, quindi interrogarlo su uno dei quattro modelli risponde che non è ignorato e non distingue fra le due cause possibili, cioè una negazione che lo salva e una tracciatura che lo protegge. La domanda giusta si pone con `--no-index` e su un percorso che non esiste ancora, perché è il file futuro e non quello presente il soggetto della regola.

*Che cosa resta vero, e perché la negazione va aggiunta lo stesso.* Cambia la motivazione e non la conclusione. La negazione non ripara un danno in corso: previene quello futuro, cioè un modello aggiunto domani sotto `.claude/templates/_notes/` che sparirebbe in silenzio e che `git add` rifiuterebbe senza `-f`, senza che nessuno colleghi il rifiuto a una riga scritta anni prima. La seconda negazione, quella su `.claude/templates/community-sources/_notes/`, riguarda un percorso che oggi contiene soltanto una cartella `fonti` vuota, e siccome git non traccia le cartelle vuote nemmeno lì si stava perdendo qualcosa: vale anch'essa come prevenzione e non come riparazione. La voce di PA-003 è stata riscritta su questa base invece di essere cancellata.

*La quinta causa, e l'unica modifica fatta nel passaggio.* Il paragrafo sulla macchina è stato portato al template con una sola differenza rispetto alla versione scritta qui: l'alias concreto `studio` è diventato `<alias>`. La ragione è che il template non deve nominare la macchina di un singolo progetto, e un esempio con un nome reale invita a copiarlo invece che a sostituirlo. La sezione dichiara ora cinque cause e l'elenco finale del contesto di una shell si apre con la macchina.

*La terza voce, comparsa mentre lavoravo.* Il template ha ricevuto oggi il commit `4b7411d`, che aggiunge a `git-identity-and-repo.md` un quarto asse di identità, cioè la GitHub CLI con il suo token OAuth su HTTPS distinto dalla chiave SSH, e nello stesso atto de-istanzia quel file portandolo da un elenco di profili concreti di una macchina a una procedura di rilevamento. Qui quel file è invece nella forma istanziata, con gli alias, le chiavi e le due identità reali di questa postazione, quindi l'allineamento non è una copia ma una re-istanziazione, ed è lavoro diverso da quello di MS-129. È stato aggiunto a PA-016 invece di essere improvvisato adesso.

Vale registrare la circostanza in sé, perché è la conferma sul campo di quanto MS-127 diceva in astratto: lo scarto dal template si è riaperto durante la sessione che lo stava chiudendo, e il verso discendente non ha alcun automatismo.

Verificato con: `git ls-files .claude/templates/_notes/` nel template, che elenca i quattro modelli, quindi tracciati; `git check-ignore -v --no-index` su un percorso inesistente sotto `.claude/templates/_notes/`, che prima della modifica lo dichiarava ignorato e dopo esce con codice 1, cioè non ignorato; lo stesso comando su un percorso sotto `_notes/` alla radice, che continua a uscire con codice 0 nominando la riga 6 del `.gitignore`, quindi la negazione è stretta e non ha disattivato la regola generale; `git status --short` nel template, che riporta i due soli file attesi; `md-unwrap --check` e `check-eol` di questo progetto lanciati sul file di regola del template, entrambi puliti; `git log --oneline -3` nel template, che riporta `4b7411d` come commit nuovo della giornata.

Esito: fatto sul disco del template, non nella sua storia. Il commit è dell'utente, e va notato che il template è sul ramo `pacchetto-allineamento` e non su `main`, quindi la propagazione non si chiude con un commit ma con una unione, come PA-003 già dichiarava dal 2026-09-14.

### MS-131 - Il deposito ricreato, il lavoro a livello di file creato e la prima corsa riuscita in sette minuti

Perimetro: ricreazione del deposito perduto con la purga, creazione del lavoro di backup a livello di file, sua prima esecuzione e verifica dell'esito dichiarato e delle dimensioni prodotte. Non comprende la prova di ripristino, che è il criterio di completamento di PA-011 e resta da fare.

Legame con il progetto: serve la fase 10 e PA-011. Da questo microstep dipende la prova di ripristino, che senza un archivio non ha oggetto, e dipende il trasferimento su `J:`.

*Che cosa è stato eseguito, nell'ordine in cui è stato eseguito.* Il deposito `repo-locale` è stato ricreato su `/home/veeam-repo`, di tipo locale, e il comando ha risposto con una conferma esplicita. Il lavoro è stato poi creato come `backup-file-sistema`, di tipo `FileLevel`, sul deposito appena ricreato, e ha ricevuto l'identificativo `{c1a02b69-50be-46ae-b0e6-fb30b1fe37b3}`. I percorsi inclusi sono due, cioè `/` e `/home`, e le esclusioni nove, cioè `/home/veeam-repo`, i quattro filesystem virtuali `/proc`, `/sys`, `/dev` e `/run`, più `/tmp`, `/snap`, `/mnt` e `/media`. Il lavoro è stato creato senza alcuna opzione di pianificazione, quindi resta manuale, e con `--maxPoints 2` e `--nosnap`.

*Perché i percorsi inclusi sono due e non uno.* L'aiuto del sottocomando, letto in MS-128, descrive `--includeDirs` soltanto come elenco di directory separate da virgola e non dichiara se l'attraversamento si fermi al confine del filesystem. Su questa macchina `/home` è una partizione distinta dalla radice e contiene i quattro prefix di Wine, cioè la parte del sistema che costa due settimane a ricostruire. Passare entrambi i percorsi rende la domanda irrilevante: se l'attraversamento avviene il secondo è ridondante e innocuo, se non avviene è indispensabile. Resta da verificare per misura che `/home` sia davvero dentro l'archivio, e la misura è la prova di ripristino.

*L'esito, con i numeri.* La sessione `{4118064a-d6fb-4eaa-bb74-46bf99bf7b56}` è partita alle 15:45 e si è chiusa alle 15:52, sette minuti, con stato dichiarato `Success`. Il backup risultante ha identificativo `{887efd5d-a539-407e-86f5-1df674d3115f}` e ha prodotto due file nel deposito, cioè `backup-file-sistema_2026-09-17T154511.vbk` da 17 GiB e il descrittore `backup-file-sistema.vbm` da 11 KiB. Il deposito occupa in tutto 17 GiB, e l'occupazione di `/home` è passata da 14 a 31 GiB.

Il rapporto fra sorgente e archivio è la prima delle due verifiche di sanità: 32 GiB di sorgente, cioè 18 sulla radice e 14 su `/home`, contro 17 GiB di archivio, cioè poco più della metà. È una compressione plausibile per un filesystem di sistema, dove buona parte del contenuto è già compresso in partenza. La seconda verifica è che l'occupazione si sia fermata: il deposito vive sullo stesso filesystem che il lavoro copia, quindi se l'esclusione di `/home/veeam-repo` non avesse tenuto l'archivio avrebbe continuato a crescere finché il disco non si fosse riempito. Si è fermato, quindi l'esclusione ha tenuto.

*Una osservazione sui permessi, che non è un difetto adesso ma lo diventa al passo successivo.* Il file di archivio ha permessi `-rw-rw-rw-`, cioè leggibile e scrivibile da chiunque, e appartiene a `root:veeam`. Non è un problema dove si trova, perché la cartella che lo contiene è `drwxrws---` e nega ogni accesso a chi non appartiene al gruppo `veeam`: la protezione effettiva viene dalla directory e non dal file, ed è una distinzione che si dimentica guardando il solo `ls` del file.

Diventa invece una questione al passo successivo. L'archivio contiene l'intero sistema, quindi anche `/etc/shadow` con le impronte delle password, e la destinazione `J:` è un disco esterno con un filesystem che non conserva permessi POSIX: là il file sarà leggibile da chiunque abbia il disco in mano, e la protezione che qui viene dalla directory non esiste. Il sottocomando di creazione del lavoro espone `--setEncryption`, quindi la cifratura è disponibile e va decisa prima del trasferimento e non dopo, perché cifrare richiede di rifare il backup e non di cifrare l'archivio esistente.

Verificato con: `veeamconfig repository create`, che risponde con conferma esplicita; `veeamconfig job create fileLevel`, idem, e `veeamconfig job list`, che riporta il lavoro con nome, identificativo, tipo `FileLevel` e deposito; `veeamconfig job start`, che restituisce l'identificativo di sessione e il percorso dei registri; `ps` filtrato sui processi di sessione, che a corsa finita non ne trova alcuno; `veeamconfig session list`, che riporta stato `Success` con gli orari di inizio e fine; `veeamconfig backup list`, che riporta l'identificativo del backup; `du -sh` sul deposito e `ls -lh` sul suo contenuto, per le dimensioni e i permessi; `df -h /home` prima, durante e dopo, che riporta 14, 28 e 31 GiB occupati.

Esito: fatto per il backup, che è la metà del lavoro. PA-011 resta aperta, perché il suo criterio di completamento non è che l'archivio esista ma che sia stato riletto.

### MS-132 - L'archivio riletto dall'interno: `/home` c'è, le impronte coincidono e l'esclusione del deposito è verificata per assenza e non per dimensione

Perimetro: montaggio del punto di ripristino, verifica che `/home` sia dentro l'archivio, confronto per impronta di sei file fra sistema vivo e archivio, conteggio dei file nei quattro prefix di Wine, e ispezione delle cartelle escluse. È la prova di ripristino parziale che PA-011 pone come criterio di completamento.

Legame con il progetto: serve la fase 10 e PA-011, e chiude la domanda lasciata aperta da MS-128 e MS-131 sull'attraversamento del confine di filesystem da parte di `--includeDirs`.

*Come si è guardato dentro l'archivio, e perché con questo sottocomando e non con un altro.* Il punto di ripristino `{0d0ce0b6-744d-4c97-ac5e-8156f9ea7049}`, di tipo `Full`, è stato montato con `veeamconfig point mount`, che espone il contenuto come un filesystem percorribile senza scrivere nulla sul sistema vivo. Dei quattro sottocomandi disponibili è il solo adatto: `restore` riscrive volumi di dati ed è precisamente ciò che non si fa su una macchina funzionante per provare un backup, `export` produce dischi virtuali che servono a portare l'archivio altrove e non a ispezionarlo, e `list` era già stato usato. Il montaggio avviene su `/mnt/backup`, che è il valore predefinito, e la sessione di montaggio è `{fceb14e7-3b33-4b6e-8868-ae42d807b293}`.

Un dettaglio operativo che vale registrare perché ha cambiato chi ha potuto fare la verifica: `/mnt/backup` risulta `drwxr-xr-x` e appartiene a `root:root`, quindi è percorribile da utente non privilegiato. Il montaggio richiede privilegi, l'ispezione no, e tutta la parte di confronto che segue è stata eseguita dall'agente senza ulteriori comandi dell'utente.

*La domanda che era aperta, e la sua risposta.* La radice dell'archivio contiene `home`, e sotto di essa `alesop95` con i tre prefix `arta64`, `easefocus64` e `vituixcad64` più `.wine`. Poiché `/home` è una partizione distinta dalla radice, questo risponde per misura alla domanda che l'aiuto di `--includeDirs` lasciava aperta. Resta non deciso, e va detto, quale delle due condizioni abbia prodotto il risultato: se l'attraversamento del confine avvenga da solo oppure se sia stato il secondo percorso esplicito a portare `/home` dentro. Il lavoro è stato creato con entrambi, quindi l'esperimento non separa le due cause, ed è una ignoranza che non costa nulla perché la forma con due percorsi funziona in entrambi i casi.

*Il confronto per impronta.* Sei file sono stati confrontati fra sistema vivo e archivio con `sha256sum`, scelti per coprire i due filesystem e le due estremità delle dimensioni: `/etc/fstab` da 770 byte e `/etc/hostname` da 21 byte sulla radice, e i quattro `system.reg` dei prefix di Wine su `/home`, cioè 2,1 MiB per `.wine`, 3,7 MiB per `arta64`, 11,0 MiB per `easefocus64` e 10,9 MiB per `vituixcad64`. Tutte e sei le impronte coincidono.

*Il conteggio, che misura una cosa diversa dalle impronte.* Un confronto per impronta prova che i file scelti sono identici e non dice nulla sui file non scelti, quindi da solo non distingue una copia integrale da una copia parziale fatta bene. Il numero di elementi sotto ciascun prefix è stato quindi contato da entrambe le parti, e coincide in tutti e quattro i casi: 1848 per `.wine`, 5434 per `arta64`, 5870 per `easefocus64` e 5626 per `vituixcad64`. È la parte che costa due settimane a ricostruire, ed è quella su cui la verifica doveva essere più stretta.

*L'esclusione del deposito, verificata nel modo forte.* In MS-131 l'esclusione di `/home/veeam-repo` era stata dedotta dal fatto che l'archivio smettesse di crescere, che è un indizio e non una prova: un archivio potrebbe fermarsi per molte ragioni. Guardando dentro, `/mnt/backup/home/veeam-repo` non esiste affatto. L'esclusione è quindi verificata per assenza, che è la misura diretta della cosa che interessa.

*Che cosa resta nelle cartelle escluse, e perché è giusto che ci resti.* Le otto esclusioni non hanno prodotto cartelle assenti ma cartelle vuote: `/snap` pesa 64 KiB e contiene i quindici nomi dei pacchetti come directory vuote, `/sys` 24 KiB con tre voci, `/run` 16 KiB con tre, e `/proc` la sola `sys`. Veeam ha cioè conservato la struttura e scartato il contenuto, ed è il comportamento corretto: quei percorsi sono punti di innesto, e un ripristino che non li trovasse dovrebbe ricrearli a mano perché il sistema possa montarvi sopra i propri filesystem virtuali.

*Che cosa questa prova dimostra e che cosa non dimostra.* Dimostra che i byte tornano identici su un campione che copre entrambi i filesystem e tre ordini di grandezza, e che la parte cara è dentro per intero e non per campione. Non dimostra che la macchina si riavvii da questo archivio, e non è un limite della prova ma del tipo di backup: il livello di file non produce una immagine avviabile, come MS-120, MS-121 e MS-122 hanno accertato, quindi il ripristino passa comunque da una installazione nuova seguita dal riversamento. L'immagine avviabile, se la si vorrà, resta quella da prendere con Clonezilla a macchina spenta.

Verificato con: `veeamconfig point list`, che riporta il punto `Full` con `Is corrupt: false`; `veeamconfig point mount --id`, che risponde con conferma e identificativo di sessione; `ls` sulla radice del montaggio, che elenca diciannove voci fra cui `home`; `sha256sum` sui sei file citati, da entrambe le parti, con impronte coincidenti; `find | wc -l` sui quattro prefix, da entrambe le parti, con conteggi coincidenti; `ls -A` e `du -sh` sulle cartelle escluse, che le riportano vuote di contenuto e presenti come struttura; `ls` su `/mnt/backup/home/veeam-repo`, che risponde che il percorso non esiste.

Esito: fatto. Il criterio di completamento di PA-011, cioè che il backup sia stato riletto e non soltanto prodotto, è soddisfatto nella forma parziale che la guida prescrive. Restano lo smontaggio del punto e il trasferimento dell'archivio su `J:`.

### MS-133 - La procedura replicabile e il troubleshooting estratti dai microstep, e l'obbligo scritto come regola invece che affidato alla mia attenzione

Perimetro: smontaggio del punto di ripristino, estensione della pagina prescrittiva `docs/10-ambiente/veeam-agent-linux.md` con la sequenza operativa completa e con una sezione di troubleshooting a nove voci, propagazione del blocco dell'ambiente al progetto gemello, e aggiunta a `CLAUDE.md` dell'obbligo che ha generato tutto questo.

Legame con il progetto: serve la fase 10 e PA-011 per la parte Veeam, e serve l'igiene del sistema di lavoro per la parte normativa. La pagina prescrittiva è ciò che rende ripetibile in poche ore una ricostruzione costata più di un giorno, che è lo scopo dichiarato di PA-011.

*L'istruzione, e perché la registro come un difetto mio.* L'utente ha chiesto che tutto sia tracciato come sequenza operativa replicabile con i soli comandi e con la sequenza di troubleshooting necessaria, aggiungendo di non volerlo ripetere. La richiesta era già stata soddisfatta il 2026-09-16, quando da MS-117 a MS-122 era stata estratta questa stessa pagina, e non lo era più oggi: dei sette microstep prodotti oggi sul fronte operativo, nessuno era stato riversato nella pagina, e i comandi effettivamente eseguiti vivevano soltanto nel registro cronologico e nella conversazione. Il registro racconta che cosa è successo una volta, in ordine di accadimento e con i ritiri; chi deve rifare la stessa cosa fra sei mesi non ricostruisce una procedura leggendo un registro, e non deve doverlo fare.

*Che cosa è stato scritto nella pagina.* La sequenza operativa in sei fasi, dalla verifica dei presupposti fino alla rilettura dell'archivio, con i soli comandi nell'ordine reale e una riga per ciascuno che dice che cosa accerta. Le fasi sono accertare i presupposti, installare, accettare i due contratti, creare il deposito, creare il lavoro, eseguire e rileggere. I valori concreti sono quelli di questa macchina e la pagina lo dichiara, così che su un'altra si sostituiscano senza cambiare la sequenza.

La sezione di troubleshooting porta nove voci, ciascuna con sintomo, causa e rimedio, e ciascuna nata da un caso incontrato e non da una previsione: il comando che sembra appeso per i due contratti, il rifiuto di permesso con la sua misura ingannevole in pipeline, il rifiuto del livello di volume su partizioni non LVM, il modulo che non compila, la configurazione azzerata dalla purga con la cartella che sopravvive e inganna, l'archivio che si copia addosso, la partizione distinta che potrebbe non entrare, i permessi dell'archivio verso una destinazione senza permessi POSIX, e l'utente che non riesce a leggere l'archivio per trasferirlo.

Due voci meritano di essere nominate perché il loro sintomo è l'assenza di sintomo, che è il caso peggiore. La settima, cioè la partizione distinta, ha come sintomo nessun sintomo: il backup riesce e dichiara successo mentre manca la metà che conta. L'ottava, cioè i permessi, non dà segno di sé finché l'archivio resta dov'è, e si manifesta solo quando è già altrove.

*L'obbligo scritto come regola.* La richiesta è stata trasformata in una prescrizione in `CLAUDE.md`, nella sezione sul tracciamento integrale, invece di essere affidata al fatto che io me ne ricordi. È lo stesso principio che il progetto applica altrove e che MS-123 aveva enunciato dopo aver violato due volte in un'ora una regola appena scritta: una convenzione che dipende dall'attenzione in un momento di stanchezza è precisamente quella che fallisce, e va difesa da un meccanismo. Il meccanismo qui è debole, perché è una riga in un file che va letto e non un controllo che fallisce, e lo dichiaro invece di far finta che sia forte: un controllo vero dovrebbe accorgersi che un fronte operativo ha prodotto microstep senza toccare alcuna pagina prescrittiva, ed è una cosa che si può scrivere e che oggi non esiste.

Le tre regole che la prescrizione porta con sé sono quelle che distinguono una procedura affidabile da una plausibile. Un comando che compare nella sequenza è stato eseguito davvero e il suo esito osservato, mai dedotto da un aiuto. Una voce di troubleshooting nasce da un caso incontrato. E la sequenza si scrive nel giro di lavoro in cui i comandi vengono eseguiti, per la stessa ragione per cui `chat-non-e-memoria.md` vieta l'aggiornamento differito.

*Lo smontaggio.* Il punto di ripristino è stato smontato con `veeamconfig session stop` usando l'identificativo della sessione di montaggio e non quello del punto, distinzione che la pagina ora dichiara perché è il genere di cosa su cui si sbaglia una volta sola ma la si sbaglia. La verifica è che `/mnt/backup` risulti vuota.

Verificato con: `veeamconfig session stop --id`, che risponde `Session has stopped.`; `ls -A /mnt/backup`, che non stampa nulla; `md-unwrap --check`, `lint-md-commands`, `check-eol` e le tre verifiche tipografiche sulla pagina estesa, tutte pulite, con la pagina passata da 152 a 366 righe; `python tools/sync-ambiente.py --check`, che riportava un file da propagare, poi il comando senza `--check`, che lo ha scritto nel gemello, poi di nuovo `--check`, che riporta tredici file allineati e nessuna deriva.

Esito: fatto. Resta il trasferimento dell'archivio su `J:`, che dipende da una scelta dell'utente fra le due vie della voce I del troubleshooting.

### MS-134 - La procedura resa portabile a qualunque macchina Linux, e la trappola di `scp` che l'ha subito messa alla prova

Perimetro: riscrittura della sequenza operativa di `docs/10-ambiente/veeam-agent-linux.md` in forma parametrica, aggiunta della sezione sulla portabilità e della fase 7 sul trasferimento, aggiunta all'utente del gruppo `veeam`, e diagnosi di un fallimento di `scp` che ha prodotto la decima voce di troubleshooting. Il trasferimento vero è in corso mentre questa voce viene scritta e il suo esito sta nel microstep successivo.

Legame con il progetto: serve la fase 10 e PA-011 per la parte di trasferimento, e serve la riusabilità del lavoro oltre questo progetto per la parte documentale, su istruzione esplicita dell'utente.

*L'istruzione, e che cosa mancava.* L'utente ha chiesto che tutto diventi anche la possibilità di replicare l'allestimento di Veeam su qualsiasi altra macchina Linux servirà. La pagina scritta poco prima in MS-133 era replicabile su questa macchina e dichiarava che i valori si potevano sostituire, il che è vero e insufficiente: una procedura che nomina percorsi concreti nel corpo dei comandi obbliga chi la riusa a rileggere ogni riga cercando che cosa sia specifico, ed è precisamente il lavoro che una procedura dovrebbe togliere.

*La forma adottata.* I valori che cambiano sono stati estratti in un blocco unico da eseguire in testa alla sessione, cioè macchina, utente, percorso e nome del deposito, nome del lavoro, percorsi inclusi ed esclusi e numero di punti di ripristino, e tutti i comandi della sequenza li usano per nome. Chi riusa la procedura modifica otto righe e copia il resto senza leggerlo.

Accanto al blocco è stata scritta la parte che un elenco di variabili non può dare, cioè le tre decisioni che cambiano davvero da macchina a macchina e il modo di prenderle. Il tipo di volumi, che decide se sia possibile il livello di volume e quindi se si otterrà una immagine avviabile o soltanto i file. Quali filesystem esistano, che decide il valore dei percorsi inclusi, perché ogni punto di innesto da salvare va nominato esplicitamente. E dove collocare il deposito, che decide le esclusioni, perché un deposito dentro ciò che si salva fa sì che il lavoro copi sé stesso. Sono state dichiarate anche le cose che non cambiano, cioè il nome del gruppo, i permessi che l'agente impone, la richiesta di accettare due contratti e il punto di innesto predefinito del montaggio, perché sapere che cosa non va deciso toglie decisioni tanto quanto sapere che cosa vada deciso.

Un avvertimento è stato scritto accanto al blocco perché la sua assenza produrrebbe fallimenti scollegati dalla causa: quelle variabili vivono nella shell in cui sono state scritte e in nessun'altra, quindi un allestimento che duri più di una sessione va appoggiato a un file caricato con `source` invece che alla memoria della shell. È la stessa famiglia di cause della quarta voce della sezione sul contesto di shell in `git-commands-format.md`.

*Il gruppo, e la sua conferma.* L'utente è stato aggiunto al gruppo `veeam` con `usermod -aG`, e `id` riporta ora `978(veeam)` fra i suoi gruppi. La scelta era fra questa via e la copia intermedia con `sudo`, ed è motivata nella voce I del troubleshooting: il gruppo è quello che il produttore crea per questo scopo, vale per tutti i trasferimenti futuri e non duplica temporaneamente diciassette gigabyte. Il prezzo dichiarato è che l'utente ottiene lettura su tutti i backup, il che su una macchina dove ha già `sudo` non allarga il perimetro.

*La trappola di `scp`, che ha invalidato un consiglio scritto un quarto d'ora prima.* La fase 7 appena scritta prescriveva di proteggere lo spazio nel percorso remoto con due livelli di virgolette, uno per la shell locale e uno per quella remota. È la ricetta tradizionale, ed è sbagliata oggi: il primo tentativo di trasferimento è fallito con `No such file or directory` su entrambi i file, mentre `find` sulla macchina li elencava presenti e leggibili.

La causa è che dalla versione 9.0 OpenSSH esegue `scp` sul protocollo SFTP invece del vecchio trasferimento attraverso una shell remota, quindi sulla macchina remota non c'è più alcuna shell che interpreti il percorso. Il secondo livello di virgolette, che una volta veniva consumato da quella shell, oggi entra a far parte del nome del file richiesto, e quel file davvero non esiste. Il rimedio è un solo livello di virgolette; se servisse il comportamento precedente, per esempio per far espandere un carattere jolly, l'opzione `-O` riporta al protocollo vecchio e con esso torna necessaria la doppia protezione.

Va registrato anche il modo di accorgersene, perché costa secondi invece di minuti: il messaggio di errore nomina il percorso comprese le virgolette, quindi leggerlo per intero invece di fermarsi alle ultime parole dice da solo qual è il problema.

La conseguenza di metodo, ed è il motivo per cui questo caso vale una voce e non una nota: un comando scritto in una procedura è una affermazione come un'altra, e vale la stessa regola che questo progetto applica alle affermazioni tecniche. Quel comando era stato scritto da me per conoscenza pregressa e non eseguito, ed è fallito al primo uso. La pagina ora dichiara, fra le regole che la governano, che un comando nella sequenza deve essere stato eseguito davvero: questa è la prima verifica di quella regola, e l'ha superata trovando un difetto.

Verificato con: `id` dopo `usermod`, che riporta il gruppo `veeam`; lettura dei primi byte del descrittore dal deposito con utente non privilegiato in una sessione nuova, che restituisce l'intestazione `<BackupMeta Vers`, quindi il permesso è effettivo; primo tentativo di `scp` con la doppia virgoletta, fallito su entrambi i file con il percorso citato comprese le virgolette; `find /home/veeam-repo -maxdepth 2 -printf`, che elenca i due file con il percorso reale; secondo tentativo con un solo livello di virgolette, che copia il descrittore di 10831 byte e prosegue sull'archivio; `md-unwrap --check`, `lint-md-commands`, `check-eol` e le verifiche tipografiche sulla pagina, tutte pulite, con la pagina passata da 366 a 431 righe; `python tools/sync-ambiente.py`, che ha propagato la pagina al progetto gemello.

Esito: fatto per la documentazione e per il permesso di lettura. Il trasferimento è in corso e il suo esito, con la verifica per impronta, chiude il microstep successivo.

### MS-135 - Archivio trasferito su J: e verificato per impronta alle due estremità: PA-011 si chiude dopo sette giorni

Perimetro: trasferimento dei due file del backup dalla macchina alla postazione, confronto per impronta fra origine e destinazione, chiusura di PA-011 nel registro delle pendenze e nello strumento che ne verifica le condizioni.

Legame con il progetto: chiude la fase 10 per la parte di backup e chiude PA-011, aperta il 2026-09-10. Da questa chiusura non dipende alcuna altra voce: il fronte torna a essere la sottofase 8.6, cioè l'import GLL di EASE Focus, che è l'unico criterio di uscita della fase 8 ancora non soddisfatto.

*Che cosa è stato trasferito, e perché due file e non uno.* Sono stati copiati l'archivio `backup-file-sistema_2026-09-17T154511.vbk` da 17 627 570 176 byte e il descrittore `backup-file-sistema.vbm` da 10 831 byte, verso `J:\backup-macchine\alessio-ubuntustudio\`. Il descrittore non è accessorio: senza di esso l'archivio resta un file che Veeam non sa più collocare in un deposito, quindi copiare il solo `.vbk` produrrebbe una copia inutilizzabile con tutta l'apparenza di essere completa.

La cartella di destinazione è stata creata adesso e non esisteva. La scelta di non scrivere nella radice di `J:` è deliberata: là convivono già una quindicina di cartelle di materiale eterogeneo, e i backup di macchina sparpagliati fra quelle sarebbero impossibili da escludere con una regola sola quando PA-013 andrà scritta. Il percorso porta il nome della macchina proprio perché ne arriveranno altri.

*La verifica, e perché non è la dimensione.* Le impronte `sha256` sono state calcolate su entrambe le estremità e coincidono per tutti e due i file: `6843b16c86756a8c37b3e84917258e1255cf4e09a272a8f7b649b5fbc09d0d3a` per l'archivio e `053629cd54138cddfc5678220b44dd045d46fcea3136ce8d578c8d404ec8dad8` per il descrittore. La dimensione non sarebbe bastata, e non per pedanteria: un trasferimento interrotto e ripreso, o un errore di scrittura su un disco esterno, possono lasciare un file della dimensione esatta e del contenuto diverso, e su un archivio di backup la differenza fra una copia integra e una plausibile è tutto ciò che conta. È la stessa ragione per cui MS-039 aveva verificato per impronte il trasferimento dei materiali e non per dimensione.

L'intera operazione, trasferimento e due calcoli di impronta su 16,4 GiB, è costata nove minuti e quarantuno secondi. Su `J:` restano 62 GB liberi.

*Che cosa chiude PA-011 e che cosa no.* Il criterio di completamento della voce era che il backup fosse stato riletto e non soltanto prodotto, perché un backup mai riletto è una speranza e non una copia. Il criterio è soddisfatto: il punto di ripristino è stato montato e ispezionato in MS-132, con sei impronte confrontate e quattro conteggi coincidenti, e la copia su disco esterno è verificata qui. La voce si chiude dopo sette giorni dall'apertura.

Restano dichiarate tre cose che la chiusura non risolve, e vanno scritte perché altrimenti la voce chiusa sembra risolvere più di quanto risolva. La destinazione è provvisoria e il NAS domestico la sostituirà, e con essa andrà ripresa la decisione sulla cifratura, qui rinviata con motivazione in MS-131. L'archivio non è una immagine avviabile e non può esserlo, per quanto accertato in MS-120, MS-121 e MS-122, quindi il ripristino passa da una installazione nuova seguita dal riversamento e l'immagine avviabile resta quella da prendere con Clonezilla a macchina spenta. E una copia sola su un disco solo non è una strategia di backup: è il primo esemplare di una, e lo diventa quando il NAS esiste.

*Lo strumento aggiornato insieme al registro.* La voce di PA-011 in `tools/check-pending-actions.py` è stata riscritta: dichiara la voce compiuta, nomina il prodotto e la variante scelti con il motivo, e aggiunge una condizione verificata a ogni corsa, cioè l'esistenza dell'archivio al percorso di destinazione con la sua dimensione. Serve a distinguere una voce chiusa da una voce chiusa il cui esito è stato nel frattempo cancellato: il disco è rimovibile e il file è cancellabile, quindi una chiusura che non si rimisura è una dichiarazione e non un fatto.

*Un difetto mio, trovato subito e registrato perché la convenzione lo prescrive.* Scrivendo questa chiusura ho prodotto il percorso di destinazione attraverso un livello di virgolette in più del necessario, e le sequenze \b e \a del percorso Windows sono state interpretate come caratteri di controllo invece che come barre: nel file è finita la stringa `J:` seguita da un backspace e da un carattere di campanello, che a video somiglia a un percorso con le barre mancanti e in un editor non si distingue da un refuso. Il difetto è stato trovato cercando la stringa `backup-macchine` nel file appena scritto e non trovandola, cioè da un controllo fatto per abitudine e non per sospetto.

Un secondo difetto della stessa famiglia e nello stesso giro: riscrivendo per righe il blocco di PA-011 dentro `tools/check-pending-actions.py` ho inserito righe terminate con `LF` in un file interamente `CRLF`, producendo 368 righe `CRLF` e 16 `LF` nello stesso file. Non lo ha trovato una rilettura ma `check-eol.py`, cioè lo strumento che esiste esattamente per questo e che nessun altro controllo può sostituire, ed è la seconda volta che quel difetto si presenta in questo repository dopo MS-063. La riparazione è la normalizzazione a `CRLF`, che è la fine riga originale del file.

Le due cose insieme dicono una cosa sola sul modo di lavorare: scrivere dentro un file esistente attraverso uno script è comodo e non è gratuito, perché lo script non conosce le convenzioni del file che tocca se non gliele si insegna. Le due convenzioni da preservare sono la fine riga e la resa letterale di ciò che si scrive, e per entrambe il presidio non è l'attenzione ma il controllo che si lancia subito dopo.

Ne discendono due cose. La riparazione, fatta costruendo il carattere per codice invece che per scrittura letterale, e un censimento dei caratteri di controllo su tutti i file toccati oggi, che non ne ha trovati altri. E la regola, che vale ogni volta che un percorso Windows entra in un documento passando per più di un interprete: dopo averlo scritto si cerca nel file il testo che si è appena inserito, perché un percorso corrotto in questo modo resta plausibile alla lettura e non lo è alla ricerca.

Verificato con: `scp` dei due file, entrambi con stato di uscita zero; `ls -l` a destinazione, che riporta le due dimensioni, con l'archivio a 17 627 570 176 byte cioè identico all'origine; `sha256sum` sui due file alla macchina e sui due file su `J:`, con le quattro impronte a due a due coincidenti; `df -h` su `J:`, che riporta 62 GB liberi; `python tools/check-pending-actions.py`, che riporta PA-011 compiuta con la condizione nuova soddisfatta e esce con codice zero.
...
### MS-138 - Il repository WineHQ aggiunto e rifiutato per l'estensione della chiave, e due scoperte che cambiano il peso del passo

Perimetro: aggiunta del repository ufficiale WineHQ per Ubuntu 26.04, fallimento dell'aggiornamento degli indici per una ragione che i quattro messaggi di errore nascondono, copia di sicurezza dei quattro prefix di Wine prima della sostituzione, e accertamento che i pacchetti WineHQ per questa distribuzione non hanno più un ramo a 32 bit. Nessuna sostituzione di Wine è stata ancora eseguita.

Legame con il progetto: serve la sottofase 8.6 della fase 8, che è l'unico criterio di uscita della fase ancora non soddisfatto, e tocca ADR-016, che prescrive l'architettura `i386` dichiarata prima di installare Wine.

*La scoperta che cambia il peso del passo, e va prima di tutto il resto.* Il repository di WineHQ per `resolute` non pubblica alcun indice a 32 bit: la richiesta di `main/binary-i386/Packages` risponde 404, e il file `.sources` ufficiale dichiara `Architectures: amd64`. Il pacchetto `wine-stable` alla versione `11.0.0.0~resolute-1` è un unico pacchetto `amd64` da 237 892 976 byte che dichiara `Conflicts` e `Replaces` su `wine-stable-i386`, e fra le sue dipendenze non compare alcuna libreria `i386`. È il nuovo WoW64, cioè un solo binario a 64 bit che esegue i programmi Windows a 32 bit senza le librerie di sistema a 32 bit.

La conseguenza per questo progetto va enunciata perché tocca una decisione registrata. ADR-016 prescrive di dichiarare l'architettura `i386` prima di installare Wine, e il prefix di Akabak e VACS gira oggi proprio su quel ramo, con `wine32:i386` e l'albero di 263 pacchetti che MS-084 aveva censito. Passare a WineHQ non toglie la capacità di eseguire programmi a 32 bit, ma la sposta su una via diversa da quella su cui funziona adesso. Akabak è l'unico programma del corredo con una licenza attivata e legata al Machine Identifier, quindi è il punto su cui il rischio si concentra, ed è il primo controllo da fare dopo la sostituzione e non l'ultimo.

*Il rifiuto del repository, e i quattro messaggi che nascondono una causa sola.* Seguita la procedura documentata da WineHQ, cioè chiave in `/etc/apt/keyrings/winehq-archive.key` e file `.sources` che vi punta con `Signed-By`, `apt update` ha prodotto in sequenza quattro esiti: un avviso secondo cui la chiave è ignorata perché il file ha un tipo non supportato, un secondo avviso di firma non verificabile con `NO_PUBKEY 76F1A20FF987672F`, un errore secondo cui il repository non è firmato, e infine, al comando successivo, `Unable to locate package winehq-stable`.

I quattro non sono quattro problemi ma uno solo, e l'ordine in cui si leggono è esattamente l'inverso dell'ordine in cui aiutano. L'ultimo è quello che si nota, perché è l'unico che interrompe il lavoro, e manda a cercare un pacchetto che non esiste o un nome sbagliato; il primo è l'unico che dice la causa, ed è un avviso in mezzo all'uscita di `apt update`. La causa è che questa versione di APT decide come leggere un file di portachiavi dalla sua estensione, e `.key` non è fra quelle riconosciute: il file viene ignorato, quindi la firma non si verifica, quindi il repository è considerato non firmato, quindi i suoi pacchetti non entrano nell'indice.

Va detto che la procedura seguita è quella pubblicata dal produttore, quindi non è un errore di esecuzione: è una istruzione che ha smesso di funzionare su una distribuzione più recente di quando è stata scritta. È lo stesso genere di sfasamento già osservato con il modulo del kernel di Veeam in MS-121, e la conseguenza di metodo è la stessa: la documentazione di un fornitore va trattata come una fonte datata e non come una specifica.

La chiave è in formato ASCII armato, verificato leggendone la prima riga, che è `-----BEGIN PGP PUBLIC KEY BLOCK-----`. L'estensione corretta per una chiave armata è quindi `.asc`, e va cambiata sia nel nome del file sia nel campo `Signed-By` del file `.sources`, perché quel campo nomina un percorso e non una chiave.

*La copia di sicurezza dei prefix, e una sorpresa nei numeri.* Prima di qualunque sostituzione i quattro prefix sono stati archiviati in `/home/alesop95/prefix-prima-di-winehq-2026-09-17.tar`, così che un ritorno costi due minuti invece della rilettura di 17 GiB da `J:`. L'archivio è però risultato di 28 GiB contro i 7,7 GiB che `du` riportava, e la differenza non è un errore.

La causa è un solo file sparso: dentro il prefix di EASE Focus, in `drive_c/users/alesop95/AppData/Local/Temp/CalculationCache/`, esiste un file da 20 479,7 MiB di dimensione apparente e zero blocchi allocati. Un file sparso dichiara una lunghezza e non occupa spazio finché non vi si scrive davvero; `du` conta i blocchi allocati e riporta quindi lo spazio reale, mentre `tar` senza opzioni legge la lunghezza dichiarata e scrive tutti quegli zeri nell'archivio. Le due misure rispondono a due domande diverse ed entrambe sono corrette.

Ne discende una regola operativa: per archiviare un albero che può contenere file sparsi si usa `tar -S`, e per prevedere la dimensione di un archivio si guarda `du --apparent-size` e non `du`. Qui la somma delle dimensioni apparenti è 27,4 GiB, che è quanto l'archivio è venuto, mentre `du` prometteva 7,7 GiB.

L'archivio resta valido come copia di ritorno ed è stato verificato: il registro del comando è vuoto, il primo elemento archiviato è `.wine/`, e nessun percorso di sistema vi compare, il che esclude il timore ragionevole che `tar` abbia seguito il collegamento `dosdevices/z:` che in ogni prefix punta alla radice del filesystem. Occupa 28 GiB su 293 liberi e va cancellato quando la sostituzione sarà validata.

Verificato con: richiesta di `main/binary-i386/Packages` per `resolute`, che risponde 404; lettura del file `.sources` pubblicato, che dichiara `Architectures: amd64`; lettura dell'indice `binary-amd64`, da cui i campi `Depends`, `Conflicts`, `Replaces` e `Size` di `wine-stable` alla 11.0; uscita completa di `sudo apt update`, con i quattro messaggi citati; `head -c 60` sulla chiave scaricata, che riporta l'intestazione PGP armata, e `file`, che la classifica come blocco di chiave pubblica PGP; `ls -lh` sull'archivio dei prefix e `df -h /home` prima e dopo, da 31 a 58 GiB occupati; `tar -tf` sull'archivio, che elenca `.wine/` come primo elemento e nessun percorso di sistema; `find -printf` sulle dimensioni apparenti e allocate, che isola il singolo file sparso da 20 479,7 MiB con zero blocchi.

Esito: bloccato in questa forma, con la causa accertata e il rimedio noto. Il passo successivo è rinominare la chiave e correggere il riferimento nel file `.sources`.

### MS-139 - Il pacchetto WineHQ ispezionato senza installarlo: i programmi a 32 bit restano, i prefix a 32 bit no

Perimetro: ispezione del contenuto del pacchetto `wine-stable` 11.0 di WineHQ scaricato e non installato, accertamento dell'architettura dei quattro prefix esistenti, e conseguenza sul prefix di AKABAK e VACS. Nessuna modifica alla macchina.

Legame con il progetto: serve la sottofase 8.6, e tocca ADR-016 e la sottofase 8.1, cioè l'installazione di AKABAK. Da questo accertamento dipende se la sostituzione di Wine possa essere fatta senza toccare il prefix che porta la licenza.

*Perché ispezionare invece di installare e vedere.* La domanda era se il pacchetto di WineHQ, che non ha più un ramo `i386`, sappia ancora eseguire i programmi Windows a 32 bit e in quale forma. La risposta cambia il piano, quindi andava avuta prima e non dopo, e un pacchetto `.deb` si può elencare senza installarlo: scaricato in `/tmp` e letto con `dpkg -c`, non tocca il sistema.

*Che cosa contiene, con i numeri.* Il pacchetto porta 1071 file sotto `lib/wine/i386-windows/`, che sono le librerie Windows a 32 bit in formato PE, e zero file sotto `lib/wine/i386-unix/`, che sarebbero la parte Unix a 32 bit. Sotto `x86_64-unix/` ce ne sono 288. Fra gli eseguibili c'è `bin/wine` e non esiste alcun `bin/wine64` né alcun preloader a 32 bit.

La lettura di questi tre numeri è univoca ed è il nuovo WoW64: un solo processo a 64 bit esegue codice Windows a 32 bit traducendone le chiamate, senza che esista alcun lato Unix a 32 bit. I programmi Windows a 32 bit continuano quindi a funzionare, ed è la parte rassicurante. Ciò che non esiste più è il caricatore a 32 bit, e un prefix dichiarato a 32 bit ha bisogno esattamente di quello.

*L'architettura dei quattro prefix, misurata.* Tre prefix sono a 64 bit, cioè `arta64`, `easefocus64` e `vituixcad64`, e lo dichiarano sia con la presenza di `drive_c/windows/syswow64` sia con la riga `#arch=win64` in testa al proprio `system.reg`. Il quarto, `~/.wine`, è a 32 bit puro: nessun `syswow64`, e `#arch=win32` nel registro. È il prefix di AKABAK e VACS, cioè l'unico del corredo che porta una licenza attivata.

*La conseguenza, e non è un ostacolo insormontabile.* Sostituendo Wine con i pacchetti di WineHQ i tre prefix a 64 bit restano utilizzabili, mentre `~/.wine` così com'è non lo è più, perché il caricatore che gli serve non è più installato. La via non è convertire quel prefix, cosa che Wine non supporta in alcun verso, ma crearne uno nuovo a 64 bit e reinstallarvi i due programmi.

Il costo di quella reinstallazione è basso proprio grazie a un accertamento già fatto. MS-085 ha stabilito che l'attivazione di AKABAK non vive nel registro del prefix ma in un file, cioè `C:\ProgramData\RDTeam\Akabak.ini`, sezione `[Security]`, voce `SecCode`, e che la licenza è legata alla macchina e non al prefix. Quel file sta dentro `drive_c` del prefix vecchio, che non viene cancellato da nulla di quanto stiamo per fare, quindi la licenza si porta nel prefix nuovo copiando un file di testo. E in ogni caso il Release Code permanente è conservato in `_notes/licenze-akabak-riservato.md`, quindi anche la via del reinserimento manuale resta aperta.

Ne segue che la sottofase 8.1, che MS-085 aveva dichiarato non necessaria perché il prefix era sopravvissuto all'azzeramento del sistema, torna necessaria per una ragione diversa dalla sua: non perché il prefix sia andato perduto, ma perché cambia il motore che lo esegue. La dichiarazione di MS-085 resta vera nel suo contesto e va letta con questa aggiunta.

*Che cosa resta non misurato.* Non è provato che AKABAK e VACS, che sono programmi a 32 bit, funzionino sotto il nuovo WoW64: la presenza delle 1071 librerie PE dice che l'ambiente per eseguirli c'è, non che quei due programmi specifici vi girino senza difetti. È la prova che va fatta subito dopo la sostituzione, ed è il primo controllo della sequenza e non l'ultimo.

Verificato con: `curl` del pacchetto `wine-stable_11.0.0.0~resolute-1_amd64.deb` in `/tmp`, 237 892 976 byte, coincidente con la dimensione dichiarata nell'indice del repository; `dpkg -c` sullo stesso, che ne elenca il contenuto senza installarlo; conteggi per prefisso di percorso, che danno 1071 per `i386-windows/`, 0 per `i386-unix/` e 288 per `x86_64-unix/`; elenco di `opt/wine-stable/bin/`, che contiene `wine` e `wineserver` e non `wine64`; presenza o assenza di `drive_c/windows/syswow64` nei quattro prefix e riga `#arch=` in testa ai rispettivi `system.reg`.

Esito: fatto per l'accertamento. La sostituzione può procedere, con la reinstallazione di AKABAK e VACS in un prefix nuovo a 64 bit come passo obbligato e non come ripiego.

### MS-140 - Wine sostituito: via i sette pacchetti della distribuzione, dentro WineHQ 11.0, con `winetricks` messo al sicuro prima

Perimetro: correzione dell'estensione della chiave che impediva ad APT di leggere il repository, adozione di `winetricks` dalla sua sorgente ufficiale, rimozione con purga della pila Wine della distribuzione, installazione di `winehq-stable` e verifica che il comando eseguito sia quello nuovo. La migrazione dei prefix e la prova dei programmi non appartengono a questa voce.

Legame con il progetto: serve la sottofase 8.6 della fase 8, ed esegue la decisione sull'ambiente che MS-114 aveva lasciato come unico candidato residuo. Modifica il presupposto di ADR-016, che va riletta alla luce di MS-139.

*La correzione che ha sbloccato il repository.* La chiave scaricata secondo la procedura di WineHQ si chiamava `winehq-archive.key`, e questa versione di APT decide come leggere un portachiavi dall'estensione: `.key` non è fra quelle riconosciute, quindi il file veniva ignorato e da lì discendevano gli altri tre messaggi. La chiave è in formato ASCII armato, verificato leggendone l'intestazione, quindi l'estensione corretta è `.asc`. Rinominata la chiave e corretto il campo `Signed-By` del file `.sources`, che nomina un percorso e non una chiave, `apt update` ha scaricato l'indice del repository, 39,0 kB, senza alcun avviso.

*Winetricks, messo al sicuro prima di toccare qualunque cosa.* Il pacchetto `winetricks` della distribuzione dipende da `wine`, cioè dal pacchetto che la sostituzione rimuove, e `wine-stable` di WineHQ non dichiara di fornirlo: rimuovendo la pila della distribuzione senza precauzioni si sarebbe perso lo strumento con cui in questo progetto sono stati installati `dotnet48` e `gdiplus`, cioè le dipendenze misurate di VituixCAD e di EASE Focus. Lo strumento è stato quindi preso dalla sua sorgente ufficiale, dove è un singolo script di shell indipendente da qualunque pacchetto, e collocato in `/usr/local/bin`, che nel PATH precede `/usr/bin`. Sono 873 846 byte, versione `20260125-next`, e lo script dichiara la propria impronta `sha256` all'avvio, il che permette di verificare il download invece di fidarsene. La versione ottenuta è anche più recente di quella della distribuzione, che era del gennaio 2025.

*La rimozione, e i numeri che la descrivono.* La purga ha tolto esattamente i sette pacchetti nominati, cioè `wine`, `wine64`, `wine32:i386`, `wine-common`, `libwine` in entrambe le architetture e `winetricks`, liberando 1 800 MB e portando il conteggio del database di dpkg da 446 645 a 443 926 file. Nessun altro pacchetto è stato rimosso, cosa verificata con una simulazione prima di eseguire.

I circa 263 pacchetti `i386` che `wine32:i386` si era tirato dietro in MS-084 sono stati deliberatamente lasciati installati, pur essendo ora orfani e segnalati come rimovibili. La ragione è che tenerli rende il ritorno immediato invece di costringere a riscaricare 250 MB, e costano 1,1 GB su 293 liberi. Vanno rimossi quando la sostituzione sarà validata, e non prima: è un debito dichiarato e non una dimenticanza.

*L'installazione.* Sono entrati due pacchetti, `wine-stable` alla versione `11.0.0.0~resolute-1` e il metapacchetto `winehq-stable` da 1 306 byte, per 238 MB scaricati a 31,1 MB/s in otto secondi e 1 532 MB occupati. Il programma si installa sotto `/opt/wine-stable` e il metapacchetto crea il collegamento in `/usr/bin`.

*Le due verifiche finali, che non sono formalità.* La prima è `wine --version`, che risponde `wine-11.0`: se avesse risposto ancora `wine-10.0` significherebbe che un residuo della distribuzione vince nel PATH, ed è precisamente il modo in cui una sostituzione sembra riuscita e non lo è. La seconda è `command -v` sui due comandi, che risponde `/usr/bin/wine` e `/usr/local/bin/winetricks`: il primo è il collegamento creato dal metapacchetto nuovo, il secondo è lo script messo al sicuro e non quello del pacchetto appena rimosso. Le due insieme dicono che l'ambiente eseguito è quello voluto, e sono la forma che qui prende la regola di `prove-che-misurano.md`, cioè che un controllo deve poter cadere.

Verificato con: `sudo apt update` dopo la rinomina, che scarica l'indice `resolute/main amd64` di 39,0 kB senza avvisi; `/usr/local/bin/winetricks --version`, che riporta `20260125-next` con la propria impronta; `sudo apt purge -s`, che elencava i sette pacchetti e nessun altro, seguita dalla purga reale con lo stesso esito e 1 800 MB liberati; `sudo apt install -y --install-recommends winehq-stable`, con i due pacchetti, i 238 MB e i 1 532 MB dichiarati; `wine --version`, che risponde `wine-11.0`; `command -v wine winetricks`, che risponde `/usr/bin/wine` e `/usr/local/bin/winetricks`.

Esito: fatto per la sostituzione. Restano la migrazione dei tre prefix a 64 bit, la ricostruzione del prefix di AKABAK e VACS che MS-139 ha reso necessaria, e la prova che chiude la sottofase 8.6.

### MS-141 - WineHQ 11.0 non popola il lato a 32 bit dei prefix, misurato sei volte: il lato a 64 bit funziona e i programmi del progetto no

Perimetro: migrazione dei prefix esistenti e creazione di prefix nuovi sotto WineHQ 11.0, con e senza display, al primo e al secondo passaggio; prove di esecuzione a 64 e a 32 bit; accertamento che le librerie a 32 bit siano installate sul sistema. Registra anche due errori miei nella conduzione delle prove.

Legame con il progetto: serve la sottofase 8.6 e ne determina l'esito. Da questa misura dipende se la sostituzione di Wine decisa in MS-140 vada tenuta o annullata.

*Il risultato, in una riga.* Sotto WineHQ 11.0 stabile un prefix a 64 bit si crea con il lato a 64 bit completo e funzionante e il lato a 32 bit vuoto, e nessun programma a 32 bit può essere eseguito. Tutti i programmi Windows di questo progetto sono a 32 bit.

*Le sei prove, e perché sono sei e non una.* Una misura sola non distingue un difetto da una circostanza, quindi la stessa domanda è stata posta in condizioni diverse. La migrazione del prefix `vituixcad64` si è fermata con il processo a zero per cento di CPU e nessun file del prefix toccato in sessanta secondi. La stessa cosa è accaduta a `easefocus64`, con il registro degli eventi che si interrompe esattamente sulle stesse tre righe. Un prefix nuovo di zecca creato con il display privato è terminato con stato `1`. Un secondo prefix nuovo creato senza alcun display, cioè togliendo dall'ambiente sia `DISPLAY` sia `WAYLAND_DISPLAY`, ha dato lo stesso esito, il che esclude che la causa sia il server grafico. Un secondo passaggio di `wineboot -u` sullo stesso prefix si è fermato come il primo, con il registro vuoto. E le prove di esecuzione hanno separato i due lati.

*La misura che dice esattamente che cosa manca.* Nei prefix nuovi la cartella `drive_c/windows/system32` contiene 802 file e la cartella `drive_c/windows/syswow64` ne contiene zero: esiste ma è vuota. Poiché in un prefix a 64 bit `syswow64` è il posto dove vivono le librerie Windows a 32 bit, un lato a 32 bit vuoto significa che nessun programma a 32 bit ha le proprie librerie.

La conferma per esecuzione è netta e sta in due comandi. Un programma a 64 bit gira: `wine cmd /c echo` stampa il proprio argomento, esce con stato zero e non produce alcuna riga di errore. Un programma a 32 bit non gira, e non perché fallisca ma perché non esiste: `syswow64/cmd.exe` non è presente sul disco, e il tentativo di eseguirlo risponde `wine: failed to open` con stato `53`.

*Che cosa esclude questa misura.* Non è un pacchetto incompleto: sul sistema `/opt/wine-stable/lib/wine/i386-windows/` contiene 1070 file e `x86_64-windows/` ne contiene 1018, quindi le librerie a 32 bit ci sono e sono quelle che MS-139 aveva contato dentro il `.deb`. Il difetto non è che manchino, è che non vengano installate nel prefix.

*Gli errori che accompagnano il fallimento.* Il registro riporta, in entrambe le prove pulite, un fallimento di marshalling OLE sull'interfaccia `{6d5140c1-7436-11ce-8034-00aa006009fa}` con codice `0x80004002`, che significa interfaccia non supportata, seguito da `err:ole:start_rpcss Failed to open RpcSs service`. La lettura ragionevole è che il servizio di chiamata di procedura remota del prefix non parta e che l'inizializzazione si interrompa prima di arrivare alla fase che popola il lato a 32 bit. È una lettura e non una diagnosi: la causa prima di quel fallimento non è stata isolata, e va dichiarato invece di essere presentato come accertato.

*La conseguenza per il progetto.* Tutti i programmi Windows del corredo sono a 32 bit. AKABAK e VACS lo sono per ADR-016, che lo ha misurato con `file` sugli eseguibili installati; EASE Focus lo è per MS-107, dove il prefix a 64 bit serve al solo servizio di database. Ne segue che WineHQ 11.0 stabile, nella forma in cui si installa su questa macchina, non è utilizzabile da questo progetto, e che la sostituzione decisa in MS-140 va annullata oppure tentata su un altro ramo dello stesso repository.

*Due errori miei nella conduzione delle prove, registrati perché insegnano.* Il primo tentativo di prefix pulito è stato fatto sotto `/tmp` ed è fallito con `wine: '/tmp' is not owned by you, refusing to create a configuration directory there`. Wine rifiuta di creare un prefix in una cartella che non appartiene all'utente, ed è una protezione ragionevole contro un prefix condiviso fra utenti; la prova andava fatta nella home e basta. Per qualche minuto quel rifiuto ha rischiato di essere letto come un fallimento di Wine, che è il genere di errore che allunga una diagnosi invece di accorciarla.

Il secondo è la ripetizione di un difetto già registrato oggi in MS-126: ho letto lo stato di uscita di un comando alla fine di una pipeline, ottenendo lo stato dell'ultimo elemento invece che quello del comando. È la seconda volta nella stessa sessione, il che conferma che la regola nota non basta e che la forma da usare è eseguire il comando da solo e leggere `$?` subito dopo.

Verificato con: `find -newermt` sui prefix durante le migrazioni, che riporta zero file toccati in sessanta e novanta secondi; `ps` con tempo di CPU, che riporta `wineboot.exe` a 0,0 per cento e zero secondi consumati; conteggio dei file in `system32` e `syswow64` dei due prefix nuovi, che dà 802 e 0 in entrambi; `wine cmd /c echo`, che stampa e esce zero senza errori; esecuzione di `syswow64/cmd.exe`, che risponde `failed to open` con stato 53; `ls` su `/opt/wine-stable/lib/wine/i386-windows/` e `x86_64-windows/`, che danno 1070 e 1018 file; lettura dei registri delle quattro prove, con gli errori OLE e `start_rpcss` citati; ripetizione della prova senza `DISPLAY` né `WAYLAND_DISPLAY`, con esito identico.

Esito: bloccato. La sostituzione di Wine non è utilizzabile in questa forma, e la decisione fra tentare il ramo staging dello stesso repository e tornare ai pacchetti della distribuzione è dell'utente.

### MS-142 - Anche staging cade, la sostituzione si annulla e i prefix si ripristinano dall'archivio: il corredo è tornato identico

Perimetro: prova del ramo staging di WineHQ, ritorno ai pacchetti Wine della distribuzione, ripristino dei tre prefix a 64 bit dall'archivio preso prima della sostituzione, verifica per esecuzione dei quattro programmi del corredo, e ritiro di una mia affermazione di poche ore prima. Chiude il tentativo aperto in MS-140.

Legame con il progetto: serve la sottofase 8.6 e la chiude sul piano dell'ambiente, e ripristina il controllo di uscita della fase 8 che MS-116 aveva superato in quattro criteri su cinque.

*Come staging è stato provato senza rischiare nulla, e perché la circostanza è stata fortunata.* L'utente ha eseguito di seguito il comando che installava staging e quelli che rimettevano i pacchetti della distribuzione, quindi staging è stato installato e subito dopo scavalcato prima di poter essere provato. Il ritorno però ha rimosso il solo metapacchetto `winehq-staging` e ha lasciato installato `wine-staging` alla versione `11.16~resolute-1` sotto `/opt/wine-staging`, marcandolo come orfano. Questo ha reso possibile la prova nella forma migliore: invocare staging per percorso assoluto, cioè `/opt/wine-staging/bin/wine`, che risponde `wine-11.16 (Staging)`, senza toccare il PATH, i pacchetti o il Wine 10 appena rimesso in servizio.

*L'esito, identico a quello del ramo stabile.* Su un prefix nuovo di zecca la creazione si è fermata e il comando è terminato per esaurimento del tempo dopo quattrocento secondi, con stato `124`. Il conteggio che conta è lo stesso di MS-141: `drive_c/windows/system32` con 816 file e `drive_c/windows/syswow64` con zero. Il ramo staging non si comporta diversamente dal ramo stabile, quindi il candidato WineHQ è escluso per misura su entrambi i rami e non su uno solo.

*Il ritorno, con i numeri.* I pacchetti della distribuzione sono stati reinstallati, sette in tutto fra `wine`, `wine64`, `wine32:i386`, `wine-common`, `libwine` nelle due architetture e `winetricks`, per 309 MB scaricati e 1 800 MB occupati. `wine --version` risponde di nuovo `wine-10.0 (Ubuntu 10.0~repack-12ubuntu1)` e `wine32` è di nuovo presente in `/usr/bin`. Il pacchetto `wine-staging` rimasto orfano è stato poi rimosso con purga, liberando 1 823 MB.

Una cosa non è tornata come prima ed è meglio di prima: `winetricks` resta quello ufficiale in `/usr/local/bin`, alla versione `20260125-next`, che vince nel PATH sul pacchetto della distribuzione reinstallato accanto. Non dipende da alcun pacchetto Wine, quindi una prossima sostituzione non lo porterà più via.

*Il ripristino dei prefix, che è il motivo per cui l'archivio esisteva.* Il prefix a 32 bit di AKABAK e VACS non era stato toccato da nulla, perché il WoW64 nuovo non lo sapeva nemmeno aprire, ed è stato verificato subito: il comando eseguito con `wine32` stampa, esce con stato zero e non produce una riga di errore, e il file della licenza `drive_c/ProgramData/RDTeam/Akabak.ini` porta ancora la voce `SecCode`.

I tre prefix a 64 bit erano invece stati sfiorati dai tentativi di Wine 11 e non tornavano utilizzabili: il primo `wineboot -u` sotto Wine 10 su `vituixcad64` è terminato per esaurimento del tempo dopo seicento secondi. Invece di insistere sono stati ripristinati dall'archivio `prefix-prima-di-winehq-2026-09-17.tar`, spostando di lato la cartella danneggiata e riestraendo con l'opzione per i file sparsi. L'estrazione è terminata con stato zero, e il conteggio dei file coincide esattamente con quello che MS-132 aveva misurato dentro il backup Veeam prima di qualunque modifica: 5626 elementi per `vituixcad64`, 5870 per `easefocus64` e 5434 per `arta64`.

*La verifica per esecuzione, che è quella che chiude.* I quattro programmi sono stati aperti tutti insieme sullo stesso display privato, ed è lo stesso controllo di uscita della fase 8 che MS-116 aveva eseguito il 2026-09-15. Le finestre presenti sul display sono `Akabak - (new)`, `VituixCAD`, `[New EASE Focus Project] - EASE Focus 3, Version 3.1.260` con la finestra di iscrizione alle notizie AFMG, e la coppia `License Registration` e `LoadingCurtainForm` di ARTA, che è il suo comportamento noto in modalità dimostrativa. Tutti e quattro i processi risultano vivi contemporaneamente. Il corredo è quindi tornato esattamente dov'era prima della sostituzione.

*Il ritiro, ed è di una mia affermazione di un'ora prima.* Avevo scritto che il messaggio `err:environ:init_peb starting ... in experimental wow64 mode`, comparso avviando i prefix a 64 bit sotto Wine 10, fosse un residuo lasciato dal contatto con il WoW64 nuovo. È falso, e la misura che lo smentisce è il ripristino stesso: i prefix riestratti dall'archivio non hanno mai incontrato Wine 11, e producono lo stesso messaggio. Il messaggio è quindi una proprietà della build Wine 10 della distribuzione quando avvia un processo a 32 bit dentro un prefix a 64 bit, ed è benigno, come dimostra il fatto che i programmi si aprono. L'affermazione era una inferenza plausibile da una correlazione temporale, cioè il classico errore di attribuire a un evento recente ciò che c'era anche prima.

Ne segue che il ripristino non era strettamente necessario per quel messaggio, e resta comunque giustificato da ciò che lo aveva motivato davvero, cioè il `wineboot -u` che non terminava. Il ripristino ha inoltre riportato i conteggi a valori identici a quelli del backup, che è la forma più forte di garanzia disponibile.

Verificato con: `/opt/wine-staging/bin/wine --version`, che risponde `wine-11.16 (Staging)`; prova su prefix nuovo con quel percorso, terminata con stato `124` e `syswow64` a zero file su `system32` a 816; `wine --version` e `command -v` dopo il ritorno, che danno `wine-10.0` e `/usr/bin/wine`, `/usr/bin/wine32`, `/usr/local/bin/winetricks`; `dpkg -l` filtrato, che riporta i sette pacchetti della distribuzione e nessun pacchetto WineHQ; esecuzione in `~/.wine` con `wine32`, stato zero e zero errori; presenza della voce `SecCode` nel file di licenza; `tar -xSf` con stato zero; `find | wc -l` sui tre prefix ripristinati, con 5626, 5870 e 5434 elementi; avvio contemporaneo dei quattro programmi e lettura dei titoli delle finestre con `xdotool`, più `pgrep` sui quattro processi.

Esito: fatto. La macchina è tornata alla configurazione nota e verificata, e il candidato WineHQ è escluso per misura su entrambi i rami.

### MS-143 - La sottofase 8.6 si chiude con otto cause escluse per misura e nessun candidato residuo

Perimetro: chiusura della sottofase 8.6 e del controllo di uscita della fase 8, sulla base delle esclusioni accumulate da MS-110 a MS-142. È una conclusione e non un intervento: nessun comando è stato eseguito per questa voce.

Legame con il progetto: chiude la fase 8, che era superata in quattro criteri di uscita su cinque dal 2026-09-15. Sblocca la fase 11, cioè la fotografia finale della macchina, e sblocca PA-017, cioè il secondo backup a setup finito.

*Che cosa si voleva e che cosa si è ottenuto.* Il criterio non soddisfatto era il caricamento di un modello GLL in EASE Focus, cioè la capacità di alimentare il catalogo dei diffusori del programma. Il programma si apre, funziona e crea progetti, ma rifiuta ogni file GLL che gli si proponga, compreso uno depositato nel prefix dall'installatore stesso, con il messaggio `The following file is no System Definition or not compatible to the current version of the application`.

*Le otto cause escluse, ciascuna per misura e non per ragionamento.* Un modulo mancante, escluso dalla traccia del caricamento delle librerie. Il servizio di database AFMG, escluso perché il programma lo avvia da sé e il rifiuto non cambia con il servizio attivo o fermo. I file stessi, esclusi perché il rifiuto è identico su modelli di provenienza e di epoca diverse. La libreria dei certificati, esclusa dalla stessa traccia. Il provider di firma digitale `dssenh.dll`, che era l'ipotesi forte di MS-112 ed è stata provata sostituendolo con la libreria originale di Windows presa dalla postazione: il rifiuto di flag scompare e l'importazione fallisce identica, quindi l'ipotesi è ritirata in MS-113. L'età del modello, esclusa in MS-114 scaricando un GLL pubblicato nel gennaio 2026, rifiutato come quelli del 2016 e del 2022. I redistributabili di Visual C++, esclusi in MS-114 perché le tre sole librerie native del programma non ne importano alcuno, incorporando staticamente la propria libreria di esecuzione. E infine la provenienza dei pacchetti di Wine, che era l'ultimo candidato: MS-141 e MS-142 lo escludono non perché il passaggio a WineHQ non abbia risolto il problema, ma per una ragione più netta, cioè che su questa macchina i pacchetti di WineHQ non eseguono affatto i programmi a 32 bit, quindi quella strada non è percorribile e non solo non è risolutiva.

*Perché questa è una chiusura e non una resa.* La differenza sta nel fatto che ogni causa è caduta per una misura ripetibile e documentata, e che le misure sono state fatte nell'ordine del costo crescente, dalla lettura di una traccia fino alla sostituzione dell'intero motore. Il valore di questo lavoro per chi verrà dopo non è la soluzione, che non c'è, ma l'elenco di ciò che non serve provare: otto strade percorse fino in fondo valgono più di una diagnosi plausibile e non verificata.

Va inoltre detto che cosa la sottofase 8.6 costa davvero al progetto, perché la risposta ridimensiona il problema. EASE Focus serve a simulare la copertura di sistemi di diffusione in un ambiente, partendo da modelli GLL dei diffusori; in questo progetto i diffusori sono quelli che si stanno costruendo, e un loro GLL non esiste finché non lo si produce. La sottofase 8.7 aveva già accertato in MS-116 che produrre un GLL con ARTA in modalità dimostrativa non è possibile, perché il salvataggio è la funzione che la modalità dimostrativa esclude, ed è PA-014. Il catalogo dei GLL di terze parti, cioè quello che l'importazione rifiuta, serve al confronto e non alla progettazione dei propri diffusori.

*Che cosa resta aperto e dove.* La capacità di importare GLL non è persa per sempre: resta come voce nota, e se un giorno servisse davvero le vie non provate sono due, cioè una versione diversa di EASE Focus e una macchina Windows reale. Nessuna delle due è un lavoro da fare adesso, e nessuna delle due è una prova rapida. La decisione sul GLL dei propri monitor resta PA-014.

Verificato con: rilettura dei microstep da MS-110 a MS-116 per le prime sette esclusioni, ciascuna con la propria misura; MS-141 e MS-142 per l'ottava; stato del corredo dopo il ripristino, con i quattro programmi aperti contemporaneamente e i loro titoli di finestra letti sul display privato.

Esito: fatto. La fase 8 è chiusa, con il criterio sull'importazione dei GLL dichiarato non soddisfatto e non soddisfacibile con i mezzi disponibili, e con le otto esclusioni che ne sono la giustificazione.

### MS-144 - Fase 11 eseguita: la fotografia finale confrontata con quella di partenza, e la prova che `/home` è sopravvissuta

Perimetro: esecuzione della fase 11 della procedura di installazione pulita sulla macchina `alessio-ubuntustudio`, confronto voce per voce con la fotografia del 2026-09-07, trasferimento della raccolta sulla postazione con verifica per impronta, nuova pagina `docs/10-ambiente/fotografia-macchina-post-reinstall.md`, e correzione di una previsione superata nella fase 11 di `docs/10-ambiente/installazione-pulita-26-04.md`.

Legame con il progetto: chiude la procedura di ricostruzione dell'ambiente, che serve le fasi da 3 a 8 del workflow perché è la macchina su cui girano tutti gli strumenti di simulazione. Non serve direttamente alcuna fase di progettazione elettroacustica: è il controllo di uscita di un lavoro di infrastruttura, e la sua utilità sta nel dichiarare quali affermazioni sullo stato della macchina sono ora verificate invece che credute.

*La raccolta, e perché è stata scritta come uno script.* Ventinove voci raccolte in una cartella `~/fotografia-post-reinstall`, nessuna vuota, con i primi ventitre nomi identici a quelli della fotografia di partenza perché il confronto potesse essere un `diff` file per file invece di una lettura a occhio. Le ultime sei sono nuove e misurano cose che nel 2026-09-07 non erano ancora in questione, cioè i limiti realtime effettivi, i gruppi dell'utente, i target di sospensione mascherati, i comandi di Wine presenti nel PATH e l'elenco dei prefix. La forma a script non è pedanteria: due raccolte fatte con comandi anche solo leggermente diversi non sono confrontabili, e la seconda esecuzione è quella in cui si sbaglia.

*La voce che conta più di tutte le altre.* La partizione `/home` porta l'identificativo univoco `4aa5afff-0647-4b5a-b168-0119b360c1aa`, che è lo stesso di quattordici giorni prima. Poiché formattare un filesystem ne genera uno nuovo, l'uguaglianza è una prova e non un indizio, ed è l'unica affermazione dell'intera procedura che non poteva essere verificata mentre la si faceva. Nello stesso confronto la radice ha cambiato identificativo, da `19351fc8` a `bdae6be9`, che è l'esito voluto, e la partizione EFI ha conservato il suo `9054-9070` perché non era stata formattata per scelta.

*Il resto del confronto, in sintesi.* Il sistema passa dalla 25.04 `plucky` alla 26.04.1 LTS `resolute` e il kernel dalla `6.14.0-35` alla `7.0.0-31`. La direttiva di aggiornamento passa da `Prompt=normal` a `Prompt=lts`, ed è la riga che impedisce al blocco di ripetersi; la conferma che sia in vigore non è la lettura del file ma la risposta di `do-release-upgrade -c`, che ora dichiara di non avere alcuna LTS da proporre. I parametri di avvio guadagnano `rcu_nocbs=all`, che non c'era prima e non è stato aggiunto da nessuno: lo scrive il pacchetto di impostazioni a bassa latenza della 26.04, e sposta il lavoro differito del meccanismo di lettura concorrente del kernel fuori dai processori che eseguono il carico, che è un miglioramento per una macchina audio. I dispositivi audio sono identici a prima. I pacchetti installati manualmente passano da quarantotto a cinquantuno, e la differenza utile è l'ingresso dei due pacchetti di Veeam, che sono la traccia di PA-011, e di `xdotool` e `xvfb`, che sono la capacità acquisita in MS-111.

*La previsione superata, che va corretta nel documento e non nei fatti.* La fase 11 della procedura dichiarava fra i confronti attesi che l'elenco delle architetture straniere sarebbe stato vuoto, mentre prima conteneva `i386`. Oggi contiene `i386` ed è giusto così. Quella riga fu scritta prima di ADR-016, quando si credeva che l'architettura a 32 bit fosse un residuo da non ricreare; ADR-016 ha poi stabilito il contrario per misura, e MS-141 lo ha rafforzato. La riga è stata corretta nella procedura dichiarando il ritiro invece di essere cancellata.

*Il difetto del mio comando, che è il terzo accertamento non atteso della giornata e il più istruttivo dei tre in forma generale.* Il censimento dei prefix è stato riprodotto nella forma della fotografia di partenza, cioè `find ~ -maxdepth 2 -name system.reg`, proprio per avere due file confrontabili. Ha riportato due prefix invece di sette, senza alcun errore. La causa non è nel comando ma nel terreno: nel 2026-09-07 esisteva il solo `~/.wine`, a profondità uno, mentre oggi i prefix del corredo stanno sotto `~/wineprefixes/` e il loro `system.reg` è a profondità tre. Il limite che era generoso allora è diventato stretto adesso, e un limite di ampiezza scritto dentro un comando è una assunzione sul terreno e non una parte della domanda. Corretto a `-maxdepth 3`, il censimento riporta i sette prefix reali: i quattro in uso, i tre danneggiati sotto `~/wineprefixes.rotto-winehq/` che sono il residuo di MS-142, e la copia di sicurezza `~/.wine-prima-di-wine10` di MS-085.

*Il trasferimento fuori dalla macchina, con una trappola che sembra un guasto.* La cartella è stata copiata sotto `_notes/fotografia-post-reinstall-2026-09-21/`, che è ignorata da git, e verificata per impronta alle due estremità. Il primo confronto ha segnalato tutte e ventinove le righe come diverse, il che per un istante sembra una copia andata male: la causa è che `md5sum` di Git Bash stampa un asterisco prima del nome per indicare la lettura in modo binario, mentre quello di Linux stampa due spazi. Confrontando le sole impronte, le ventinove coincidono.

Verificato con: `bash ~/fase11.sh` sulla macchina, con `find -type f -empty` che non riporta alcun file vuoto su ventinove; `diff -u` fra le due fotografie su quattordici file confrontabili; `md5sum` alle due estremità con ventinove impronte coincidenti; `find $HOME -maxdepth 3 -name system.reg` che riporta sette prefix contro i due della forma a profondità due.

Esito: fatto. La fase 11 è compiuta e la procedura di installazione pulita è chiusa in tutte le sue fasi. Il confronto ha prodotto tre scarti non attesi, di cui due diventano azioni differite nelle due voci seguenti e uno ha corretto il comando che lo aveva nascosto.

### MS-145 - Il repository WineHQ è rimasto sulla macchina, e nel suo contenuto c'è la causa prima che MS-141 dichiarava di non avere isolato

Perimetro: accertamento sul residuo `/etc/apt/sources.list.d/winehq-resolute.sources` trovato dalla fotografia finale, lettura del file `Release` pubblicato da WineHQ per quattro suite di Ubuntu, e apertura di PA-018. Nessuna modifica alla macchina.

Legame con il progetto: serve la fase 8, di cui consolida la chiusura, perché trasforma l'ottava esclusione di MS-143 da comportamento osservato su una macchina a proprietà dichiarata di un repository, verificabile da chiunque senza installare nulla. Serve inoltre alla manutenzione futura, perché un repository di terze parti dimenticato è uno stato che nessuno ha deciso di lasciare.

*Che cosa è rimasto.* Il file delle sorgenti di WineHQ per la suite `resolute`, datato 2026-09-17, e la chiave in `/etc/apt/keyrings/winehq-archive.asc`. MS-142 aveva rimosso i pacchetti con purga e rimesso quelli della distribuzione, ma non il repository, e la cosa non era stata notata perché nessuno stava guardando le sorgenti di apt. Da solo non fa danno: nessun pacchetto di quella provenienza è installato, e `apt upgrade` non installa ciò che non c'è.

*La domanda che il contenuto pone, e perché la risposta è andata cercata invece di essere data.* Il file dichiara `Architectures: amd64`, quindi apt non scarica da quel repository alcun indice a 32 bit, e la cronologia di apt conferma che il 2026-09-17 furono installati soltanto `winehq-stable:amd64` e `wine-stable:amd64`. La lettura immediata è che si trattasse di una configurazione sbagliata, che avrebbe invalidato la prova di MS-141; quella lettura, se accettata, avrebbe riaperto la sottofase 8.6. La verifica costa una richiesta e non una installazione, e dice il contrario: il file `Release` pubblicato da WineHQ per `resolute` dichiara esso stesso `Architectures: amd64`, e l'indice `binary-i386` risponde 404. La riga nel file delle sorgenti rispecchiava quindi correttamente ciò che il repository offre.

*Dove sta la cesura.* Interrogando quattro suite, `noble` per la 24.04 LTS e `plucky` per la 25.04 pubblicano `amd64 i386`, mentre `questing` per la 25.10 e `resolute` per la 26.04 LTS pubblicano il solo `amd64`. La fotografia di partenza lo conferma dal proprio lato: i due file WineHQ presenti sulla macchina precedente, per `noble` e per `plucky`, dichiaravano entrambi `Architectures: amd64 i386`.

*Che cosa cambia, e che cosa no.* MS-141 aveva misurato sei volte che i pacchetti di WineHQ non popolano il lato a 32 bit dei prefix, e aveva dichiarato apertamente che la causa prima non era stata isolata. Ora lo è: per Ubuntu 26.04 WineHQ non pubblica alcuna metà a 32 bit, quindi il caricatore ELF a 32 bit non esisteva da installare e l'unica strada disponibile era il WoW64 nuovo. L'esclusione ottava di MS-143 resta valida e ne esce rafforzata, perché non è più un fatto di questa macchina. Restano due cose che questo accertamento non dimostra, e vanno dette perché la tentazione di estenderlo è forte: non dimostra che il WoW64 nuovo sia difettoso in generale, dato che il motivo per cui qui non ha inizializzato `syswow64` resta non isolato, e non dimostra che WineHQ sia inutilizzabile in assoluto, dato che sulle due versioni precedenti di Ubuntu la metà a 32 bit esiste.

*La conseguenza pratica.* Finché il corredo di questo progetto è fatto interamente di programmi a 32 bit, i pacchetti della distribuzione sono l'unica fornitura possibile su Ubuntu 26.04, e ADR-016 è confermata per la terza volta e per una ragione nuova. Che fare del repository rimasto è una decisione e non una copia, quindi non l'ho presa io: è PA-018, con le tre opzioni e il loro costo.

Verificato con: `cat /etc/apt/sources.list.d/winehq-resolute.sources` e `ls -la /etc/apt/keyrings/`; `apt policy winehq-stable`, che riporta il candidato `11.0.0.0~resolute-1` come `amd64` e nessuna controparte; `apt-cache policy wine-stable-i386`, che riporta candidato assente; `grep winehq /var/log/apt/history.log`, che riporta i soli pacchetti `amd64`; `curl -I` sugli indici `binary-amd64` e `binary-i386` di `resolute`, che rispondono 200 e 404; lettura della riga `Architectures` nei file `Release` di `noble`, `plucky`, `questing` e `resolute`.

Esito: fatto. La causa prima è isolata, l'esclusione ottava è rafforzata, e il residuo è tracciato come decisione aperta invece di restare uno stato che nessuno ha scelto.

### MS-146 - La catena audio non si misura da una sessione SSH: due cause indipendenti, e ciò che l'osservazione non dimostra

Perimetro: accertamento sulla voce `pactl info` della fotografia finale, che riporta l'assenza di schede audio, isolamento delle due cause con `getfacl` e con i gruppi del processo `wireplumber`, e apertura di PA-019. Nessuna modifica alla macchina.

Legame con il progetto: serve la fase 6 del workflow, cioè la verifica della catena audio, e per suo tramite la fase 1, cioè la misura reale della stanza, che non si esegue senza una catena audio funzionante. Il suo contributo non è una correzione ma un metodo: dice in quale forma quella verifica va fatta perché il suo esito significhi qualcosa.

*L'osservazione.* PipeWire riporta `Default Sink: auto_null`, l'elenco delle schede è vuoto e l'unica uscita è una `Dummy Output`, mentre ALSA, cioè lo strato sottostante, vede regolarmente la scheda integrata `ALC887-VD` con i suoi cinque dispositivi di riproduzione. Due strati adiacenti danno risposte opposte sulla stessa domanda, ed è il genere di contraddizione che non si risolve guardandola meglio.

*La prima causa, misurata.* I nodi in `/dev/snd/` appartengono a `root:audio` con permessi `crw-rw----` e portano una lista di controllo di accesso che `systemd-logind` assegna alla sola sessione attiva del posto. La lista riporta `user:sddm:rw-` e nessuna voce per l'utente, e infatti la sessione attiva di `seat0` è quella dello schermo di accesso: la sessione grafica dell'utente esiste ancora, risulta `Active=no` e `State=online`, e ha perso l'accesso ai dispositivi quando lo schermo di accesso ha preso il posto.

*La seconda causa, indipendente dalla prima.* Il processo `wireplumber` di quella sessione gira con i gruppi `4 24 27 30 46 100 111 115 1000`, dove mancano il gruppo `audio`, che è il 29, e il gruppo `pipewire`, che è il 982. L'utente vi appartiene, e `id` lo conferma, ma un processo riceve i gruppi quando la sessione nasce e non li rilegge mai più: quella sessione grafica è iniziata il 2026-09-09 alle 12:57, cioè prima che MS-077 aggiungesse l'utente ai due gruppi. È la stessa forma della quarta causa della regola sul contesto di shell, dove un terminale non rilegge il PATH aggiornato dopo una installazione; qui l'ambiente ereditato sono i gruppi.

*Ciò che l'osservazione non dimostra, ed è la parte che conta.* Non dimostra che la catena audio sia guasta. Nessuna delle due cause riguarda la configurazione del sistema, ed entrambe cadono con un accesso nuovo alla console, che riattiva la sessione e le assegna i gruppi correnti. Dimostra invece una cosa diversa e utile, cioè che una verifica della catena audio condotta da SSH riporta l'assenza di schede anche su una macchina perfettamente funzionante, quindi il suo esito negativo non è informativo. Presentare quella misura come un difetto sarebbe stato un allarme falso costruito su dati veri.

*La lezione, che generalizza una già scritta.* MS-093 aveva scoperto che Wine non trova un display da una sessione SSH, e la scheda che ne è nata sta in `docs/10-ambiente/wine-troubleshooting.md`. Le due osservazioni sono casi della stessa cosa e conviene enunciarla una volta sola: una sessione remota non è una finestra sulla macchina, è una sessione diversa, con un proprio ambiente, propri gruppi e nessun posto assegnato. Tutto ciò che dipende dalla sessione grafica va misurato da quella, e un esito raccolto altrove non vale né in positivo né in negativo.

Verificato con: `pactl info`, `pactl list short cards` che non riporta alcuna scheda, e `wpctl status` che riporta la sola `Dummy Output`; `aplay -l` e `/proc/asound/cards`, che riportano la scheda integrata; `getfacl -p /dev/snd/controlC0`, che riporta `user:sddm:rw-` e nessuna voce per l'utente; `loginctl show-session` sulle due sessioni e `loginctl show-seat seat0`, che dichiara attiva quella dello schermo di accesso; la riga `Groups` in `/proc/2210/status` confrontata con `id -G`, con i gruppi 29 e 982 mancanti nel processo; `journalctl --user -u wireplumber`, che non riporta alcun errore sullo strato ALSA.

Esito: fatto come accertamento, aperto come verifica. La misura della catena audio va rifatta da una sessione grafica attiva sulla console, ed è PA-019.

### MS-147 - Lo strumento delle azioni differite taceva su sei voci su diciannove, ed era il difetto che il suo stesso docstring descrive

Perimetro: estensione di `tools/check-pending-actions.py` con la funzione `controlla_voci_recenti`, che copre PA-014, PA-016, PA-017, PA-018 e PA-019, e correzione di una riga di PA-011 rimasta indietro. Rimandi alla nuova pagina della fotografia finale aggiunti in `docs/README.md` e in `docs/10-ambiente/README.md`.

Legame con il progetto: non serve alcuna fase del workflow, e va detto apertamente perché un legame inventato sarebbe peggio di uno assente. Serve al presidio che impedisce a un impegno di sparire fra una sessione e l'altra, e il progetto lo esercita a ogni apertura di sessione per prescrizione di `CLAUDE.md`.

*Il difetto, e perché è imbarazzante.* Lo strumento elencava le voci da PA-001 a PA-013 e taceva su tutte quelle aperte dopo il 2026-09-15, cioè PA-014, PA-015, PA-016 e PA-017, alle quali oggi si aggiungono PA-018 e PA-019. Il docstring della funzione `controlla_pa010_pa011` enuncia da tempo la regola esatta che il resto del file violava, cioè che uno strumento che tace su una voce la fa sparire, e la enuncia proprio per giustificare la scelta di stampare voci le cui condizioni non sono automatizzabili. La regola era scritta, il suo rispetto no, ed è la forma più comune in cui un presidio smette di funzionare senza che nessuno se ne accorga: non si rompe, resta indietro.

*Perché nessuno se n'era accorto, che è la parte generalizzabile.* Lo strumento esce con stato zero e stampa un elenco lungo, quindi la sua uscita ha sempre l'aspetto di un controllo passato. Chi lo lancia legge le voci che ci sono e non può leggere quelle che mancano, perché una voce assente non lascia traccia nell'uscita. È lo stesso schema della regola sulle prove che misurano, dove una prova verde può significare che il comportamento è corretto oppure che non viene esercitato, e vale la stessa conseguenza: un controllo che elenca va confrontato ogni tanto con la sorgente che dovrebbe elencare, perché da solo non può segnalare la propria incompletezza.

*Che cosa le sei voci nuove dicono adesso.* PA-017 risulta eseguibile, perché la sua condizione di sblocco è soddisfatta dal 2026-09-21, e lo strumento ne stampa anche i due residui da cancellare prima della corsa, che sono il motivo per cui una esecuzione distratta costerebbe trentacinque gigabyte di archivio inutile. PA-019 risulta bloccata dalla presenza fisica dell'utente, e la voce dichiara che non è automatizzabile per costruzione, dato che il contesto della sessione è precisamente ciò che va verificato. PA-011 riportava ancora la sottofase 8.6 come aperta con il rimando a MS-114, e ora rimanda a MS-143, che l'ha chiusa.

*Una scelta di forma, dichiarata perché non è ovvia.* PA-015 non è stata aggiunta, perché è chiusa insieme a PA-016 dal 2026-09-17 e le voci compiute in questo strumento compaiono solo quando la loro verifica ha ancora un valore, come per PA-001 e PA-007, dove serve a confermare che una cancellazione sia avvenuta. Per una guardia software installata, la verifica che ha valore è `check-copie-modelli.py`, che gira già nella catena prima di un commit.

Verificato con: `python tools/check-pending-actions.py`, che ora stampa diciannove voci contro le tredici precedenti e termina con stato zero; `python tools/check-eol.py tools/check-pending-actions.py`, che riporta il file come CRLF puro dopo la modifica, dato che la patch lavora su una copia a fine riga normalizzata e ripristina la forma originaria in scrittura.

Esito: fatto. Il presidio elenca di nuovo tutto ciò che deve elencare, e la lezione su come ha smesso di farlo è scritta qui perché si ripeterà alla prossima voce aggiunta senza toccare lo strumento.

### MS-148 - I due residui del tentativo su Wine cancellati, dopo la verifica che li rende cancellabili

Perimetro: cancellazione da `/home` dell'archivio `~/prefix-prima-di-winehq-2026-09-17.tar` e della copia danneggiata `~/wineprefixes.rotto-winehq`, più il registro vuoto `~/tar-prefix.log`, preceduta dalla verifica dello stato dei prefix in uso. Compie il prerequisito di PA-017.

Legame con il progetto: serve la fase 8 in modo indiretto e PA-017 in modo diretto. I due residui stanno sotto `/home`, che è nel perimetro del backup, quindi senza questa cancellazione il secondo archivio avrebbe portato con sé trentasei gigabyte di materiale che nessuno vorrebbe ripristinare. Non tocca alcuna capacità del corredo.

*Perché la cancellazione era sicura, e perché la domanda va posta prima e non dopo.* Una cancellazione irreversibile si giustifica con una ridondanza misurata e non con un ricordo, quindi prima di eseguirla sono stati contati i file dei tre prefix a 64 bit e verificata la presenza della licenza nel prefix a 32 bit. I conteggi sono 5626 per `vituixcad64`, 5871 per `easefocus64` e 5434 per `arta64`, contro i 5626, 5870 e 5434 misurati in MS-142 subito dopo il ripristino. Lo scarto è di un solo file su `easefocus64` ed è atteso, perché quel programma è stato aperto dopo il ripristino e scrive nel proprio prefix; va comunque dichiarato invece di essere arrotondato, perché un conteggio che cambia senza spiegazione è il genere di cosa che si scopre troppo tardi.

*La ridondanza vera, che non è il tar.* L'archivio da 28 GiB conteneva i tre prefix a 64 bit come erano prima della sostituzione di Wine, ed è servito in MS-142 per ripristinarli. Gli stessi tre prefix, negli stessi stati, stanno anche dentro il punto di ripristino Veeam del 2026-09-17, i cui conteggi erano stati confrontati in MS-132 proprio montandolo, e quel punto esiste in due copie, una sulla macchina e una su `J:`. La cancellazione del tar non toglie quindi alcuna copia unica: toglie la terza.

*La prova funzionale, e un difetto mio nel modo di leggerla.* Prima della cancellazione il prefix a 32 bit è stato esercitato con `wine32`, che ha stampato e terminato con stato zero, e il registro non riporta alcuna riga di errore. Il comando complessivo ha però restituito stato uno, e non perché qualcosa fosse fallito: l'ultimo elemento era `grep -c err:`, che risponde uno quando non trova nulla. È la terza occorrenza dello stesso difetto di lettura, dopo MS-126 e MS-141, e stavolta con una variante che merita una riga, perché qui lo stato diverso da zero era proprio il risultato buono.

*Che cosa è stato liberato.* L'occupazione di `/home` passa da 66 GB a 31 GB, cioè trentacinque gigabyte, coerente con i 28 GiB più 7,6 GiB dei due residui. Le voci più grandi rimaste sotto la cartella dell'utente sono i prefix con 6,7 GB e l'archivio personale con 2,3 GB.

Verificato con: `find | wc -l` sui tre prefix a 64 bit prima della cancellazione; `grep -c SecCode` sul file di licenza, che riporta uno; `ls | wc -l` su `system32` e `syswow64` del prefix a 32 bit, che riportano 842 e zero, cioè la forma corretta di un prefix a 32 bit puro; esecuzione di `wine32 cmd /c echo` con stato zero e zero righe di errore; `ls` sui due percorsi dopo la cancellazione, che rispondono percorso inesistente; `df -h /home` prima e dopo.

Esito: fatto. Il prerequisito di PA-017 è compiuto e la corsa del secondo backup non porterà con sé i residui.

### MS-149 - Il secondo punto di ripristino, pieno e indipendente per decisione dell'utente, prodotto, riletto e portato fuori: PA-017 si chiude

Perimetro: corsa del lavoro Veeam `backup-file-sistema` con l'opzione di pieno attivo, prova di rilettura montando il punto e confrontando tredici impronte e quattro conteggi, trasferimento dell'archivio e del suo file di metadati su `J:` con verifica per impronta alle due estremità. Chiude PA-017.

Legame con il progetto: non serve una fase del workflow ma la sua ripetibilità. La ricostruzione dell'ambiente è costata più di un giorno di lavoro fra il 9 e il 17 settembre, e lo scopo di questo punto è renderla ripetibile in poche ore invece che in un giorno. A differenza del punto del 2026-09-17, questo fotografa la macchina a fase 8 chiusa e a fase 11 compiuta, cioè lo stato che qualcuno vorrebbe davvero ripristinare.

*La decisione, che era il vero contenuto di PA-017.* L'utente ha scelto la copia piena indipendente invece dell'incrementale sulla catena esistente, e il criterio è quello che la voce aveva enunciato all'apertura: il punto del 17 settembre fotografa uno stato intermedio, con la fase 8 non ancora chiusa e i residui del tentativo su Wine ancora sul disco, quindi far dipendere il punto utile da quello inutile avrebbe aggiunto una superficie di guasto senza aggiungere valore. Il costo pagato per l'indipendenza è di circa 17 GiB di spazio in più, su un disco esterno che ne aveva 88 liberi. Un secondo argomento, che all'apertura non era stato scritto e vale registrarlo, è che un punto incrementale non si porta fuori da solo: senza il pieno da cui dipende non ripristina nulla, quindi la copia esterna sarebbe stata una catena e non un file.

*La corsa.* Il comando è `veeamconfig job start --name backup-file-sistema --activeFull`, eseguito alle 13:04 e terminato alle 13:11, quindi sette minuti, esattamente come la corsa del 17 settembre. L'archivio prodotto è `backup-file-sistema_2026-09-21T130457.vbk`, da 18 214 821 888 byte, e la sessione dichiara `Success`. L'elenco dei punti riporta ora due voci, entrambe di tipo `Full` e con `Is corrupt` a `false`, il che è la prova che l'opzione ha fatto ciò che si voleva: senza di essa la seconda voce sarebbe stata un incrementale.

Un dato che sorprende e ha una spiegazione: il pieno nuovo è più grande del precedente di circa 587 milioni di byte, malgrado i trentasei gigabyte di residui cancellati poche ore prima. Il motivo è che i residui non erano mai stati nell'archivio precedente, dato che furono creati alle 17:29 e alle 18:32 del 17 settembre mentre la corsa era terminata alle 15:52; la crescita è quindi quella reale della macchina fra le due date, cioè i tre prefix ripristinati, i pacchetti Wine reinstallati e la fotografia della fase 11.

*La prova di rilettura, che è la parte che rende il backup tale.* Il punto è stato montato su `/mnt/backup` e confrontato con il sistema vivo su tredici impronte e quattro conteggi, tutte coincidenti. Le impronte coprono il file di licenza di AKABAK, una libreria di sistema del prefix a 32 bit, i tre registri dei prefix a 64 bit, un file della fotografia della fase 11, `/etc/fstab`, le sorgenti apt e i cinque file dei limiti in `/etc/security/limits.d/`. I conteggi sono 1848 elementi per `~/.wine`, 5626 per `vituixcad64`, 5871 per `easefocus64` e 5434 per `arta64`, cioè gli stessi del sistema vivo.

Due controlli in più rispetto alla prova di MS-132, entrambi negativi per costruzione e per questo utili. Il primo verifica che i due residui cancellati in MS-148 non siano nell'archivio, e non ci sono. Il secondo verifica che il deposito non sia finito dentro se stesso, cioè che l'esclusione di `/home/veeam-repo` dichiarata nel lavoro sia stata rispettata, e non c'è. Un backup che contiene il proprio deposito è un difetto che cresce da solo a ogni corsa, e costa un comando accertare che non stia accadendo.

*Un mio errore nella chiusura del montaggio, di quelli che costano un minuto.* Ho provato `veeamconfig point umount`, che non esiste: la risposta è `Unknown argument` con stato 255. La forma corretta è `veeamconfig session stop` con l'identificativo della sessione di montaggio e non del punto, ed è già scritta nella pagina prescrittiva, che avrei dovuto leggere invece di indovinare. Vale come promemoria del motivo per cui quella pagina esiste.

*Il trasferimento fuori dalla macchina.* Sono stati copiati su `J:` l'archivio nuovo e il file di metadati `backup-file-sistema.vbm`, che descrive la catena e senza il quale i punti non sono interpretabili; la sua copia precedente su `J:` conosceva un punto solo ed è stata sostituita. Le impronte coincidono alle due estremità, cioè `3f6683148f88913136b48e5690d9ec5c` per l'archivio e `aeca45ad0788c02d0c2c56d3097557f3` per i metadati. Su `J:` restano ora entrambi i punti pieni, per 35 842 392 064 byte complessivi, e lo spazio libero scende da 88 a 71 GiB. Su `/home` l'occupazione passa da 31 a 47 GB per via del secondo archivio nel deposito locale.

*Che cosa resta dichiarato e non risolto, come alla chiusura di PA-011.* La destinazione `J:` resta provvisoria in attesa del NAS domestico, e con essa resta rinviata la decisione sulla cifratura, che va ripresa quando la destinazione diventerà definitiva. Un backup a livello di file non dà una immagine avviabile, e quella resta da prendere con Clonezilla a macchina spenta. Nessuna delle tre è cambiata oggi, e vanno ridette invece di essere date per note.

Verificato con: `veeamconfig job start --activeFull`, che restituisce identificativo di sessione e percorso dei registri; `veeamconfig session list`, che riporta la sessione `Success` dalle 13:04 alle 13:11; `veeamconfig point list --backupId`, che riporta due punti `Full` non corrotti; `veeamconfig point mount` seguito dallo script di confronto, con tredici impronte e quattro conteggi coincidenti e i due controlli negativi superati; `veeamconfig session stop` per il montaggio; `md5sum` sull'archivio e sui metadati alle due estremità, con le due coppie coincidenti; `ls -la` e `df -h` su `J:` e su `/home`.

Esito: fatto. PA-017 è compiuta: il secondo punto esiste, è pieno e indipendente per decisione registrata, è stato riletto montandolo e sta anche fuori dalla macchina con le impronte confrontate.

### MS-150 - La procedura Veeam non richiede privilegi come dichiarava: ritirata la voce B del troubleshooting, e aggiunta la fase della corsa successiva

Perimetro: correzione di `docs/10-ambiente/veeam-agent-linux.md` in tre punti, cioè il ritiro del rimedio dichiarato nella voce B del troubleshooting, una nota sui privilegi in testa alla sequenza operativa, e una fase 8 nuova per la corsa successiva con la decisione fra incrementale e pieno indipendente.

Legame con il progetto: nessuna fase del workflow, e va detto apertamente. Serve la ripetibilità del backup, che è ciò che PA-011 e PA-017 esistono per garantire, e serve il progetto gemello `home-lab-cybersec-networking`, da cui viene l'esperienza precedente citata nella pagina e a cui la pagina è dichiaratamente riusabile.

*Che cosa la pagina affermava, e perché era falso.* La voce B del troubleshooting descriveva correttamente il sintomo, cioè che `veeamconfig` risponde con un rifiuto di permesso a qualunque sottocomando, e correttamente la causa, cioè che il binario appartiene a `root:veeam` con permessi `-rwxr-x---`. Concludeva però che rimedio non ce ne fosse e che ogni passo fosse lavoro privilegiato, e questa conclusione è falsa. Il gruppo `veeam` esiste proprio per dare quell'accesso, l'installazione del pacchetto vi aggiunge l'utente, e una sessione che lo porta esegue il binario senza `sudo`.

*La misura che lo smentisce, fatta oggi senza cercarla.* L'intera corsa di MS-149 è stata eseguita da una sessione SSH ordinaria e non interattiva: elenco dei depositi, elenco dei lavori, lettura della configurazione del lavoro, avvio con `--activeFull`, elenco dei backup e dei punti, montaggio del punto di ripristino e chiusura della sessione di montaggio, tutti con stato zero. Non è un caso particolare di un sottocomando benevolo: comprende esattamente i due passi che la pagina descriveva come privilegiati per definizione, cioè l'avvio del lavoro e il montaggio.

*Perché la voce era stata scritta così, ed è la parte che vale oltre Veeam.* Quando la pagina fu scritta, il 2026-09-16, la sessione da cui si lavorava era stata aperta prima dell'installazione del pacchetto, quindi non portava il gruppo appena creato. Un processo riceve i propri gruppi quando la sessione nasce e non li rilegge mai più, quindi il rifiuto era reale e la spiegazione sbagliata: non serviva `sudo`, serviva una sessione nuova. È la stessa forma della quarta causa della regola sul contesto di shell, dove un terminale non rilegge il PATH dopo una installazione, ed è la stessa forma della prima causa di MS-146, dove un `wireplumber` nato prima di MS-077 non ha i gruppi che l'utente ha. Tre occorrenze in tre contesti diversi nell'arco di dodici giorni, e conviene enunciare la regola generale invece di riscoprirla: dopo una installazione che tocca utenti, gruppi o variabili d'ambiente, la prima verifica si fa da una sessione aperta dopo, altrimenti si misura lo stato di prima e lo si attribuisce al programma.

*La fase nuova, e perché non era prevedibile prima di oggi.* La pagina si fermava alla prima corsa, perché era stata scritta il giorno della prima corsa. La fase 8 aggiunta dice che cosa cambia dalla seconda in avanti, cioè che le fasi da 0 a 4 non si rifanno, e porta la decisione fra incrementale e pieno indipendente con il criterio per scegliere, che non è lo spazio occupato ma se il punto precedente fotografi uno stato che qualcuno voglia davvero ripristinare. Vi è annotato anche il vincolo pratico che decide da solo molti casi, cioè che un incrementale non si porta fuori dalla macchina da solo.

*Che cosa non è stato cambiato, per scelta.* I comandi della sequenza continuano a portare `sudo`, e la nota nuova spiega perché: è la forma che funziona sempre, anche da una sessione aperta prima dell'installazione, che è precisamente la situazione di chi sta seguendo la procedura per la prima volta. Togliere `sudo` dai comandi avrebbe reso la pagina corretta per il lettore esperto e rotta per quello a cui serve.
...
### MS-153 - Il pacchetto LaTeX istanziato nei due progetti, e due difetti del `.gitignore` colti prima del commit

Perimetro: istanziazione del pacchetto `latex` in questo progetto e nel gemello `home-recording-training-mixing-setup`, secondo la mappa dichiarata nel README del pacchetto, con il blocco degli artefatti aggiunto a entrambi i `.gitignore`. Comprende la correzione di due difetti che avevo introdotto e la misura di un terzo, preesistente, nel gemello.

Legame con il progetto: serve PA-021 e, più avanti, la voce della roadmap sul report di progetto dei monitor alle fasi 4 e 5. Non serve alcuna fase del workflow adesso, e la decisione di anticipare l'istanziazione rispetto all'innesco dichiarato è dell'utente ed è registrata come ADR-022.

*Che cosa è stato istanziato.* Sette file per progetto, copiati byte per byte dal modello: i quattro script di `setup-tex` e `build` per Windows e Linux sotto `scripts/`, il manifesto `tex-packages.txt` alla radice, il `.latexmkrc` che fissa l'engine a `pdflatex`, e la skill `latex-build` sotto `.claude/skills/`. La distribuzione TeX non è stata installata: TinyTeX è user-local, condivisa fra i progetti, e non si versiona, quindi l'installazione è un atto separato che si esegue con `scripts/setup-tex` quando serve compilare davvero.

*Il primo difetto mio, e il più insidioso dei due.* Il blocco degli artefatti che ho aggiunto al `.gitignore` conteneva `*.pdf` come pattern globale. Questo file porta in testa un commento che spiega perché i pattern per tipo di documento siano ancorati alla radice, cioè che da globali nascondevano per errore file di strumenti e di modelli che vanno versionati, e `CLAUDE.md` ripete l'avvertenza. Un PDF può essere un riferimento legittimamente versionato, quindi il suo pattern non va reso globale: `*.pdf` è stato tolto, resta il `/*.pdf` ancorato che c'era già, e quando il documento LaTeX avrà una collocazione si aggiungerà il suo percorso preciso. Gli ausiliari, cioè `.aux`, `.fls`, `.fdb_latexmk` e gli altri, restano globali di proposito, perché quelli non si versionano in nessuna cartella e appartengono alla stessa categoria degli archivi e degli eseguibili già trattati così.

Va detto che nessun file tracciato sarebbe stato nascosto, verificato con una ricerca sull'indice: `.gitignore` non tocca ciò che è già tracciato, quindi il danno sarebbe stato interamente futuro. È la stessa forma di MS-130, dove una negazione previene il caso futuro e non ne ripara uno in corso, ed è la ragione per cui il difetto era invisibile e andava cercato invece che atteso.

*Il secondo difetto mio, che è il primo rovesciato.* Il pacchetto istanzia `tex-packages.txt` alla radice del progetto, dove il pattern `/*.txt` lo nasconde. L'ironia è che il commento in testa a questo stesso `.gitignore` cita proprio `.claude/templates/latex/tex-packages.txt` come esempio del file che i pattern globali nascondevano: il difetto è stato risolto per la copia modello e si ripresenta identico per la copia istanziata, perché l'ancoraggio alla radice, che protegge le sottocartelle, colpisce esattamente la radice. La correzione è una negazione, `!/tex-packages.txt`, che è la stessa forma adottata al template in MS-130.

*Il terzo difetto, che non è mio e che la misura ha trovato nel gemello.* Il `.gitignore` di `home-recording-training-mixing-setup` porta ancora i pattern per tipo in forma globale, cioè nella forma che questo progetto ha corretto tempo fa. La misura, fatta con `git status --ignored`, dice che là sono nascosti quattro elementi che dovrebbero essere versionati: `.claude/templates/CLAUDE.local.md`, la cartella `.claude/templates/_notes/`, `.claude/templates/automation-starter/workflows/automation-routine-prompt.txt` e `.claude/templates/latex/tex-packages.txt`. Nessun file con estensione `.txt` risulta tracciato in quel repository. Non l'ho corretto: ancorare quei pattern è una modifica alla convenzione di un altro repository e farebbe entrare quattro elementi nel suo indice, quindi è una decisione dell'utente. Nel gemello ho aggiunto la sola negazione minima per la copia istanziata, dichiarando nel commento che la copia modello resta nascosta e perché.

*Perché i due difetti sono stati colti.* Non da un controllo automatico, che non esiste per questo, ma perché dopo l'istanziazione ho confrontato l'elenco dei file attesi con `git status` e ne mancava uno. È il metodo che vale registrare più del caso singolo: dopo una copia di file, l'elenco di ciò che si è copiato va confrontato con l'elenco di ciò che git vede, perché la differenza fra i due è precisamente l'insieme dei file che il `.gitignore` sta nascondendo in silenzio.

Verificato con: `git status --short` nei due progetti, che elenca tutti e sette i file istanziati come non tracciati, `tex-packages.txt` compreso dopo la negazione; `git check-ignore -v` su `docs/prova.pdf`, che non risulta ignorato, e su `docs/prova.aux`, che lo risulta; ricerca sull'indice delle estensioni del blocco nuovo, che non trova alcun file tracciato; `git status --ignored --short` nel gemello, che elenca i quattro elementi nascosti; `python tools/check-copie-modelli.py`, che riporta le copie allineate.

Esito: fatto. Il pacchetto è istanziato in entrambi i progetti e nessuno dei due `.gitignore` nasconde qualcosa che questa istanziazione voleva versionare. Resta aperta nel gemello la questione dell'ancoraggio dei pattern per tipo, che è una decisione e non una copia.

### MS-154 - La catena audio esce dal silenzio: profilo analogico disponibile, uscita reale e tono udito, con due limiti dichiarati

Perimetro: verifica della catena audio sulla macchina `alessio-ubuntustudio` dopo il collegamento di cuffie al jack analogico posteriore, misura dello stato di PipeWire e della sessione, e tre riproduzioni di prova ascoltate dall'utente. Riguarda PA-019 e la fase 6 della procedura di installazione pulita.

Legame con il progetto: serve la fase 6 e, per suo tramite, la fase 1 del workflow, cioè la misura reale della stanza, che non si esegue su una macchina incapace di riprodurre. Non chiude la fase 6, che resta bloccata da PA-012 per la parte di ingresso, e chiude la parte di uscita.

*Che cosa è cambiato collegando le cuffie, e perché il cambiamento è esso stesso una misura.* Prima del collegamento i profili analogici della scheda erano dichiarati `available: no`, il profilo attivo era `output:iec958-stereo+input:analog-stereo`, cioè l'uscita digitale S/PDIF, e l'unica uscita disponibile era una presa a cui non era collegato nulla. Collegando le cuffie al jack verde posteriore i profili analogici sono passati a `available: yes` e PipeWire è passato da solo al profilo `output:analog-stereo+input:analog-stereo`. Ne segue che il rilevamento della presenza sul jack funziona, il che non era scontato e non era stato verificato: l'assenza di uscite analogiche misurata in MS-146 non era un difetto della scheda ma la descrizione fedele di una presa vuota.

*Lo stato misurato dopo.* Sink `alsa_output.pci-0000_00_1f.3.analog-stereo`, porta attiva `analog-output-lineout`, non silenziato, volume al quaranta per cento su entrambi i canali, e uscita predefinita puntata su di esso. La sessione attiva del posto è tornata a essere quella dell'utente.

*Le tre prove, e la ragione per cui sono state tre.* La prima, con il messaggio vocale a due canali, è stata mandata mentre l'utente era alla postazione Windows e le cuffie erano sulla macchina Ubuntu: ha suonato in una stanza vuota. È un mio errore di conduzione e non della catena, e la lezione è ovvia una volta detta, cioè che una prova di ascolto richiede che chi ascolta sia dove esce il suono, il che su due macchine distinte non è implicito. La seconda, ripetuta per tre minuti per dare il tempo di spostarsi, non ha avuto riscontro perché l'utente non era ancora là. La terza, un tono puro a 440 Hz ripetuto per cinque minuti su richiesta dell'utente, è stata udita: l'utente riporta il tono presente e alternato correttamente fra i due canali.

*Il limite della terza prova, dichiarato dall'utente stesso e conservato come tale.* Essersi seduti a riproduzione già iniziata conferma l'alternanza fra i canali ma non il loro ordine assoluto, quindi da questa prova non discende che il canale sinistro esca dall'orecchio sinistro: un cablaggio invertito produrrebbe esattamente la stessa osservazione. È una distinzione che conta per il progetto e non per l'ascolto ordinario, perché una inversione di canali rispecchierebbe ogni misura stereo successiva. La verifica mirata, cioè un tono sul solo canale sinistro, è stata lanciata subito dopo.

*Il criterio sui gruppi, che va riformulato e non spuntato.* PA-019 chiedeva fra i suoi criteri che il processo `wireplumber` della sessione portasse i gruppi 29 e 982. Non li porta, e non poteva portarli: quel processo non appartiene alla sessione grafica ma al gestore di servizi dell'utente, che nasce al primo accesso e resta vivo finché esiste almeno una sessione di quell'utente, comprese le connessioni SSH aperte da qui. Un logout grafico non lo tocca, e nemmeno un riavvio del servizio lo aiuterebbe, perché un servizio riavviato eredita i gruppi del gestore che lo lancia. L'unica via pulita è il riavvio della macchina.

La prescrizione che avevo dato, cioè fare un logout vero invece di sbloccare lo schermo, era quindi insufficiente e va corretta apertamente. Corretta è anche la lettura del criterio: i gruppi sono una ridondanza rispetto alla lista di controllo di accesso, non un requisito funzionale, e la prova che siano ridondanti è che la catena funziona senza di essi. Il criterio resta utile come igiene e va soddisfatto al prossimo riavvio, che è comunque opportuno per altre ragioni, cioè dodici giorni di attività e ventuno aggiornamenti in attesa; non è però un criterio di funzionamento e non va trattato come tale.

*Una osservazione dell'utente sul pannello frontale, conservata perché è un dato.* Il case espone davanti due sole prese, con il simbolo delle cuffie e quello del microfono, e l'utente ha preferito il jack verde posteriore. La prova non dice quindi nulla sul fatto che il cavetto interno del pannello frontale sia innestato sulla scheda madre, che resta non verificato e verificabile in trenta secondi ricollegando davanti e guardando se compaia una porta distinta.

Verificato con: `pactl list cards` prima e dopo il collegamento, con i profili analogici che passano da `available: no` a `available: yes` e il profilo attivo che cambia da solo; `pactl list short sinks` e `pactl info`, che riportano il sink analogico come predefinito; `pactl list sinks` per porta attiva, stato di silenziamento e volume; `loginctl show-seat seat0`, che riporta attiva la sessione dell'utente; `speaker-test -c 2 -t wav` e `speaker-test -c 2 -t sine -f 440`, con l'ascolto riferito dall'utente; lettura della riga `Groups` in `/proc` del processo `wireplumber`, invariata, e della sua ora di avvio, che è quella del 2026-09-09.

Esito: fatto per la parte di uscita, con due punti dichiarati aperti, cioè l'ordine assoluto dei canali e i gruppi da rinnovare al prossimo riavvio. La parte di ingresso della fase 6 resta bloccata da PA-012 e non da questa verifica.

### MS-155 - Ordine dei canali verificato e simmetria misurata, dopo una prova contaminata da me: PA-019 si chiude

Perimetro: completa MS-154, che lasciava aperto l'ordine assoluto dei canali. Comprende una prova invalidata per mio errore, la sua diagnosi, la prova rifatta in forma non ambigua, e la misura della simmetria fra i canali su tutti i controlli della scheda. Chiude PA-019 per la parte di uscita.

Legame con il progetto: serve la fase 6 e, per suo tramite, ogni misura stereo futura. Una inversione dei canali si propaga a tutte le misure successive ed è invisibile all'ascolto ordinario, quindi va accertata prima e non dopo.

*L'errore, e come si è manifestato.* La prima verifica dell'ordine dei canali è stata lanciata mentre la prova precedente, un tono alternato di cinque minuti, era ancora in esecuzione. Le due riproduzioni hanno suonato insieme sullo stesso dispositivo: l'utente ha riferito di sentire il canale sinistro tre volte e poi anche il destro, che è esattamente il risultato atteso dalla somma di una prova sul solo canale sinistro e di una prova alternata. La misura era contaminata e non valeva nulla.

La causa è banale e vale enunciarla come regola. Ho lanciato una prova nuova senza accertare che la precedente fosse terminata, fidandomi del fatto che fosse stata avviata con un limite di tempo: il limite era di trecento secondi e non erano trascorsi. Una riproduzione non lascia traccia visibile a chi la lancia da remoto, quindi lo stato va letto e non dedotto dal tempo che si crede passato. Il controllo costa un comando, cioè cercare il processo di prova e contare i flussi attivi verso l'uscita, e da qui in avanti precede ogni prova di ascolto.

*La prova rifatta, in forma non ambigua.* Fermate le riproduzioni e verificato il silenzio, cioè nessun processo di prova e zero flussi verso l'uscita, la sequenza è stata ricostruita in modo che nessun dato dipenda dall'istante in cui l'ascoltatore si siede: trenta secondi di silenzio, poi venti secondi sul solo canale sinistro, poi quattro secondi di silenzio, poi venti secondi sul solo canale destro. L'utente riferisce il primo tono a sinistra e il secondo a destra. I canali non sono invertiti.

Vale registrare il metodo oltre l'esito, perché è il punto in cui due tentativi precedenti erano falliti. Un tono puro non dichiara la propria identità, quindi una prova alternata dice soltanto che i canali sono due e distinti, non quale sia quale: l'informazione sull'ordine sta nell'istante iniziale, che l'ascoltatore perde se arriva a riproduzione avviata. Le due forme che risolvono il problema sono il segnale parlato, che dichiara il proprio nome ed è quindi indipendente dal momento in cui si ascolta, oppure una sequenza con una pausa iniziale dichiarata, che riporta l'istante iniziale sotto il controllo di chi ascolta. La seconda è quella usata qui, su preferenza dell'utente per il tono puro.

*La simmetria fra i canali, misurata su richiesta dell'utente.* Nessuno scarto in alcun punto della catena. Il sink di PipeWire riporta i due canali a 26214 su 65536, cioè il quaranta per cento, pari a meno 23,88 dB, identici. Sul mixer della scheda tutti i controlli stereo di riproduzione hanno i due canali allineati: `PCM` a 251 su 255, cioè il novantotto per cento, pari a meno 0,80 dB su entrambi; `Front` a 64 su 64, cioè il cento per cento, pari a 0 dB su entrambi; `Surround` idem. Non esiste alcuno sbilanciamento né una impostazione di bilanciamento diversa da zero.

Il limite di questa misura va dichiarato, perché la domanda dell'utente riguardava la sonorità percepita e la misura risponde a una domanda più stretta. Ciò che è stato accertato è che la catena elettrica e digitale tratta i due canali in modo identico; che il suono arrivi alle due orecchie con lo stesso livello dipende anche dalle cuffie e dal loro accoppiamento, e non è verificabile senza un microfono di misura, che è precisamente ciò che PA-012 blocca. La distinzione non è pedanteria: una asimmetria di trasduttore è possibile e questa misura non la escluderebbe.

*Un accertamento non cercato, sul pannello frontale.* Il controllo `Headphone` del mixer risulta a zero per cento e disattivato, mentre il controllo `Front`, che governa l'uscita di linea posteriore in uso, è al cento per cento. Ne segue che l'uscita del pannello frontale non è guasta né scollegata: è silenziata a livello di mixer. Se in futuro servisse usarla, il rimedio è attivare quel controllo e alzarlo, non cercare un cavetto interno. L'osservazione nasce dal fatto che l'utente aveva chiesto se potesse collegare le cuffie davanti, e in MS-154 la questione era rimasta non verificata.

Verificato con: `pgrep` sul processo di prova e `pactl list short sink-inputs`, che riportano rispettivamente il processo ancora vivo durante la contaminazione e poi nessun processo e zero flussi dopo l'arresto; sequenza con pausa iniziale dichiarata e due riproduzioni a canale singolo, con l'esito riferito dall'utente; `pactl list sinks` per i volumi per canale; `amixer -c 0 scontents` con estrazione dei controlli stereo di riproduzione, che riporta i due canali allineati su tutti; `wpctl get-volume` per il volume complessivo.

Esito: fatto. PA-019 è compiuta per la parte di uscita: la scheda è vista, l'uscita analogica è attiva e udibile, i canali non sono invertiti e la catena è simmetrica. Restano dichiarati due punti, cioè i gruppi del processo di gestione audio da rinnovare al prossimo riavvio, come spiegato in MS-154, e la simmetria acustica alle orecchie, non verificabile senza microfono di misura.

### MS-156 - Il riavvio rinnova i gruppi e chiude PA-019, e la stessa lettura rimuove un limite dichiarato e spiega il ritorno all'uscita digitale

Perimetro: lettura della macchina `alessio-ubuntustudio` dopo il riavvio eseguito dall'utente il 2026-09-21 alle 16:04, condotta da una sessione SSH, per chiudere il criterio residuo di PA-019 sui gruppi del processo di gestione audio. Comprende due accertamenti che non erano cercati, cioè la caduta del limite di osservabilità dichiarato in MS-146 e la causa misurata del ritorno all'uscita digitale, più un residuo del sistema precedente trovato per via di essi. Nessuna modifica alla macchina.

Legame con il progetto: serve la fase 6 e, per suo tramite, la fase 1 del workflow, cioè la misura reale della stanza. Chiude PA-019 nel quarto e ultimo criterio, nella forma riformulata che MS-154 gli aveva dato. Non sblocca la parte di ingresso della fase 6, che resta ferma su PA-012, e non cambia il fronte, che resta PA-020.

*Il criterio sui gruppi, soddisfatto come MS-154 aveva previsto.* Il processo `wireplumber` è nato con la macchina, alle 16:04:08, e porta i gruppi `4 24 27 29 30 46 100 111 115 978 982 1000`, dove il 29 è `audio` e il 982 è `pipewire`. La previsione di MS-154, cioè che solo un riavvio li rinnovasse perché quel processo appartiene al gestore di servizi dell'utente e non alla sessione grafica, è confermata dalla misura e non soltanto dall'argomento. Nell'elenco compare anche il 978, che è `veeam`, ed è la stessa proprietà di MS-150 vista dall'altro lato: un processo nato dopo una assegnazione di gruppi li porta tutti, e uno nato prima non ne porta nessuno dei nuovi, senza che la differenza dipenda da quale gruppo sia.

*Un limite dichiarato che è caduto, e la correzione di portata che ne discende.* La sessione attiva del posto è ora la 3, cioè quella dell'utente su `tty2`, e non più quella dello schermo di accesso; la mia connessione SSH è la sessione 5 e non ha alcun posto assegnato. Di conseguenza `pactl` interrogato da SSH riporta ora la scheda integrata e una uscita reale, dove in MS-146 riportava `auto_null`. Questo non ritira MS-146, che aveva isolato correttamente le sue due cause, ma ne corregge la portata: la proprietà "una misura raccolta da SSH non è informativa" era contingente su chi tenesse il posto, non sul canale SSH, e quindi non era una proprietà del canale. La lezione generale vale oltre l'audio, ed è la stessa che la regola sulle fonti web non recuperabili enuncia per le etichette di indisponibilità: quando cade lo stato che rendeva cieca una misura, il canale va riprovato invece di essere dato per cieco, altrimenti un limite sopravvive alla propria causa e scoraggia dal misurare.

*Perché l'uscita è tornata digitale, e perché non è una regressione.* L'uscita predefinita è ora il sink S/PDIF `alsa_output.pci-0000_00_1f.3.iec958-stereo`, sospeso, con porta attiva `iec958-stereo-output` e volume al quaranta per cento su entrambi i canali, mentre tutti i profili analogici della scheda sono dichiarati `available: no` e le due porte di uscita analogica, cioè `analog-output-lineout` e `analog-output-headphones`, sono dichiarate non disponibili. La tentazione era di leggerlo come la scelta di MS-154 non sopravvissuta al riavvio, e la lettura dello stato che WirePlumber conserva su disco mostra che non c'è alcuna scelta che non sia sopravvissuta: il file `default-profile` non esiste, quindi nessuna preferenza di profilo è mai stata registrata. In MS-154 il passaggio all'analogico non fu una scelta di nessuno ma una selezione automatica, avvenuta perché il profilo era diventato disponibile e la sua priorità è 6565 contro i 5565 del digitale. Non c'era nulla da persistere, e nulla ha mancato di persistere.

Resta da spiegare perché, a jack vuoti, la selezione automatica scelga proprio il digitale, e la spiegazione sta in una riga del censimento delle porte: `iec958-stereo-output` dichiara `availability unknown`, mentre ogni porta analogica dichiara di non essere disponibile. Una presa S/PDIF non ha rilevamento della presenza, quindi non può dichiararsi vuota, quindi non viene mai esclusa. Ne segue che con ogni jack analogico libero il profilo digitale è il più prioritario fra quelli disponibili, e che l'uscita analogica torna da sé appena si collega qualcosa al jack posteriore, che è esattamente ciò che MS-154 aveva osservato accadere. Non c'è nulla da riparare e nulla da rendere persistente.

*Un residuo del sistema precedente, trovato dalla stessa lettura e conservato perché è una trappola.* Il file `default-routes` porta la data del 14 novembre 2025, cioè precede di quasi quattro mesi la reinstallazione dell'8 settembre, ed è sopravvissuto dentro `/home` insieme al resto. Il suo contenuto riguarda soltanto `hdmi-output-0` e ne registra i volumi dei due canali a zero, senza silenziamento. Oggi è inerte perché l'HDMI non è in uso, e va scritto proprio perché è inerte oggi: chi in futuro selezionasse l'uscita HDMI la troverebbe muta con il volume a zero e il silenziamento disattivato, cioè nella forma in cui un difetto di configurazione si maschera da guasto, e cercherebbe la causa nel cavo o nella scheda. È anche, di passaggio, una conferma indipendente in più che `/home` sia sopravvissuto alla reinstallazione, dopo quella per identificativo univoco di MS-144: un file di stato di undici mesi prima non potrebbe esistere su un filesystem creato a settembre.

*La pagina prescrittiva che questo fronte doveva avere e non aveva.* La verifica della catena audio è un fronte operativo, cioè lavoro fatto di comandi eseguiti su una macchina, e la prescrizione del 2026-09-17 vuole che ogni fronte operativo produca anche la sequenza replicabile con il solo troubleshooting accanto. MS-146, MS-154 e MS-155 non l'hanno prodotta, e la conoscenza è rimasta soltanto nella forma cronologica del registro, che non è la forma utile a chi deve rifare la stessa verifica. La lacuna è colmata in questo giro con `docs/10-ambiente/catena-audio-pipewire.md`, in forma parametrica e riusabile su un'altra macchina, con otto voci di troubleshooting tutte nate da casi incontrati fra il 20 e il 21 settembre e nessuna prevista. Il fatto che la lacuna sia stata notata soltanto adesso, quattro giorni dopo l'introduzione della regola, è esso stesso il dato: una prescrizione nuova non si applica da sé al fronte che è già in corso quando arriva.

Verificato con: `uptime -p`, che riporta la macchina in piedi da quattro minuti al momento della lettura; `id`, che conferma l'appartenenza dell'utente ai dodici gruppi; `pgrep -x wireplumber` con la riga `Groups` di `/proc/2335/status` e `ps -o lstart=`, che datano il processo alle 16:04:08 del 2026-09-21 e ne elencano i gruppi; `loginctl list-sessions` e `loginctl show-seat seat0`, che riportano `ActiveSession=3` sulla sessione dell'utente e la connessione SSH come sessione senza posto; `pactl info`, `pactl list short cards` e `pactl list short sinks`, che riportano la scheda e il sink digitale come predefinito; `pactl list cards`, per la disponibilità di ogni profilo e di ogni porta e per il profilo attivo; `pactl list sinks`, per porta attiva, silenziamento e volumi per canale; `wpctl status`; e la lettura di `~/.local/state/wireplumber/`, che elenca `default-routes` datato 14 novembre 2025 e `stream-properties` riscritto al riavvio, e non contiene alcun `default-profile`.

Esito: fatto. PA-019 è compiuta in tutti e quattro i criteri, il quarto nella forma riformulata da MS-154, e nulla resta da correggere sulla macchina. Le due conseguenze da tenere sono dichiarazioni e non lavori: l'uscita analogica dipende da un jack occupato e non da una impostazione conservata, quindi non va cercata una persistenza che non serve; e il giorno in cui arriverà l'interfaccia esterna di PA-012 la macchina avrà due schede, la selezione automatica per priorità non basterà più, e il dispositivo predefinito diventerà una scelta da dichiarare.

### MS-157 - Due interventi committati e non scritti, e il presidio che non esiste sulle pagine prescrittive

Perimetro: chiusura in forma additiva di due lacune di MS-156, trovate da un controllo di completezza chiesto dall'utente dopo il commit `0112973`. Comprende la registrazione di due interventi eseguiti e committati senza comparire in alcun microstep, cioè l'aggiornamento dei criteri di PA-019 in `tools/check-pending-actions.py` e la propagazione del blocco dell'ambiente al progetto gemello, e l'apertura di una voce in PA-003 per un presidio che non esiste. Nessuna modifica alla macchina.

Legame con il progetto: nessuna fase, e va detto apertamente perché un legame inventato è peggio di un legame assente. Serve l'integrità del registro, che è la condizione perché ogni altra voce sia credibile: un registro che omette una modifica al codice non è un registro incompleto, è un registro di cui non si sa quanto ometta.

*Che cosa non era scritto, e perché la forma è additiva e non una correzione.* Due interventi del giro di lavoro precedente sono entrati nel commit `0112973` senza comparire in alcuna voce. Il primo è l'aggiornamento dei sei criteri di PA-019 in `tools/check-pending-actions.py`, dove il quarto è passato da da confermare a mano a soddisfatto con il riferimento a MS-156, e dove sono state aggiunte le due dichiarazioni che sostituiscono altrettanti lavori, cioè la dipendenza dell'uscita analogica da un jack occupato e la scelta esplicita del dispositivo predefinito quando le schede saranno due. Il secondo è la propagazione al gemello dei due file del blocco dell'ambiente, eseguita con `tools/sync-ambiente.py`, che riporta il blocco a sedici file su sedici allineati. Nessuno dei due è un intervento grande, ed è precisamente la ragione per cui è sfuggito: MS-156 è stata scritta intorno alla lettura della macchina, e ciò che è venuto dopo la lettura non è stato riletto prima di committare. La convenzione vieta di riscrivere una voce committata, quindi questa voce esiste al posto di una correzione di MS-156.

*Il presidio che non esiste, ed è la parte che vale oltre il caso.* MS-156 ha registrato che il fronte della catena audio era rimasto quattro giorni senza la propria pagina prescrittiva dopo l'arrivo della regola che la impone, e ne ha tratto la lezione che una prescrizione nuova non si applica da sé al fronte già in corso. Il complemento è che nessuno lo verifica: la catena di controlli prima di un commit conta sette comandi e nessuno di essi chiede se un fronte operativo abbia prodotto la propria pagina, quindi la risposta dipende da chi se ne ricorda, che è esattamente la condizione in cui il verso discendente del riallineamento al template è rimasto indietro fra il 12 e il 17 settembre. È la stessa forma del buco che `check-copie-modelli.py` ha chiuso il 2026-09-17, dove due esemplari dello stesso strumento non avevano nulla che li tenesse insieme.

La voce va a PA-003 perché la correzione appartiene al template e non a questo solo progetto, e vi va dichiarata come candidata e non come presidio già progettato. La ragione è che il difetto è più difficile di quello delle copie: che cosa conti come fronte operativo è un giudizio, non un fatto leggibile dal disco, e un controllo che deve giudicare produce falsi positivi, che è il modo documentato in cui un presidio si impara a ignorare. La formulazione che ha qualche speranza di funzionare è indiretta e sta nella voce: non chiedere se un fronte abbia la sua pagina, ma segnalare quando un microstep nuovo nomina comandi eseguiti su una macchina e nessuna pagina sotto `docs/` è stata toccata nello stesso commit.

Verificato con: `git log --oneline -3` e `git status --short`, che riportano `0112973` in testa e l'albero di lavoro pulito, quindi i due interventi committati; ricerca dentro il testo di MS-156 dei nomi dello strumento e della propagazione, senza alcuna occorrenza, che è la misura della lacuna e non la sua supposizione; `python tools/sync-ambiente.py --check`, che riporta sedici file nel blocco, sedici allineati, zero da propagare e nessuna deriva; e la riesecuzione della catena completa prima del commit, con i sette comandi tutti a codice di uscita zero.

Esito: fatto per la registrazione. Resta aperta la voce di PA-003, che è una decisione sul template e non lavoro eseguibile da qui, e resta vera la conseguenza che questa voce non nasconde: il controllo che ha trovato le due lacune è stato una domanda dell'utente e non un comando, quindi finché quel presidio non esiste la completezza del registro dipende da chi chiede.

### MS-158 - Il rilievo della stanza diventa un protocollo con le sue tabelle, ricavato da ciò che le tre fasi a valle pretendono

Perimetro: pagina nuova `docs/35-rilievo-geometrico.md`, allegato operativo alla fase 2, con la convenzione di origine e assi, la sequenza del rilievo, le sei tabelle da riempire e la procedura di ricalco in Blender; rimandi da `docs/README.md` e dalla pagina della fase 2; nota di stato in PA-020. Nessun comando eseguito su alcuna macchina e nessuna misura raccolta: è lavoro documentale, e la stanza non è stata toccata.

Legame con il progetto: serve la fase 2, che è bloccata sul dato geometrico e non sullo strumento, e per suo tramite la fase 3, che dalle stesse quote ricava le frequenze modali e il riverbero. Serve anche la fase 1, perché una delle quote richieste è l'altezza dell'orecchio al punto di ascolto, che è la quota del microfono. È il lavoro di PA-020 eseguibile senza la stanza e senza il metro laser.

*Che cosa aggiunge, dato che la fase 2 aveva già una sezione sulla geometria.* Quella sezione, scritta il 2026-09-21, decide il metodo, cioè quale via si sceglie fra le quattro e perché una scansione è una sagoma e non un modello. Questa pagina decide altro: quali quote si prendono, in quale ordine e su che cosa si scrivono. La distinzione non è di dettaglio, perché un metodo scelto non impedisce a un rilievo di tornare incompleto, e la sola cosa che la impedisce è avere davanti l'elenco dei campi mentre si è dentro la stanza con la scala.

*Come l'elenco dei campi è stato ricavato, che è la parte non ovvia.* Non da una idea di che cosa serva a un rilievo, ma leggendo che cosa le tre fasi a valle chiedono e mettendo in tabella l'unione. La fase 3 vuole le tre dimensioni principali per i modi, e poiché il soffitto è spiovente vuole due altezze e una pendenza invece di una altezza media, perché la sua stessa pagina dichiara la formula rettangolare una approssimazione di partenza. La stessa fase vuole il tempo di riverberazione secondo Sabine, che dipende dal volume e dall'assorbimento delle superfici. La fase 2 vuole gli ingombri e le posizioni di mobili, schermo, scrivania e seduta, non la loro forma. La fase 1 vuole l'altezza dell'orecchio e registra due vincoli già noti, cioè il punto di ascolto sul lato basso del soffitto e la scrivania non negoziabile.

Da questa unione emerge il campo che si dimentica, ed è il motivo per cui valeva fare l'esercizio: il materiale di ogni superficie. L'assorbimento non è una proprietà della geometria, quindi chi misura pensando al modello geometrico non lo prende, e a differenza di ogni altro campo non è ricostruibile dopo, perché un'area si ricalcola dalle quote mentre un intonaco non si indovina da casa. Una tabella intera, la C, esiste per quel campo.

*Due prescrizioni che nascono dallo strumento e non dalla geometria.* La pendenza del soffitto si calcola come rapporto fra due altezze verticali e una distanza orizzontale, e non si misura come angolo: uno strumento a mano libera puntato lungo una superficie inclinata misura la distanza dal primo ostacolo, e l'angolo che si crede di leggere dipende da come si tiene lo strumento, mentre le tre misure che compongono il rapporto sono tre misure che quello strumento fa bene. E le posizioni si prendono sempre dai due muri dell'origine dichiarata e mai da un mobile, perché un mobile si sposta e una parete no, con la variante esplicita di misurare dal muro opposto annotando che la quota è complementare.

*L'ordine della sequenza, che non è indifferente e ha due punti fragili.* Il guscio col laser va prima della scansione, perché la scansione si porta in scala sulle quote del guscio e fare il contrario significa scalarla su se stessa. E le quattro diagonali di controllo vanno prese nella stessa sessione della scansione, con gli arredi fermi: se qualcuno sposta una sedia fra le due, il confronto misura lo spostamento della sedia e lo attribuisce alla deriva di scala, che è il difetto che le diagonali esistono per cogliere.

*Che cosa la pagina deliberatamente non ha, e perché l'assenza è dichiarata.* Non ha la sezione di troubleshooting che le pagine prescrittive di questo progetto portano, e le sue tabelle sono vuote. Le due cose hanno la stessa causa: il protocollo non è stato eseguito, e una voce di troubleshooting nasce da un caso incontrato e non da una previsione. La pagina è quindi un protocollo oggi e diventerà la pagina prescrittiva del fronte dopo l'esecuzione, quando le tabelle porteranno numeri e il troubleshooting porterà i casi. Vale distinguerlo dalla lezione di MS-157, che riguarda il non scrivere la pagina dopo aver eseguito: scrivere il protocollo prima non è la stessa cosa e non la contraddice, perché ciò che si scrive prima è la lista dei campi e non l'esito di comandi.

Verificato con: lettura delle tre pagine a valle per ricavare i campi invece di inventarli, cioè `docs/40-analisi-octave.md` per i modi e per Sabine, `docs/30-modellazione-e-simulazione.md` per il contenuto del modello e per l'ordine di grandezza della precisione, e `docs/20-misura-stanza.md` per l'altezza dell'orecchio e i due vincoli noti; catena completa di verifica prima del commit, con i sette comandi a codice di uscita zero.

Esito: fatto per la parte di PA-020 che non richiede la stanza. Restano due cose e nessuna delle due è lavoro documentale: il metro laser, che è l'unico acquisto e serve alla sola tabella A, e la verifica da trenta secondi di Scaniverse sul dispositivo. Le tabelle da B a E si possono riempire con un metro a nastro prima che il laser arrivi, e spezzare il lavoro così è sensato invece di attenderlo tutto.

### MS-159 - Scaniverse sul dispositivo: due esperienze e non una, e il piano gratuito dichiarato dall'app stessa

Perimetro: primo tratto della verifica sul dispositivo prevista da PA-020, eseguita dall'utente sul Galaxy S25 Ultra il 2026-09-21 e osservata da me su tre schermate. Comprende l'installazione, la scelta fra le due esperienze che l'applicazione propone all'avvio, il salto dell'accesso e la schermata iniziale raggiunta. Non comprende le modalità di cattura, la verifica di scala e l'esportazione, che restano aperte.

Legame con il progetto: serve PA-020 e per suo tramite la fase 2, perché da questa verifica dipende se la via decisa, cioè la scansione come sagoma su cui ricalcare il modello, sia praticabile su questo dispositivo o vada sostituita dal rilievo interamente a metro laser.

*Che cosa l'applicazione dichiara di sé, e perché conta più delle fonti di terze parti già in registro.* All'avvio chiede di scegliere fra due esperienze. La prima si chiama `Classic` e dichiara tre proprietà in tre righe: cattura a scansione singola, elaborazione e archiviazione sul dispositivo, gratuita e senza account richiesto. La seconda è marcata `NEW` e dichiara scansioni combinate, splat gaussiani da fotocamere a 360 gradi, collaborazione di gruppo e sistema di posizionamento visivo, con una riga in corsivo che avverte che le funzioni di `Classic` arriveranno in seguito. L'accesso si può saltare, e l'utente lo ha saltato con la voce `or skip for now`.

Questo promuove a fatto, e da fonte autorevole, ciò che il registro delle fonti aveva da articoli di confronto di terze parti: l'elaborazione è sul dispositivo e non richiede account, perché lo dichiara l'applicazione stessa nella schermata di scelta. Cade quindi metà dell'incognita che avevo sollevato sull'esportazione: l'account non serve. Resta l'altra metà, cioè se l'esportazione in OBJ sia essa stessa gratuita, che nessuna delle due fonti copre e che va vista sul dispositivo.

*L'avvertenza che le due esperienze introducono, e che non era prevista in PA-020.* La via scelta dal progetto ha bisogno di una mesh, perché il modello si ricalca in Blender su una sagoma e perché a valle servono piani a cui assegnare un assorbimento. Uno splat gaussiano non è una mesh: è una rappresentazione del campo luminoso fatta di primitive sfocate, ottima per guardare una scena e inservibile per ricavarne superfici. Ne segue che l'esperienza utile a questo progetto è `Classic` e non quella nuova, e che la riga sulle funzioni di `Classic` in arrivo è la dichiarazione che la seconda non le ha ancora.

*La schermata raggiunta, e il ritiro di una mia inferenza.* La terza schermata mostra una mappa a tutto schermo con un pulsante di cattura circolare al centro in basso, la libreria in basso a sinistra e una voce di scoperta in basso a destra, e la mappa è centrata su una località predefinita e non su quella dell'utente. Da questa forma avevo inferito che la sessione potesse non essere dentro `Classic` pur avendola l'utente selezionata, e l'inferenza era sbagliata: va ritirata invece di essere lasciata cadere. Aprendo la cattura compaiono due modalità, cioè `Splat`, descritta come scansioni realistiche condivisibili sulla mappa, e `Mesh`, descritta come modalità classica con più opzioni di esportazione. L'utente conferma di avere scelto `Classic` all'avvio.

Ne segue il fatto che correggeva l'inferenza, e vale oltre questa applicazione: `Classic` non è una seconda interfaccia ma una modalità di cattura dentro la stessa, e la schermata iniziale con la mappa è condivisa fra le due esperienze. Avere dedotto l'una dall'altra era un ragionamento sulla grafica al posto di una lettura, cioè la stessa forma di errore che questo progetto ha già registrato altrove: la scelta dichiarata dall'utente all'avvio e la modalità disponibile alla cattura sono due cose distinte, e solo la seconda decide che cosa si ottiene. La descrizione di `Mesh` porta inoltre un indizio a favore sul criterio ancora aperto, perché nomina le opzioni di esportazione come ciò che la distingue, ma un indizio non è una verifica e l'esportazione resta da provare.

Vale registrare anche una conseguenza che non riguarda la geometria. Una esperienza costruita attorno a una mappa pubblica e alla condivisione porta il rischio che una scansione di un ambiente domestico finisca caricata, e l'archiviazione sul dispositivo dichiarata da `Classic` è quindi anche la scelta corretta sul piano della riservatezza, non solo su quello tecnico. Non è stato osservato alcun caricamento e non si sta affermando che avvenga: si sta scegliendo la via che non lo rende possibile.

Verificato con: quattro schermate raccolte dall'utente sul dispositivo e lette da me, cioè la schermata di scelta dell'esperienza con il testo delle due schede, la schermata di accesso con la voce per saltarlo, la schermata iniziale raggiunta dopo il salto, e il selettore di modalità aperto dal pulsante di cattura con le due voci e le loro descrizioni; più la conferma dell'utente su quale esperienza avesse scelto. Nessuna misura di lunghezza è stata raccolta, e nessuna affermazione su scala o esportazione discende da queste schermate.

Esito: aperto, con il primo dei tre punti chiuso. La modalità che produce una mesh esiste su questo dispositivo e si chiama `Mesh`, quindi la via decisa da PA-020 è praticabile e l'applicazione non va sostituita. Restano la verifica di scala su un oggetto di lunghezza nota e l'esportazione in OBJ senza pagare, in questo ordine, perché una esportazione riuscita da un modello fuori scala non servirebbe a niente.

### MS-160 - Uno strumento per la verifica di scala senza strumenti di misura, e il limite che ne ha cambiato la prova

Perimetro: strumento nuovo `tools/obj-bbox.py`, che misura il parallelepipedo contenente di una mesh OBJ e lo confronta con una dimensione nota; prova su quattro casi costruiti, compresi due che devono fallire; e correzione della forma della prova sul dispositivo dopo un limite trovato scrivendolo. Nessun comando eseguito sulla macchina Ubuntu.

Legame con il progetto: serve PA-020 e la fase 2. La verifica di scala è il presidio che la pagina della fase 2 prescrive contro la deriva, ed è anche la tabella F di `docs/35-rilievo-geometrico.md`; finora era una prescrizione senza attuazione, perché confrontare quattro diagonali a occhio su un modello non è una misura.

*Il problema che risolve, e perché non è il problema che sembra.* La premessa della via scelta è che la scala della scansione esca metrica. Verificarlo sembrava richiedere un metro a nastro, che l'utente non aveva in ufficio, oppure lo strumento di misura dell'applicazione, che potrebbe non esistere e la cui precisione non è dichiarata da nessuno. Nessuna delle due serve: basta scansionare un oggetto le cui dimensioni siano note per norma e non per misura, cioè un foglio A4, che per la ISO 216 misura 297 per 210 millimetri. Il riferimento non si misura, si sa, ed è disponibile in qualunque ufficio.

*Il limite trovato scrivendo lo strumento, che ha cambiato la prova invece dello strumento.* Il parallelepipedo contenente è allineato agli assi del file e non all'oggetto, quindi un oggetto ruotato rispetto a quegli assi produce lati più lunghi del vero. La rotazione attorno alla verticale di una scansione è arbitraria, perché dipende da dove stava chi scansionava, quindi i due lati orizzontali sovrastimano di una quantità che nessuno conosce. Il lato verticale invece è affidabile, perché ARCore conosce la direzione della gravità e la mesh ne esce allineata.

La conseguenza è la parte utile, e va nella direzione opposta a quella che viene istintiva: non si sofistica lo strumento, cioè non si calcola un parallelepipedo orientato, si cambia il modo di appoggiare il foglio. Messo in piedi con il lato lungo verticale, il foglio offre 297 millimetri lungo il solo asse di cui si possa fidare, e se è la cosa più alta nell'inquadratura l'estensione verticale della mesh è la sua. Il limite è dichiarato nel docstring e ristampato a ogni esecuzione accanto ai numeri, perché un limite che vive nella sola documentazione non raggiunge chi legge l'uscita.

*La prova, e i due casi che devono fallire.* Quattro casi costruiti a mano con vertici noti. Una scatola da 297 per 210 per 50 millimetri in metri passa con scarto nullo. La stessa in centimetri, cioè con la scala sbagliata di un fattore cento, esce con codice diverso da zero. La stessa con una deriva del cinque per cento esce con codice diverso da zero, e con la soglia alzata a sei torna a zero, che è la prova che la soglia si applica davvero e che il fallimento non era accidentale. Un file senza vertici viene rifiutato invece di produrre un parallelepipedo di dimensione infinita.

Va registrato un mio errore nella lettura della prova, perché è una ripetizione. Il codice di uscita del terzo caso l'ho letto attraverso una pipeline, e ho ottenuto zero da un comando che restituiva uno: lo stato di una pipeline è quello dell'ultimo elemento, ed è esattamente la trappola già registrata nella voce B del troubleshooting di `docs/10-ambiente/veeam-agent-linux.md`. La regola esisteva e non l'ho applicata, il che la rende una lezione sull'uso delle proprie regole e non una scoperta.

Verificato con: le quattro esecuzioni descritte sopra con i codici di uscita letti senza pipeline, cioè 0, 1, 1 e 1, più la quinta con la soglia a sei che torna 0; catena completa di verifica prima del commit.

Esito: fatto. Lo strumento è pronto per l'esito della scansione di prova, e la stessa esecuzione servirà poi alle quattro diagonali della tabella F sul modello vero.

### MS-161 - La cattura di prova fallisce, e il fallimento è quello previsto dalla pagina della fase 2: due modalità di elaborazione, non una

Perimetro: primo tentativo di cattura con Scaniverse sul Galaxy S25 Ultra, il 2026-09-21, su un foglio A4 bianco e una scatolina di cartone sopra una scrivania. Comprende il selettore delle modalità di elaborazione, il fallimento dell'elaborazione e un artefatto di cattura dello schermo. Osservato su tre schermate raccolte dall'utente. Nessuna mesh prodotta e nessuna misura di scala raccolta.

Legame con il progetto: serve PA-020 e la fase 2. Il fallimento non blocca la via scelta e la precisa, perché identifica quale delle due modalità di elaborazione serve alla stanza e quale al riferimento di scala, che non sono la stessa.

*Il fatto che corregge una semplificazione delle note, ed è il più importante.* A elaborazione l'applicazione chiede di scegliere fra due modalità, descritte con le sue parole. `Area`, dichiarata migliore per stanze e spazi, che lavora per `DEPTH FUSION`, cioè per fusione di profondità. `Detail`, dichiarata migliore per oggetti con trama e marcata come raccomandata, che lavora per `PHOTOGRAMMETRY`. Ne segue che su questo dispositivo la via basata sulla profondità esiste e non è soltanto fotogrammetria, il che conferma per misura il supporto all'interfaccia di profondità di ARCore che le fonti dichiaravano, e smentisce la semplificazione con cui le note di ripresa avevano ridotto la verifica a guardare se una modalità basata su sensore ci fosse o no. La domanda era mal posta: non c'è un sensore dedicato, e c'è comunque una via per profondità.

Ne segue anche una conseguenza operativa che il protocollo non conteneva: la modalità per la stanza e `Area`, non quella raccomandata dall'applicazione. La raccomandazione dell'applicazione riguarda l'oggetto tipico dei suoi utenti e non l'ambiente, e seguirla per la stanza sarebbe seguire un consiglio dato per un altro caso.

*Il fallimento, e perché era prevedibile leggendo la nostra documentazione.* L'elaborazione in `Detail`, cioè fotogrammetrica, si è fermata con un errore riferito dall'utente, dopo essere arrivata al tre per cento del passo dichiarato come `MATCHING FEATURES`, cioè la ricerca di punti corrispondenti fra le immagini. Il soggetto era un foglio bianco e una scatolina di cartone chiaro, cioè due superfici quasi prive di trama: la fotogrammetria ricostruisce la geometria riconoscendo gli stessi punti in immagini diverse, quindi su superfici senza trama non ha nulla da riconoscere e si ferma esattamente al passo che porta quel nome. È il difetto strutturale del metodo che `docs/30-modellazione-e-simulazione.md` descrive per le pareti tinteggiate, incontrato su scala ridotta e sullo stesso principio.

La correzione è duplice e nessuna delle due parti è una rinuncia. Per il riferimento di scala si usa un foglio stampato invece di uno bianco, cioè una pagina di testo qualunque: resta esattamente 297 millimetri per la ISO 216, quindi il riferimento non perde nulla, e diventa una superficie ricchissima di punti riconoscibili. Per la stanza si usa `Area`, che non dipende dal riconoscimento della trama.

*L'artefatto di cattura, dichiarato come ipotesi e non come diagnosi.* Registrando lo schermo durante la cattura, l'anteprima della fotocamera appare come un motivo a righe diagonali rosse e bianche. L'ipotesi, non verificata, è che sia un artefatto della cattura dello schermo e non un guasto dell'applicazione: Android sostituisce con un motivo segnaposto le superfici che non consente di registrare, e l'anteprima della fotocamera è una di queste. Se l'ipotesi è giusta, sullo schermo reale l'anteprima si vede normalmente e il problema riguarda soltanto il tentativo di riprenderla. La verifica costa un secondo e non è stata fatta: consiste nel guardare lo schermo mentre si scansiona senza registrare.

Va detto perché l'ipotesi merita di essere scritta invece di essere data per ovvia. Se fosse falsa, cioè se le righe comparissero anche senza registrazione, il difetto sarebbe nell'accesso alla fotocamera e cambierebbe la natura del problema: una cattura fatta alla cieca non produce una mesh utile, e nessuna scelta di modalità di elaborazione la salverebbe.

Verificato con: tre schermate raccolte dall'utente, cioè il selettore delle modalità di elaborazione con il testo delle due schede, l'elaborazione in corso al tre per cento con il nome del passo, e l'anteprima a righe durante la registrazione dello schermo; più il resoconto dell'utente sull'errore di elaborazione, di cui non è stato raccolto il testo esatto.

Esito: aperto. Il tentativo va rifatto con un foglio stampato, e il testo esatto dell'errore va raccolto se si ripresenta. Restano aperti i due criteri di PA-020 sulla scala e sull'esportazione, e si aggiunge la verifica da un secondo sull'ipotesi dell'artefatto di cattura.

### MS-162 - Scansione abbandonata per decisione dell'utente, e il rilievo torna alla quarta via senza restare bloccato

Perimetro: chiusura del tentativo di rilievo per scansione con Scaniverse sul Galaxy S25 Ultra, dopo i tentativi del 2026-09-21 registrati in MS-159 e MS-161. Decisione dell'utente, presa il 2026-09-22 dopo ulteriori tentativi non riusciti di cui non è stato raccolto il dettaglio. Nessuna mesh è mai stata prodotta.

Legame con il progetto: riguarda PA-020 e la fase 2. Non sposta alcuna fase e non ne blocca nessuna, e la ragione per cui non le blocca è il contenuto di questa voce.

*Che cosa si sa e che cosa no, e la distinzione va tenuta.* Si sa che l'applicazione si installa, che `Classic` e `Mesh` esistono, che le modalità di elaborazione sono due e quale serve a un ambiente, e che un tentativo è fallito al passo di ricerca dei punti corrispondenti su un soggetto privo di trama, il che era prevedibile e aveva un rimedio identificato, cioè il foglio stampato. Non si sa se quel rimedio avrebbe funzionato, perché non è stato provato, e non si conosce il testo esatto di alcun errore. Ne segue che questa voce non afferma che Scaniverse non funzioni su questo dispositivo: afferma che non ha funzionato nei tentativi fatti, e che l'utente ha deciso di non proseguire. La distinzione non è formale, perché scrivere che l'applicazione non funziona scoraggerebbe un riesame futuro sulla base di una prova che non è stata fatta, che è esattamente ciò che la regola sulle fonti non recuperabili vieta a proposito delle etichette di indisponibilità.

*Perché PA-020 non resta bloccata, ed è il punto che rende la rinuncia sostenibile.* La scansione non era un requisito del rilievo ma un accorciamento: serviva a catturare a poco prezzo la disposizione degli arredi, che è la parte che a mano costa tempo, mentre le quote che decidono il comportamento modale, cioè il guscio e la pendenza del soffitto, dovevano comunque venire dal metro laser. La pagina della fase 2 elenca quattro vie e la quarta è il rilievo manuale; il protocollo di `docs/35-rilievo-geometrico.md` la copre già senza modifiche, perché le sue tabelle sono indipendenti dallo strumento con cui si riempiono. Ciò che si perde è misurabile in ore e non in giorni: gli arredi vanno misurati uno per uno invece di essere ricalcati da una sagoma.

Ne segue che PA-020 non aspetta più una verifica ma un acquisto, cioè il metro laser, e cambia quindi natura diventando simile a PA-012. Le tabelle da B a E restano riempibili anche prima, con un metro a nastro, perché riguardano aperture, materiali, arredi e punto di ascolto, che si misurano da vicino; la sola tabella A richiede il laser, perché le altezze del soffitto spiovente sono fuori portata.

*Che cosa resta annotato per un eventuale ritorno.* KIRI Engine resta la riserva già registrata in PA-020, con tre esportazioni gratuite a settimana, che per una stanza bastano. Provarla è una decisione di un altro giorno e non un lavoro in coda: va presa solo se il rilievo manuale degli arredi si rivelerà più oneroso del previsto, perché altrimenti sarebbe tempo speso per risparmiare tempo già speso.

Verificato con: il resoconto dell'utente, che è la sola fonte su questi ultimi tentativi e viene registrato come tale; le tre schermate di MS-159 e le tre di MS-161 per lo stato accertato prima della rinuncia.

Esito: fatto, nel senso che la decisione è presa e registrata. I due criteri di PA-020 sulla scala e sull'esportazione decadono insieme alla via che li richiedeva, e vanno letti come non più pertinenti invece che come non soddisfatti.

### MS-163 - Il quarto asse di identità re-istanziato sui valori letti, e una avvertenza del template che qui non si verifica

Perimetro: re-istanziazione della sezione sulla GitHub CLI in `.claude/rules/git-identity-and-repo.md`, con i valori rilevati su questa postazione invece di quelli generali del template; registrazione dell'autorizzazione OAuth in `docs/90-riferimenti/licenze-e-registrazioni.md`; aggiornamento di PA-016, che resta aperta sulla sola decisione relativa a `hooks-starter`. Tutti i comandi eseguiti sono di sola lettura e nessuna autenticazione è stata creata o modificata.

Legame con il progetto: nessuna fase del workflow, e va detto apertamente perché un legame inventato è peggio di un legame assente. Serve l'igiene dell'identità con cui questo repository viene firmato e operato, che è una condizione di correttezza del lavoro e non un suo contenuto.

*Perché era una re-istanziazione e non una copia, che è il punto che PA-016 aveva individuato.* Il template ha de-istanziato quel file nello stesso commit in cui gli aggiungeva il quarto asse, portandolo da un elenco di profili concreti a una procedura di rilevamento; qui il file è nella forma istanziata, con gli alias, le chiavi e le identità reali di questa postazione. Copiare la sezione del template avrebbe quindi introdotto una prescrizione di rilevare dentro un documento che esiste per contenere il rilevamento fatto. La forma corretta era eseguire il rilevamento e scriverne l'esito con la data, ed è quella adottata.

*Lo stato misurato.* L'eseguibile risponde: versione 2.100.0 del 2026-09-03, in `C:\Program Files\GitHub CLI\gh.exe`, raggiungibile per nome. L'autenticazione esiste e nomina l'account personale: accesso a `github.com` come `alesop95`, token nel portachiavi del sistema e non in un file, account attivo, protocollo delle operazioni git su `ssh`, ambiti `gist`, `read:org` e `repo`. Ne segue che su questa postazione i quattro assi coincidono tutti sull'identità personale, e la coincidenza è un fatto misurato e non una necessità: è precisamente perché possono divergere che si verificano uno per uno.

*L'avvertenza del template che qui non si verifica, e come va trattata.* Il template avverte che `gh` ricava il repository dal remoto `origin` e che un remoto costruito su un alias host può non essere riconosciuto, perché l'alias non è un host reale. Su questa postazione non si verifica: il remoto porta l'alias `github-personal` e `gh repo view` senza alcun flag risolve correttamente `alesop95/diy-2way-monitors-home` con codice di uscita zero. Non c'è contraddizione da sanare, perché l'avvertenza è formulata come possibilità e non come regola, e la lezione è quella generale del sistema: una avvertenza di livello generale si verifica dove si lavora invece di essere riportata, e l'esito della verifica si scrive.

La prescrizione sul flag `-R` resta comunque, e cambia il motivo per cui resta, che è la parte interessante. Non serve più a rimediare a un mancato riconoscimento che qui non avviene: serve a rendere il comando indipendente dalla cartella in cui viene incollato, che è la terza causa della sezione sul contesto di shell di `git-commands-format.md`, quella dei due messaggi di successo su tre in un repository sbagliato. Una prescrizione che sopravvive alla propria motivazione originale con una motivazione diversa va scritta con quella nuova, altrimenti al prossimo riesame cade insieme alla vecchia.

*Un fatto non cercato, dalla stessa misura.* Il repository è pubblico. Non cambia nulla sul piano tecnico e cambia la lettura di un vincolo che l'utente dà a ogni sessione, cioè che nessun valore segreto entri nei file tracciati: non è una formalità di igiene ma una condizione di riservatezza, perché l'intera documentazione di questo progetto è leggibile da chiunque. Il token della GitHub CLI vive nel portachiavi del sistema operativo, quindi la sezione nuova non ne espone alcuno, ed è la ragione per cui poteva essere scritta in un file tracciato.

*La registrazione dell'autorizzazione, e perché sta fra le licenze.* Non è una licenza, e sta lì per la stessa ragione per cui ci stanno le licenze: è un legame durevole fra questa macchina e un account, che sopravvive alla sessione e al progetto. La voce dichiara ambiti, luogo del token e pagina di revoca, e dichiara anche ciò che non si sa, cioè la data in cui l'autorizzazione fu concessa, perché lo stato dice che esiste e non quando è nata. Porta inoltre un accorgimento che vale oltre il caso: la revoca dal pannello non cancella il token dal portachiavi della macchina, quindi il gesto completo è duplice, e la metà che si dimentica è quella che nessuno guarda.

Verificato con: `which gh` e `gh --version`, che riportano il percorso e la versione; `gh auth status`, che nomina account, portachiavi, protocollo e ambiti; `git remote get-url origin`, che mostra l'alias; `gh repo view --json nameWithOwner` senza flag e con il repository indicato esplicitamente, entrambi a codice di uscita zero, il primo dei quali è la smentita dell'avvertenza sul riconoscimento e il secondo la conferma della visibilità pubblica; catena completa di verifica prima del commit.

Esito: fatto. PA-016 resta aperta su un solo punto, che è una decisione dell'utente e non lavoro: se istanziare qui il pacchetto `hooks-starter`. Va posta adesso, perché la condizione che ne rimandava la posa, cioè un fronte operativo aperto, non esiste più.

### MS-164 - hooks-starter istanziato con quattro hook su sette, e un modello che era fermo a tre

Perimetro: decisione dell'utente sul quinto criterio di PA-016, riallineamento del modello del pacchetto dal template, istanziazione dei quattordici file in `.claude/hooks/`, attivazione di quattro hook in `.claude/settings.json` e prova di ciascuno dei quattro. Chiude PA-016. Nessun comando eseguito sulla macchina Ubuntu.

Legame con il progetto: nessuna fase del workflow, e lo dichiaro invece di inventare un legame. Serve il ciclo di lavoro del progetto, cioè la procedura di ripresa che il `CLAUDE.md` impone come primo atto di ogni sessione e che finora dipendeva dal ricordarsene.

*Lo scarto trovato prima di poter istanziare, che è la parte non prevista.* Il modello del pacchetto qui presente portava tre hook su sette, cioè soltanto `protect-sensitive-files`, `secret-scan` e `session-context`, e aveva README e entrambi i frammenti di impostazioni divergenti da quelli del template. È un'altra occorrenza del verso discendente che MS-127 descrive, e la sua particolarità è che nessun presidio poteva vederla: `check-copie-modelli.py` confronta il modello con la copia istanziata dentro questo repository, e qui la copia istanziata non esisteva, quindi il confronto non aveva nulla da fare e riportava quindici copie allineate. Ne segue un limite generalizzabile di quel presidio, che vale la pena enunciare perché non è un difetto da correggere ma una proprietà da conoscere: un controllo che confronta due esemplari dentro lo stesso repository non può accorgersi che siano indietro entrambi rispetto a un terzo, e il verso discendente resta scoperto per costruzione. La voce è stata aggiunta a PA-003.

Riallineato il modello, il confronto con il template non riporta alcuna differenza, e dopo l'istanziazione `check-copie-modelli.py` riporta ventinove copie allineate contro le quindici di prima, zero divergenti.

*La decisione, e perché tre hook restano spenti.* L'utente ha scelto di attivare i quattro che qui pagano davvero, cioè `session-context` e `apertura-sessione` all'apertura, `md-unwrap-auto` sulle scritture e `protect-sensitive-files` come difesa in profondità. I tre spenti lo sono per due ragioni distinte e vanno scritte, perché una decisione di non attivare è un esito e non un vuoto.

`pre-commit-checks` e `secret-scan` intercettano un `git commit` eseguito dall'agente attraverso lo strumento Bash, e in questo progetto quel comando è negato dalla baseline dei permessi, perché commit e push restano gesti dell'utente nel proprio terminale. Nessun hook di Claude Code vede un comando digitato là, quindi i due non scatterebbero mai: attivarli darebbe l'impressione di una protezione che non esiste, che è peggio di non averla. La via per coprire davvero quel caso sarebbe un hook nativo di git, cioè `core.hooksPath`, e non è stata intrapresa.

`chiusura-sessione` è spento per una ragione più sottile e più interessante, ed è un conflitto fra il pacchetto e una prescrizione di questo progetto. Quell'hook registra l'impronta di ripresa a fine sessione; il `CLAUDE.md` prescrive che l'impronta si registri dopo i commit dell'utente e dopo l'aggiornamento del file di ripresa, e dichiara che registrarla prima produce un falso positivo alla riapertura, cioè esattamente il genere di rumore che insegna a ignorare un presidio. Un hook che scatta alla chiusura non sa se i commit siano stati fatti, quindi registrerebbe uno stato che il file di ripresa non descrive ancora. La rete che quell'hook offre, cioè coprire la chiusura distratta, vale meno del danno che farebbe qui, e la decisione può essere rivista se la registrazione manuale si rivelerà dimenticata spesso.

*La prova di ciascuno dei quattro, e perché non bastava vederli registrati.* Il README del pacchetto avverte che un hook che non trova il proprio strumento non fallisce, esce con codice zero senza fare niente, e che un hook silenzioso è indistinguibile da uno che ha lavorato: nel bundle di origine due hook giravano a vuoto a ogni sessione senza che nulla lo dicesse. La verifica è stata quindi fatta eseguendo, non leggendo.

`session-context` stampa ramo, ultimi commit e file modificati, tutti corretti. `apertura-sessione` esegue la verifica di ripresa e riporta le tre divergenze reali del momento, cioè i tre commit comparsi dopo l'ultima registrazione, l'albero di lavoro diverso da quello registrato e lo snapshot di memoria che dichiara un commit più vecchio di `HEAD`: sono corrette tutte e tre, perché l'impronta risale al giorno prima e il lavoro è in corso, quindi l'hook misura e non rumoreggia. `protect-sensitive-files` è stato provato nei due versi, cioè blocca una scrittura su `.env` con codice di uscita 2 e messaggio, e lascia passare sia `.env.example` sia un file Markdown.

`md-unwrap-auto` ha richiesto la prova più attenta, perché sul primo tentativo è uscito zero senza stampare nulla, che è precisamente lo stato ambiguo di cui il README avverte. La distinzione è stata fatta costruendo un file con un paragrafo davvero spezzato su tre righe sorgente e dandolo in pasto all'hook: il file è passato da cinque righe a tre e il paragrafo è tornato su una riga sola. L'hook funziona e tace quando non c'è nulla da fare, che è il comportamento giusto; senza quel caso costruito non si sarebbe potuto distinguerlo da uno rotto.

Verificato con: `diff -rq` fra il modello qui e quello del template, prima e dopo il riallineamento, che passa da nove differenze a nessuna; `diff -rq` fra il modello e la copia istanziata, identici; `check-copie-modelli.py`, che riporta ventinove copie allineate e zero divergenti; lettura del `settings.json` con verifica che i quattro percorsi esistano davvero sul disco e che nessuno dei tre hook esclusi vi compaia; esecuzione diretta dei quattro hook con gli esiti descritti sopra, compresi i due versi di `protect-sensitive-files` e il caso costruito di `md-unwrap-auto`; catena completa di verifica prima del commit.

Esito: fatto, e PA-016 si chiude. Da questa sessione in avanti l'apertura di una sessione stampa da sé lo stato e il confronto con l'impronta registrata, quindi la procedura di ripresa smette di dipendere dal ricordarsene; resta dell'agente il passo che nessun hook può fare, cioè invocare la skill che interpreta la divergenza. La registrazione dell'impronta resta interamente manuale per la scelta descritta sopra.
