...
*Il trasferimento dati fra AKABAK e VACS su Linux non passa dalle pipeline COM.* È il fatto più importante dei sette, e ha una sezione propria. La formulazione di questa riga è stata corretta il 2026-09-10: diceva che passa dagli appunti, che è vero ma incompleto, perché la guida del programma documenta due alternative a COM e non una.
...
Vale precisare che cosa faccia COM in questo caso concreto, perché detto in astratto sembra un dettaglio di implementazione e invece decide la forma del lavoro quotidiano. Quando AKABAK finisce di calcolare una osservazione spettrale, non scrive un file e non chiede nulla: chiama un metodo su un oggetto che vive dentro l'altro processo, e VACS riceve le curve e le disegna. La guida lo dice nei termini del meccanismo, cioè che in quello scenario VACS fa da server COM, e ne descrive la conseguenza più visibile, cioè che VACS si avvia da sé se non è già aperto. Il ricalcolo aggiorna le curve già presenti. L'operatore, in breve, non trasferisce nulla: guarda due finestre che si parlano.

Wine implementa una parte di COM, ma non tutto, e in particolare l'infrastruttura che serve alla comunicazione fra processi distinti attraverso questo canale non è completa. L'autore lo constata sul suo software in una email e indica la via alternativa che gli viene in mente, cioè gli appunti di sistema; la guida del prodotto, letta il 2026-09-10 in MS-090, dichiara la stessa cosa in forma più ampia e più autorevole, perché nomina Linux come esempio di sistema che non supporta il trasferimento COM e documenta due alternative invece di una.

Le tre vie sono quindi `COM`, `Clipboard` e `Files`, e la scelta si fa in due posti. Il default globale sta nel menu `Options/Preferences`, pagina `VACS`. La sovrascrittura per singola osservazione sta nella pagina `Range` della form di quella osservazione, il che significa che la scelta globale è un default e non un vincolo. L'emissione avviene alla fine di un calcolo, oppure a richiesta con il menu `Processing/Output Spectra`.

Con `Clipboard` le curve finiscono negli appunti e nient'altro accade: si seleziona VACS e si usa il suo menu `Edit/Paste`, e conviene avere creato prima una form di grafico vuota; all'aggiornamento basta incollare di nuovo, perché VACS cerca da sé le curve da aggiornare. Con `Files` i dati vanno su disco con nomi generati dal programma, nella cartella del progetto se non se ne specifica un'altra, e in VACS si importano con il menu `IO/Import Data`, che accetta più file in un colpo e applica lo stesso meccanismo di aggiornamento. In entrambi i casi il formato del dataset segue lo schema `VACS - Import Control Setting`, documentato nella guida di VACS.

Una terza fonte sottostima le vie nello stesso verso delle altre due, ed è stata osservata il 2026-09-14 dentro il programma: il testo informativo stampato nella pagina `VACS` delle preferenze dichiara che la comunicazione è o automatica tramite COM o manuale tramite appunti, e non nomina i file, mentre il menu a tendina che gli sta sotto li offre. Le fonti che sottostimano sono quindi tre, cioè l'email dell'autore, una pagina della guida e il testo dentro il programma; quelle che concordano sulle tre vie sono due, cioè il capitolo dell'appendice e il controllo reale. Fra una descrizione e la cosa descritta decide la cosa.

Fra le due alternative questo progetto sceglie `Files`, ed è ADR-020, impostata e verificata il 2026-09-14 in MS-094. Le ragioni sono la riproducibilità, dato che un file sopravvive alla sessione e gli appunti no; l'ispezionabilità dello stato intermedio, che è un principio già scritto altrove in questo sistema di progetto; la difesa dall'errore silenzioso descritto nel paragrafo sul metodo qui sotto; e la traccia della convergenza attraverso le molte iterazioni della fase 5, che con gli appunti non esisterebbe.

Questo non è un difetto di configurazione da risolvere: è una proprietà dell'ambiente, dichiarata dall'autore del software e confermata dalla guida del prodotto. Va quindi accettata e pianificata, non inseguita. La conseguenza sul workflow della fase 5 è concreta e non cambia con la scelta del modo: il passaggio dei risultati dal solutore allo strumento di analisi resta un passo che l'operatore deve compiere, sia esso un incollaggio o una importazione, quindi va tenuto in conto nel tempo di ogni iterazione, e le iterazioni della fase 5 sono molte, perché il ciclo consiste nel modificare parametri e materiali finché la risposta non rientra nella tolleranza dichiarata. Ciò che cambia con `Files` non è il numero di passi ma la loro tracciabilità.

C'è anche una conseguenza sul metodo, che vale enunciare perché è la ragione principale della scelta di ADR-020. Un passo manuale ripetuto molte volte è un punto in cui si commettono errori silenziosi, per esempio portare in VACS il risultato di una simulazione precedente credendo che sia quella appena eseguita. La differenza fra le due alternative sta esattamente qui: negli appunti un dato vecchio è indistinguibile da uno nuovo, quindi l'errore non è rilevabile, mentre un file porta un nome e una data di modifica, quindi l'errore diventa visibile e la disciplina di verifica ha qualcosa su cui appoggiarsi.
...
| Limite delle pipeline COM su Linux, e le due alternative | assente del tutto | dichiarato dall'autore, poi documentato dalla guida del prodotto |
...
*9 e 10 settembre 2026.* La reinstallazione pulita del sistema mette alla prova sperimentale l'affermazione su cui poggiava tutta la decisione, e la conferma. Il prefix `~/.wine` sopravvive all'azzeramento della radice perché vive in `/home`, contiene i due programmi installati e la licenza attiva, e aperto sotto Wine 10 dopo una copia di sicurezza da 831 MB si migra senza rompersi: la reinstallazione e la riattivazione previste dalla procedura diventano inutili. L'avvio richiede però il comando `wine32` e non `wine`, perché su Ubuntu quest'ultimo è uno script che sceglie il caricatore a 64 bit ogni volta che `wine64` esiste, ed è ADR-019.

Sulla licenza il quadro si chiude con due letture indipendenti del 10 settembre. La prima è documentale: l'attivazione non sta nel registro del prefix ma nel file `C:\ProgramData\RDTeam\Akabak.ini`, a livello di macchina, e il codice scritto là coincide con quello conservato; è la collocazione che spiega perché la migrazione del prefix non l'abbia toccata. La seconda è in interfaccia: la finestra del release code dichiara `Release Code valid` con il Machine Identifier `C937-FD3A` invariato, e la finestra di informazioni sul programma dichiara `Standard Edition`, `Valid Release Code`, il profilo `NT 10.0 (Build 19043)` e la memoria `1876 / 2047 MBytes`. L'edizione Standard è quindi confermata dopo la reinstallazione e non solo prima, che è l'unica conferma che provi qualcosa sulla sua stabilità. Il racconto sta in MS-085 e MS-089.

Va ritirata invece una lettura intermedia che avevo registrato il 9 settembre, cioè che la barra del titolo del programma, riportando `32 Professional`, contraddicesse l'edizione Standard. Non la contraddice, perché quella stringa non dichiara l'edizione di AKABAK: la contraddizione era mia e nasceva dall'avere trattato due stringhe come affermazioni sullo stesso attributo per la somiglianza di una parola.
