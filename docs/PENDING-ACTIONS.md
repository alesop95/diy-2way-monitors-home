...
Aperta il 2026-09-21, ed è il completamento della voce precedente più che una voce nuova. La quinta causa portata al template il 17 settembre prescrive che un blocco contenente `ssh` dichiari di essere per la macchina di partenza, e quella prescrizione è stata rispettata il 2026-09-21 mentre l'errore accadeva ugualmente: il blocco era per la postazione, la prosa accanto lo diceva, e l'utente lo ha incollato in un terminale già aperto sulla macchina di destinazione, dove l'alias non esiste. Il pezzo mancante è che una dichiarazione scritta accanto al blocco non viaggia con il blocco, perché chi copia copia il riquadro e non il paragrafo che lo precede, ed è la stessa ragione per cui la terza causa prescrive di mettere il `cd` dentro il blocco invece di scriverlo in prosa.

Il testo da portare al template dice due cose. La prima è che, quando il lavoro si svolge su una macchina remota, la forma predefinita da consegnare è quella che si esegue su quella macchina, senza `ssh`, perché è la forma che funziona nel terminale dove il lavoro accade; la forma con `ssh` si consegna soltanto quando chi esegue ha dichiarato di trovarsi sulla macchina di partenza, e non per difetto. La seconda è che, quando la forma con `ssh` serve davvero, la dichiarazione della macchina va dentro il blocco come commento sulla prima riga, così da viaggiare con ciò che si copia. Il caso è registrato in MS-152, e come le altre voci di questa pendenza si chiuderà con una unione e non con un commit.

Aperta il 2026-09-21, come candidata e non come presidio già progettato, ed è la terza voce nata dalla stessa giornata. Nessun controllo verifica che un fronte operativo abbia prodotto la propria pagina prescrittiva. La regola che la impone è arrivata il 2026-09-17, la catena di controlli prima di un commit conta sette comandi e nessuno di essi la guarda, e il caso reale è che il fronte della catena audio è rimasto quattro giorni senza pagina senza che nulla lo segnalasse: la lacuna è stata colta il 2026-09-21 scrivendo MS-156, cioè per iniziativa e non per misura. È la stessa forma del buco che `check-copie-modelli.py` ha chiuso lo stesso 17 settembre, dove due esemplari dello stesso strumento non avevano nulla che li tenesse insieme, e ha la stessa conseguenza: il difetto non si manifesta qui ma nel prossimo progetto allineato, che riceve una prescrizione senza il presidio che la rende osservabile.

Perché va dichiarata candidata. Che cosa conti come fronte operativo è un giudizio e non un fatto leggibile dal disco, e un controllo che deve giudicare produce falsi positivi, che è il modo documentato in cui un presidio si impara a ignorare. La forma diretta, cioè chiedere se ogni fronte abbia la sua pagina, non è implementabile per questa ragione. La forma indiretta che ha qualche speranza di funzionare è segnalare quando un microstep nuovo nomina comandi eseguiti su una macchina e nessuna pagina sotto `docs/` risulta toccata nello stesso commit, con la possibilità di dichiarare l'eccezione dentro il microstep stesso, come già avviene per il legame con la fase del progetto quando la fase non c'è. Resta da decidere se il posto giusto sia un controllo nella catena oppure un hook di commit, e la decisione appartiene al template perché là vive la regola. Il caso è registrato in MS-157.

Aperta il 2026-09-22, come constatazione e non come correzione già progettata. Il presidio `check-copie-modelli.py`, nato qui e portato al template, confronta il modello di un pacchetto con la sua copia istanziata dentro lo stesso repository, e per costruzione non può accorgersi che i due esemplari siano indietro entrambi rispetto al template. Il caso reale è del 2026-09-22 ed è in MS-164: il modello di `hooks-starter` qui presente portava tre hook su sette con README e frammenti divergenti, e il controllo riportava quindici copie allineate e zero divergenti, perché la copia istanziata non esisteva e non c'era nulla da confrontare. Il verso discendente resta quindi scoperto da qualunque controllo interno al progetto, ed è la stessa asimmetria che MS-127 aveva enunciato in astratto. La forma di un presidio possibile non è ovvia e va decisa nel template, perché richiede di conoscere il percorso del template da dentro un progetto istanziato, che oggi nessun file dichiara.
...
Voce discendente compiuta il 2026-09-22, ed è MS-163. Il quarto asse è re-istanziato in `.claude/rules/git-identity-and-repo.md` con i valori letti sulla postazione e non con quelli generali del template, come la natura istanziata di quel file richiedeva: GitHub CLI 2.100.0 presente e autenticata sull'account `alesop95`, token nel portachiavi e non in un file, ambiti `gist`, `read:org` e `repo`, e i quattro assi che su questa macchina coincidono tutti sull'identità personale, come fatto misurato e non come necessità. L'avvertenza del template sul riconoscimento del repository attraverso un alias SSH è stata verificata e qui non si verifica, quindi il flag `-R` resta con una motivazione diversa da quella originale. L'autorizzazione OAuth è registrata fra le operazioni manuali in `docs/90-riferimenti/licenze-e-registrazioni.md` con ambiti, luogo del token e pagina di revoca. Resta quindi aperto il solo quinto criterio, cioè la decisione su `hooks-starter`, e la condizione che ne rimandava la posa non esiste più.

COMPIUTA il 2026-09-22, ed è MS-164. Il quinto criterio è soddisfatto: l'utente ha deciso di istanziare `hooks-starter` attivando quattro hook su sette, cioè `session-context` e `apertura-sessione` all'apertura, `md-unwrap-auto` sulle scritture e `protect-sensitive-files` come difesa in profondità. I tre spenti lo sono con motivo registrato: `pre-commit-checks` e `secret-scan` non scatterebbero mai, perché intercettano un `git commit` dell'agente che la baseline dei permessi nega e che qui è comunque un gesto dell'utente nel proprio terminale, invisibile a qualunque hook di Claude Code; `chiusura-sessione` è spento perché registrerebbe l'impronta senza sapere se i commit siano stati fatti, producendo il falso positivo che il `CLAUDE.md` prescrive di evitare. Ciascuno dei quattro attivi è stato eseguito e non soltanto registrato, compresi i due versi di `protect-sensitive-files` e un caso costruito per `md-unwrap-auto`, perché un hook che non trova il proprio strumento esce zero senza fare nulla.
...
Data di apertura: 2026-09-21. Stato: COMPIUTA lo stesso giorno. La misura che la apre è MS-145, l'esecuzione è MS-151.
...
Esito del 2026-09-21. La decisione dell'utente è stata la rimozione di entrambi, con il motivo che MS-145 aveva misurato: per Ubuntu 26.04 WineHQ non pubblica alcuna metà a 32 bit, quindi conservare quel repository non conserva alcuna capacità utile a un corredo interamente a 32 bit. Prima di rimuovere è stato accertato che la chiave fosse referenziata dal solo file di sorgenti che spariva con essa e che nessun pacchetto di quella provenienza fosse installato. Dopo la rimozione le sorgenti sono tre, la cartella delle chiavi è vuota, una ricerca di `winehq` sotto `/etc/apt/` non trova nulla, `winehq-stable` non è più conosciuto da apt, e i sette pacchetti Wine della distribuzione sono intatti alla versione `10.0~repack-12ubuntu1` con il prefix a 32 bit che esegue con stato zero. Il racconto, compreso un mio errore nella consegna dei comandi che ha fatto fallire il primo tentativo senza conseguenze, è in MS-151 e MS-152.
...
Esito del 2026-09-21. Tre criteri su quattro sono soddisfatti e il quarto va riformulato invece che spuntato. Collegate le cuffie al jack analogico posteriore, i profili analogici della scheda sono passati da non disponibili a disponibili e PipeWire è passato da solo all'uscita analogica, con porta attiva `analog-output-lineout` e sink predefinito reale al posto di `auto_null`: il rilevamento della presenza sul jack funziona, e l'assenza di uscite misurata in MS-146 descriveva fedelmente una presa vuota. La riproduzione di prova è stata udita. L'ordine dei canali è stato verificato con una sequenza a pausa iniziale dichiarata e risulta corretto, cioè primo tono a sinistra e secondo a destra, dopo una prima prova invalidata da due riproduzioni sovrapposte per mio errore. La simmetria fra i canali è stata misurata su richiesta dell'utente e non presenta alcuno scarto, né nel sink di PipeWire né su alcun controllo stereo del mixer della scheda.
...
Condizione di sblocco soddisfatta il 2026-09-21: il dispositivo è un Samsung Galaxy S25 Ultra con sistema aggiornato. Da qui discende la via, e la scelta merita tre righe perché non è quella che il titolo della voce faceva prevedere.

Il dato sul sensore, promosso da inferenza a fatto il 2026-09-21 con una ricerca. Il Galaxy S25 Ultra non ha un sensore LiDAR né un sensore a tempo di volo dedicato: Samsung ha abbandonato l'hardware di profondità dedicato dopo la serie S20, e sui modelli Ultra successivi la profondità si ricava dall'autofocus laser e dall'elaborazione su più fotocamere. Il dispositivo supporta però l'interfaccia di profondità di ARCore, che è il requisito che le applicazioni di scansione pongono davvero. Le fonti sono nel registro, e resta valida la verifica da trenta secondi sul dispositivo, che costa meno di quanto costi discutere se la fonte sia aggiornata.

L'applicazione scelta, con il motivo della scelta e quello delle esclusioni. Si usa *Scaniverse*, di Niantic Spatial, per tre ragioni che la documentazione del produttore dichiara: i requisiti su Android sono Android 7 o superiore, 4 GB di memoria e il supporto ad ARCore con l'interfaccia di profondità, che questo dispositivo soddisfa; la modalità fotogrammetrica esiste ed è dichiarata per i dispositivi privi di sensore di profondità; e l'esportazione delle mesh comprende OBJ, con elaborazione interamente sul dispositivo, quindi senza crediti né code di rendering. Polycam è esclusa perché il suo piano gratuito esporta soltanto in GLTF e OBJ richiede un abbonamento, e perché il suo supporto Android è dichiarato meno curato di quello iOS. KIRI Engine resta come riserva, con tre esportazioni gratuite a settimana, che per una stanza bastano. RealityScan è esclusa perché è orientata agli oggetti con molta trama e non agli ambienti.

Due accorgimenti pratici che pesano più della scelta dell'applicazione. Il primo rimedia al difetto strutturale della fotogrammetria, cioè che una parete tinteggiata non offre punti riconoscibili e il modello ne esce bucato proprio dove servono le superfici: si attaccano temporaneamente sulle pareti lisce qualche decina di elementi visibili e irregolari, per esempio foglietti adesivi o crocette di nastro di carta, distribuiti e non allineati, e si tolgono dopo. Il secondo è che il soffitto spiovente va inquadrato deliberatamente, perché è la parte che si tende a saltare ed è quella da cui dipende il comportamento modale.

La conseguenza buona è che questo non riporta il caso alla fotogrammetria pura. ARCore ricostruisce la posa della fotocamera combinando le immagini con i sensori inerziali, quindi la scala esce metrica e non arbitraria, e non serve includere un oggetto di lunghezza nota nell'inquadratura. Resta la deriva di scala su una scansione lunga, che è precisamente ciò che la verifica con il metro laser esiste per cogliere.

Il limite specifico di questa stanza, che non vale in generale e che decide quale applicazione usare. Le applicazioni che producono planimetrie modellano una stanza come pareti estruse verticalmente con un soffitto piano: sono le più comode e sbagliano proprio la caratteristica che qui conta, dato che il soffitto è spiovente e la sua pendenza è ciò che distingue il comportamento modale di questa stanza da quello di una scatola. Serve quindi uno scanner che produca una mesh e non una planimetria.

La via scelta è ibrida e non sceglie fra le due. Il telefono cattura la disposizione reale, cioè dove stanno scrivania, schermo, seduta e mobili, che è la parte che a mano costa ore. Il metro laser prende il guscio, cioè le tre dimensioni principali, le due altezze del soffitto spiovente e la sua pendenza, che sono le quote da cui dipendono le frequenze modali. In Blender si ricalca il modello pulito usando la scansione come sagoma e le quote laser come vincoli. Il metro laser resta un acquisto necessario in ogni caso.

La formulazione generale della condizione, che resta valida per un'altra macchina o un altro dispositivo, era la seguente. Sapere quale dispositivo mobile sia disponibile, perché è il dato che decide la via. Serve sapere se abbia un sensore a tempo di volo e se supporti l'interfaccia di profondità di ARCore: senza almeno la seconda, la scansione degrada a fotogrammetria e conviene valutare il rilievo manuale. La condizione originaria, che resta la formulazione generale, era la seguente. Sapere quale dispositivo mobile sia disponibile, perché è il dato che decide la via. Un iPhone o un iPad Pro dal 12 Pro in avanti porta un sensore LiDAR e rende la scansione la via migliore, con scala metrica per costruzione; un dispositivo Android rende la via possibile ma di resa variabile; l'assenza di entrambi lascia la fotogrammetria, che al chiuso è il caso peggiore del metodo, oppure il rilievo manuale con il metro laser. Il metro laser serve comunque in tutti e quattro i casi per la verifica di scala, quindi il suo acquisto non dipende da questa scelta.
...
Protocollo scritto il 2026-09-21, ed è MS-158. La parte di questa voce che non richiede la stanza è fatta: `docs/35-rilievo-geometrico.md` porta la convenzione di origine e assi, la sequenza con i suoi due punti fragili, le sei tabelle da riempire e il ricalco in Blender, e i suoi campi sono l'unione di ciò che le fasi 1, 2 e 3 pretendono invece di un elenco plausibile. Ne è emerso il campo che si dimentica, cioè il materiale di ogni superficie, che serve al calcolo di Sabine nella fase 3 e è il solo non ricostruibile dopo essere usciti dalla stanza. Restano due cose e nessuna è documentale: il metro laser, che serve alla sola tabella A, e la verifica da trenta secondi di Scaniverse sul dispositivo. Le tabelle da B a E si possono riempire con un metro a nastro prima che il laser arrivi.

Due criteri aggiunti il 2026-09-21 dopo il primo tratto della verifica sul dispositivo, perché questa voce non contemplava né il caso del dispositivo non supportato né quello dell'esportazione a pagamento, ed è MS-159. Il primo è che l'applicazione parta e offra una modalità di cattura che produca una mesh e non soltanto splat gaussiani, dato che all'avvio propone due esperienze e la nuova dichiara le funzioni di `Classic` come non ancora presenti: uno splat non è una mesh e da esso non si ricavano superfici a cui assegnare un assorbimento. Soddisfatto lo stesso 2026-09-21: il selettore di cattura offre `Mesh`, descritta dall'applicazione come modalità classica con più opzioni di esportazione, accanto a `Splat`. Il secondo è che l'esportazione in OBJ sia disponibile senza pagare; l'account non serve, perché lo dichiara l'applicazione stessa per `Classic`, mentre sull'esportazione nessuna fonte del progetto si pronuncia. Se il secondo criterio cade, cambia l'applicazione e non il metodo, con KIRI Engine come riserva già annotata.

Misurato il 2026-09-21 e registrato in MS-161, con una conseguenza sulla scelta della modalità. L'applicazione offre due modalità di elaborazione, cioè `Area` per fusione di profondità, dichiarata migliore per stanze e spazi, e `Detail` per fotogrammetria, dichiarata migliore per oggetti con trama e raccomandata dall'applicazione. Ne segue che su questo dispositivo la via per profondità esiste e la scansione non degrada a fotogrammetria, il che conferma per misura il supporto all'interfaccia di profondità di ARCore; e ne segue che per la stanza va scelta `Area` e non la raccomandata, perché la raccomandazione vale per l'oggetto tipico e non per un ambiente. La domanda con cui questa voce era nata, cioè se ci fosse una modalità basata su sensore, era mal posta: il sensore dedicato non c'è e una via per profondità c'è comunque.

Via della scansione abbandonata il 2026-09-22 per decisione dell'utente, ed è MS-162. Non è accertato che l'applicazione non funzioni su quel dispositivo, perché il rimedio identificato non è stato provato e nessun testo di errore è stato raccolto: è accertato che non ha funzionato nei tentativi fatti. La voce non resta bloccata, perché la scansione era un accorciamento e non un requisito, e il rilievo torna alla quarta via della pagina della fase 2, cioè quella manuale, che il protocollo copre già senza modifiche. Cambia però la natura della voce: non attende più una verifica ma un acquisto, cioè il metro laser, e diventa in questo simile a PA-012. I due criteri aggiunti il 2026-09-21 sulla scala e sull'esportazione decadono con la via che li richiedeva e vanno letti come non più pertinenti invece che come non soddisfatti. KIRI Engine resta la riserva annotata, da valutare solo se il rilievo manuale degli arredi si rivelerà più oneroso del previsto.
