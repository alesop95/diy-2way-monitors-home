...
La quarta è l'equivalente su Android, basato sull'interfaccia di profondità di ARCore o su un sensore a tempo di volo dove presente. Esiste e funziona, ma la resa dipende dal dispositivo molto più che nel caso precedente, quindi non si può raccomandare senza sapere quale sia. Va però tolto un equivoco che il confronto con la via precedente induce: una scansione basata su ARCore non è fotogrammetria pura, perché la posa della fotocamera si ricostruisce combinando le immagini con i sensori inerziali, quindi la scala esce metrica e non va ricavata da un oggetto di riferimento. Ciò che si perde rispetto a un sensore dedicato è la precisione geometrica sulle superfici, non la dimensione assoluta.
