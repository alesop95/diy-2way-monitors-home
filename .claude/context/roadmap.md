...
La quarta priorità è la misura reale della stanza, e accanto a essa va ora nominato il rilievo geometrico, che è cosa diversa e non dipende da alcun acquisto oltre a un metro laser. Il modello tridimensionale della stanza si può costruire adesso, ed è l'unico lavoro di progetto eseguibile mentre la scelta dell'interfaccia resta aperta: le vie per ottenerlo, con il vincolo che una scansione è un riferimento su cui modellare e non il modello, stanno in `docs/30-modellazione-e-simulazione.md`, e la voce è PA-020. Il modello si costruisce prima e si valida dopo, perché il suo criterio di validazione è l'accordo con la misura. È il primo dato vero del progetto e va fatta prima di qualunque altra cosa di progettazione, perché fino ad allora ogni simulazione è priva di riscontro. Si può eseguire con un diffusore qualsiasi come sorgente, quindi non attende l'acquisto dei driver.
...
*L'approfondimento concettuale su elementi concentrati ed elementi di contorno, e il report LaTeX del progetto.* Richiesta esplicita dell'utente il 2026-09-10, con un innesco dichiarato: si fa quando si arriva al progetto dei monitor, cioè alle fasi 4 e 5 del flusso, e non prima. La ragione dell'innesco non è di comodo ed è la stessa per cui la roadmap esiste: un approfondimento scritto prima di avere in mano una simulazione reale spiegherebbe la teoria di due metodi invece della loro applicazione a questo diffusore, che è la sola cosa che l'utente non può trovare in un manuale.

Il contenuto concettuale ha un perimetro già leggibile dal sottotitolo che AKABAK dichiara di sé, cioè `Lumped and Boundary Element Analysis`, e le due metà rispondono a due domande diverse. Il modello a elementi concentrati descrive il diffusore come un circuito equivalente, dove massa, cedevolezza e resistenza meccaniche diventano induttanze, capacità e resistenze, e vale finché le lunghezze d'onda in gioco sono grandi rispetto alle dimensioni delle parti, cioè alle basse frequenze: è il dominio in cui si decidono volume del cabinet, accordo dell'eventuale condotto e comportamento del woofer sotto la risonanza. Il metodo degli elementi di contorno descrive invece la radiazione nello spazio risolvendo il campo sulla sola superficie che irradia, quindi cattura la diffrazione sui bordi del baffle e la direttività fuori asse, che sono precisamente i fenomeni che un circuito equivalente non può vedere e che decidono il comportamento alle frequenze medie e alte. Il punto interessante del progetto è la cerniera fra i due, cioè dove finisce la validità del primo e comincia la necessità del secondo, e quella cerniera dipende dalla geometria reale del cabinet e non da una regola generale.

Il report si produce con il pacchetto `latex` dei modelli ereditati dal template, che in questo progetto non è ancora istanziato: vive sotto `.claude/templates/latex/` e porta gli script `setup-tex` e `build` per Windows e per Linux, il manifesto `tex-packages.txt` dei pacchetti da installare, un `.latexmkrc` che fissa l'engine a `pdflatex`, e la skill `latex-build` che incapsula la procedura. La distribuzione TeX è TinyTeX, installata user-local e condivisa fra i progetti, e non si versiona; si versionano il sorgente, il manifesto e gli script, mentre il PDF e gli ausiliari sono derivati e restano fuori da git. Tre attriti sono già documentati nel README del pacchetto e conviene conoscerli prima invece di scoprirli: `tlmgr` può richiedere un aggiornamento di se stesso prima di installare, il pacchetto `courier` serve se il preambolo unisce `psnfss` a `\texttt` su `fontenc` T1, e se il sorgente non sta nella radice del progetto va passato esplicitamente allo script di build, che usa `-cd` per non sporcare la radice con gli ausiliari.

Due fatti rendono l'innesto meno costoso di quanto sembri, e vanno registrati perché altrimenti la scelta dell'engine si rifà da zero. Il primo è che la catena tipografica del progetto già conosce i file `.tex`: `fix-accents.py`, `fix-missing-accents.py` e `fix-dashes.py` mascherano gli argomenti di un insieme chiuso di macro prima di operare, così da non riscrivere etichette, chiavi bibliografiche e nomi di file inclusi, e `tools/test-tipografia.py` verifica separatamente che gli identificatori restino intatti e che la prosa accanto sia corretta. Il secondo è che il documento sarà in italiano con accenti e formule: `pdflatex` con `fontenc` T1 li gestisce, quindi l'engine di default va bene e il passaggio a `lualatex` o `xelatex` servirebbe soltanto se si volessero `fontspec` o `unicode-math`, che per un report tecnico non sono necessari.

Sulla forma del documento resta una decisione da prendere quando la voce si apre, e non va anticipata adesso: se il report sia un documento unico che racconta il progetto dei monitor dall'inizio alla verifica finale, oppure una relazione per fase con un documento per ciascuna. La prima è migliore per chi legge una volta, la seconda per chi lavora, e la scelta dipende da quanto materiale la fase 5 produrrà, che oggi non si sa.

Una richiesta dell'utente del 2026-09-21 tocca questa direzione senza coincidere con essa, e la distinzione va tenuta perché altrimenti l'innesco dichiarato qui sopra viene aggirato senza deciderlo. L'utente ha chiesto un report in LaTeX sul gain staging della catena di ingresso per l'home recording: è un documento diverso, con un contenuto diverso e un lettore diverso, e appartiene al progetto gemello, dove vive la valutazione dell'interfaccia. Istanziare là il pacchetto `latex` non anticipa la voce di questo progetto e non la sostituisce; semmai la rende più facile, perché la procedura sarà già stata percorsa una volta. La voce è PA-021.
